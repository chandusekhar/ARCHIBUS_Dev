...
},
{
    ...
    z_MESSAGE_SESSION_EXPIRATION_TITLE: 'Session About to Expire',
    z_MESSAGE_SESSION_TIMEOUT_TITLE: 'Session Timeout',
    z_MESSAGE_SESSION_EXPIRATION: 'Your session will expire soon. Do you wish to extend the session?',
    ...
    /**
     * Handles session expiration notification. The server may notify the client that the session is about to expire.
     */
    handleSessionExpiration: function() {
        var title = View.getLocalizedString(Ab.workflow.Workflow.z_MESSAGE_SESSION_EXPIRATION_TITLE);
        var message = View.getLocalizedString(Ab.workflow.Workflow.z_MESSAGE_SESSION_EXPIRATION);
        Ext.MessageBox.confirm(title, message, function(button) {
            if (button === 'yes') {
                // send an HTTP request to keep the session alive
                View.openDialog('ab-about-and-license.axvw', null, false, {
                    width: 0,
                    height: 0,
