...
Calendar.getControllerForEvent = function (event, anchorName, abSchemaSystemGraphicsFolder, width, height){
	...
	if(valueExistsNotEmpty(this.anchorObject.value)){
		var inputDate =  getDateWithISOFormat(this.anchorObject.value);
		inputDate = inputDate.split('-');
		inputDate[1] = (inputDate[1]+'').replace(/^0/, '');
		inputDate[2] = (inputDate[2]+'').replace(/^0/, '');
		Calendar.formatCalendar(inputDate[0],inputDate[1], inputDate[2], true);
	}
	//show calendar
	this.setCalendarVisibility(true);
};
...
Calendar.formatCalendar = function (year, month, day, isShowTargetDate){
	month = month + "";
	month = parseInt(month, 10);
	...
	if(valueExists(isShowTargetDate) && isShowTargetDate){
		curYear = year;
		curMonth = month;
	}
...
Calendar.getCalendarTop = function (month){
	...
	str = str +'<tr nowrap="1"><td style="padding-top: 5px; padding-left: 4px"><input  id="AFMCALENDAR_but_up" name="AFMCALENDAR_but_year_up" onclick="Calendar.getMM_YYYY(true);return false;" WIDTH="18" HEIGHT="18" HSPACE="1" type="image" src="'+this.abSchemaSystemGraphicsFolder+'/but_prev.gif"/>';
	str = str +'<\/td>';
	str = str +'<td class="calendarmmyyyyA" style="padding-top: 3px">';
	str = str + this.getMonthNames(month);
	str = str +'<\/td><td><\/td>';
	str = str +'<td class="calendarmmyyyyA" style="padding-top: 2px">';
	...
	//str = str +'<BR \/>';
	// use <hr> to remove the space between up/down arrow button for all browsers.
	str = str +'<hr style="height:0px; visibility:hidden; margin-top: -1px; margin-bottom:-10px;">';
	...
	str = str +'<\/td>';
	str = str +'<td style="padding-top: 5px">';
