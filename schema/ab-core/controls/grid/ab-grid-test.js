...
function createGrid() {
	...
	var columns = [
	    new Ab.grid.Column('rm.checked', '', 'checkbox', onCheck),
	    new Ab.grid.Column('rm.selected', '', 'radiobutton', onSelect),
	    new Ab.grid.Column('rm.bl_id', 'Building ID', 'text'),
	    new Ab.grid.Column('rm.fl_id', 'Floor ID', 'text'),
	    new Ab.grid.Column('rm.rm_id', 'Room ID', 'text'),
	    new Ab.grid.Column('rm.area', 'Area', 'number'),
        new Ab.grid.Column('rm.editable', '', 'image', onClick),
	    new Ab.grid.Column('rm.editable', '', 'button', onClick)];
	...
	var grid_simple = new Ab.grid.Grid('grid-simple', configObj);
	grid_simple.build();
	...
	//var grid_complex = new Ab.grid.MiniConsole('grid-complex', rows, columns);

	configObj = new Ab.view.ConfigObject();
	configObj['groupIndex'] = 0;
	configObj['viewDef'] = '';

	// construct the grid without a viewDef
	var grid_complex = new Ab.grid.ReportGrid('grid-complex', configObj);
...
//    grid_complex.selectedColumn = 2;
	...
	var columns2 = [
	    new Ab.grid.Column('', 'One', 'text', null, null, 4),
	    new Ab.grid.Column('', 'Two', 'text', null, null, 4),
	    new Ab.grid.Column('', 'Three', 'text', null, null, 4)];
	var rows2 = [{}];
	configObj = new Ab.view.ConfigObject();
	configObj['rows'] = rows2;
	configObj['columns'] = columns2;

	var grid_2column = new Ab.grid.Grid('grid-2column', configObj);
	for (var i = 0; i < 12; i++) {
	    grid_2column.addColumn(new Ab.grid.Column('', '' + i, 'text', null, null, 1, "25px"), 2);
	}
	grid_2column.build(); 
}
