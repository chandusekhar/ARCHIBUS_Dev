...
var abCoursesByEm = View.createController('abCoursesByEm',{
	...
	catPanel_onCatIdLink: function(row, action){
		...
		if (courseAdminFlag == true) {
			...
			var rest = "exists (select 1 from uc_certifications u inner join em on u.em_id=em.em_id where UC_course_categories.category_id=u.category_id and " + makeLiteralOrNull(View.user.employee.id) + " in (em.em_id,em.reports_to)) and category_id="+makeLiteralOrNull(catId,"=") + ")" 
	...
	coursesPanel_onCourseIdLink: function(row, action){
		if (courseAdminFlag == false) {
			...
			rest += " and exists (select 1 em where uc_certifications.em_id=em.em_id and " + makeLiteralOrNull(View.user.employee.id) + " in (em.reports_to,em.em_id)";
		...
		this.detailsPanelNorth.setTitle(courseId + " - Active Employees" )
		this.detailsPanelCenter.setTitle(courseId + " - Expired Employees" )
		this.detailsPanelSouth.setTitle("Position " + courseId + " - Missing Employees" )
	},
