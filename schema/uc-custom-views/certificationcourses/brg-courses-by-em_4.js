...
var abCoursesByEm = View.createController('abCoursesByEm',{
   ...
   selectEmNew: function(commandObject){					
		var rest = "em.status='A'";
		if (!View.user.isMemberOfGroup('COURSE-ADMIN')) {
			rest += " and " + makeLiteralOrNull(View.user.employee.id) + " in (em.reports_to,em.em_id)"
		}
   ...
   detailsPanelNorth_onViewHistory: function(row)
	...
	addNewCertFormPanel_onSubmit: function() {	
		
		var addForm = this.addNewCertFormPanel;
		var activeGridPanel2 = View.panels.get("detailsPanelNorth");
					
		var dataSource = View.dataSources.get(View.panels.get('addNewCertFormPanel').dataSourceId);		
						
		var em_id = this.addNewCertFormPanel.getFieldValue('UC_certifications.em_id');
		var category = this.addNewCertFormPanel.getFieldValue('UC_courses.category_id');
		var coursename = this.addNewCertFormPanel.getFieldValue('UC_certifications.course_name');
		var startdate = this.addNewCertFormPanel.getFieldValue('UC_certifications.start_date');
		var expirydate = this.addNewCertFormPanel.getFieldValue('UC_certifications.expiry_date');
		var courseid = this.addNewCertFormPanel.getFieldValue('UC_certifications.course_id');
		var renewdate = this.addNewCertFormPanel.getFieldValue('UC_certifications.date_renew');
		var doc = ""; 
		var description = this.addNewCertFormPanel.getFieldValue('UC_certifications.description');
		var certnumber = this.addNewCertFormPanel.getFieldValue('UC_certifications.cert_number');																		
		var certtype = this.addNewCertFormPanel.getFieldValue('UC_certifications.type');
		var statusNew = "A"; 
		...
		if(startdate == ""){
			...
			View.showMessage("Please Enter a Employee ID before saving");
		}
		else{
			...
			this.addNewCertFormPanel.closeWindow();			
	...
	onChangeCourseIdNew: function()
	{
		var restriction = new Ab.view.Restriction();
		var theCourse = this.addNewCertFormPanel.getFieldValue("UC_certifications.course_id");
		if (theCourse == '')
			 this.addNewCertFormPanel.setFieldValue("UC_certifications.course_name", "");
		restriction.addClause('UC_certifications.course_id',theCourse,'=');
		var ds = View.dataSources.get(View.panels.get('addNewFormPanel').dataSourceId);
		var record =  ds.processOutboundRecord(ds.getRecord(restriction));
		var theCategory = record.getValue('UC_courses.category_id');
		...
		this.addNewCertFormPanel.setFieldValue("UC_certifications.course_name", theName);
		this.addNewCertFormPanel.setFieldValue("UC_courses.course_name", theName);
		this.addNewCertFormPanel.setFieldValue("UC_courses.category_id", theCategory);
		//this.addNewFormPanel.setFieldValue("UC_courses.type", theType);
    },
   
	onChangeCourseNameNew: function()
    {
	    var theName = this.addNewCertFormPanel.getFieldValue("UC_certifications.course_name");
	    this.addNewCertFormPanel.setFieldValue("UC_courses.course_name", theName);
    },
	
	onChangeCategoryNew: function(t) //clear course_id
	{
		this.addNewCertFormPanel.setFieldValue("UC_certifications.course_id", "");
		this.addNewCertFormPanel.setFieldValue("UC_certifications.course_name", "");
		if (this.addNewCertFormPanel.getFieldValue("UC_courses.course_id", "") != "") {
			this.disableDetailsPanelField ("UC_certifications.course_name"); 
		}
		else {
			this.enableDetailsPanelField ("UC_certifications.course_name"); 
		}
		if (t==1) { this.addNewCertFormPanel.setFieldValue("UC_courses.type", "-");}
	},
	...
	selectCoursesNew: function(commandObject)
	{
		var theRest = "UC_courses.status='A' and exists (select 1 from uc_course_categories c where c.category_id=uc_courses.category_id and c.status='A')";
		var theCat = this.addNewCertFormPanel.getFieldValue ("UC_courses.category_id");
		//var theType = this.addNewCertFormPanel.getFieldValue ("UC_courses.type");
		if (theCat != '') {theRest = theRest + " and rtrim(UC_courses.category_id) = "+ makeLiteralOrNull(theCat);}
		//if (theType != '-') {theRest = theRest + " and rtrim(UC_courses.type) = "+ makeLiteralOrNull(theType);}
...
function onAddNewCertification() {
		...
		var addCertFormPanel = View.panels.get('addNewCertFormPanel');
		
		addCertFormPanel.show(true, false);
		addCertFormPanel.showInWindow({
			x: 500,
			y: 150,		
			width: 500,
			height: 420,
		...
		/*
		addCertFormPanel.setFieldValue('UC_certifications.em_id', em_id);
		addCertFormPanel.setFieldValue('UC_courses.category_id', category);
		addCertFormPanel.setFieldValue('UC_certifications.course_name', coursename);
		addCertFormPanel.setFieldValue('UC_certifications.start_date', startdate);
		addCertFormPanel.setFieldValue('UC_certifications.expiry_date', expirydate);
		addCertFormPanel.setFieldValue('UC_certifications.course_id', courseid);
		addCertFormPanel.setFieldValue('UC_certifications.date_renew', renewdate);
		addCertFormPanel.setFieldValue('UC_certifications.doc', doc);
		addCertFormPanel.setFieldValue('UC_certifications.description', description);
		addCertFormPanel.setFieldValue('em.name_first', firstname);
		addCertFormPanel.setFieldValue('em.name_last', lastname);
		addCertFormPanel.setFieldValue('UC_certifications.cert_number', certnumber);																		
		addCertFormPanel.setFieldValue('UC_certifications.type', certtype);
		addCertFormPanel.setFieldValue('UC_certifications.status', statusNew);
		*/
		//startdateold=(startdateold.getMonth() + 1) + '/' + startdateold.getDate() + '/' +  startdateold.getFullYear();
		addCertFormPanel.setFieldValue('start_date_old', startdateold);
}
