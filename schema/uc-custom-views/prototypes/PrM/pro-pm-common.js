...
function getSchedDatesRestriction(siteId, blId, flId, pmGroup, trId, pmpType){
    var restriction = '';
    if (siteId) {
        restriction += "EXISTS (SELECT 1 FROM pms ${sql.as} pms1 LEFT JOIN pmp ON pms1.pmp_id = pmp.pmp_id LEFT OUTER JOIN eq ON pms1.eq_id = eq.eq_id WHERE pms1.pms_id = pms.pms_id AND ((pmp.pmp_type = 'EQ' AND eq.site_id = '" + siteId + "'" + ") OR (" +
    ...
    if (blId) {
        ...
        restriction += "EXISTS (SELECT 1 FROM pms ${sql.as} pms2 LEFT JOIN pmp ON pms2.pmp_id = pmp.pmp_id LEFT OUTER JOIN eq ON pms2.eq_id = eq.eq_id WHERE pms2.pms_id = pms.pms_id AND ((pmp.pmp_type = 'EQ' AND eq.bl_id = '" + blId + "'" + ") OR (" +
    ...
    if (flId) {
        ...
        restriction += "EXISTS (SELECT 1 FROM pms ${sql.as} pms3 LEFT JOIN pmp ON pms3.pmp_id = pmp.pmp_id LEFT OUTER JOIN eq ON pms3.eq_id = eq.eq_id WHERE pms3.pms_id = pms.pms_id AND ((pmp.pmp_type = 'EQ' AND eq.fl_id = '" + flId + "'" + ") OR (" +
