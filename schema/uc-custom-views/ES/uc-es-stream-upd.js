...
//this.vnId = '3M CANADA COMPAN';
	...
	bill_form_onSubmit: function(){
		...
		/*
		if (this.bill_line_form.visible){
			this.bill_line_report.refresh()
			this.bill_line_report.show(false,true)
		}
		var lines = this.bill_line_report.rows
		var msg = "Please enter Line Items with a quantity greater than 0 prior to Submitting"
    ...
    selectStream: function(){
		...
		View.selectValue(
			...
			"bill_type.activity_id = 'AbRiskES' and exists (select 1 from vn_ac a inner join uc_vn_acloc l on a.vn_id=l.vn_id and a.vn_ac_id=l.vn_ac_id and l.status='A' inner join bill_unit u on u.bill_type_id=a.vn_ac_id and u.status=1 where a.vn_ac_id=bill_type.bill_type_id and a.vn_id='" + selectedVnId.replace(/'/g, "''") + "')",
			this.onSelectStream,
			true,
			true,
			null,
...
*/
	...
	bill_line_select_location: function(){  
		...
		var vn_id = this.bill_form.getFieldValue("bill.vn_id");
		var rest = " vn_id = '"+vn_id.replace(/'/g, "''")+"' and vn_ac_id = '" + currentVnAcId.replace(/'/g, "''") + "' and uc_vn_acloc.status='A'";
	...
	saveData: function(){
		...
		if (this.bill_form.save()){
			this.makeButtonsEnabled()
			//this.bill_form.actions.get("new").show(true);
			//this.bill_line_report.refresh("bill_line.bill_id='" + this.bill_form.getFieldValue("bill.bill_id").replace(/'/g, "''") + "'")
			...
			/*if (!newRec) {
				...
				//var streamField = this.bill_form.fields.get("bill.vn_ac_id");
				//streamField.config.readOnly = true;
				//streamField.dom.disabled= true;
				//var vnField = this.bill_form.fields.get("bill.vn_id");
				//vnField.config.readOnly = true;
				//vnField.dom.disabled= true;
	...
	validateAndSaveForm: function(){
	   ...
	   //validate location
	   var selectedLocation = this.bill_line_form.getFieldValue("bill_line.loc_id");
	   var currentVnAcId = this.bill_form.getFieldValue("bill.vn_ac_id");
	   //get uc_vn_acloc record for this loc id
	   var acloc_id = BRG.Common.getDataValue('uc_vn_acloc','loc_id', "vn_ac_id='"+currentVnAcId.replace(/'/g, "''")+"' and loc_id = '" + selectedLocation.replace(/'/g, "''") + "'", true);

		if (acloc_id == null){
		  View.showMessage("Invalid location. Please select a valid location.");
		  return;
		}
		 else{
		  //save form
...
function printBill(){
	...
	var reportViewName = "uc-es-stream-print.axvw";
