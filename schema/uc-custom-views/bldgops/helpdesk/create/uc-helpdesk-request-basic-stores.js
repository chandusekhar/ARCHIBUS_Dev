...
var helpDeskCreateBasicController = View.createController("helpDeskBasicController", {
	...
	accountPanel_onCheckacct: function() {
		var valid=1;
		var newacct="";
		
		var busUnit=$('ac_id_part1').value;
		var fund=$('ac_id_part2').value;
		var dept=$('ac_id_part3').value;
		var account=$('ac_id_part4').value;
		var program=$('ac_id_part5').value;
		var internal=$('ac_id_part6').value;
		var proj=$('ac_id_part7').value;
		var activity=$('ac_id_part8').value;
		
		//required fields - can't be left blank
		if (!UC.Data.getDataValue('uc_ac_Business_Unit_PS', 'Business_Unit', "Business_Unit='" + busUnit + "'")) {valid=0;};
		if (!UC.Data.getDataValue('uc_ac_Fund_PS', 'Fund_Code', "Fund_Code='" + fund + "'")) {valid=0;};
		if (!UC.Data.getDataValue('uc_ac_Dept_PS', 'Dept_id', "Dept_id='" + dept + "'")) {valid=0;};
		if (!UC.Data.getDataValue('uc_ac_Acct_PS', 'Acct_Code', "Acct_Code='" + account + "'")) {valid=0;};
		//optional fields. change to 0's if left blank
		if (!program) {program='00000'};
		if (!UC.Data.getDataValue('uc_ac_Program_PS', 'Program_Code', "Program_Code='" + program + "'")) {valid=0;};
		if (!internal) {internal='XXXXXXXXX'};
		if (!UC.Data.getDataValue('uc_ac_Internal_PS', 'Internal_Code', "Internal_Code='" + internal + "'")) {valid=0;};
		if (!proj) {proj='00000'};
		if (!UC.Data.getDataValue('uc_ac_Project_PS', 'Project_Code', "Project_Code='" + proj + "'")) {valid=0;};
		if (!activity) {activity='00000'};
		if (!UC.Data.getDataValue('uc_ac_Proj_Activity_PS', 'Activity_ID', "Activity_ID='" + activity + "'")) {valid=0;};
		
		//alert(valid);
		
		if (valid) {
			
			newacct = busUnit + "-" + fund + "-" + dept + "-" + account + "-" + program + "-" + internal + "-" + proj + "-" + activity;
			//alert(newacct);
			//check to see if the merged account code is in archibus.
			if(!UC.Data.getDataValue('ac', 'ac_id', "ac_id='" + newacct + "'")) {
				//if the merged account code is not in archibus, insert it in.  Build the account description.
				var description = UC.Data.getDataValue('uc_ac_Fund_PS', 'Fund_Desc', "Fund_Code='" + fund + "'") + "-";
				description += UC.Data.getDataValue('uc_ac_Dept_PS', 'Dept_Desc', "Dept_id='" + dept + "'") + "-";
				description += UC.Data.getDataValue('uc_ac_Acct_PS', 'Acct_Desc', "Acct_Code='" + account + "'") + "-";
				description += UC.Data.getDataValue('uc_ac_Program_PS', 'Program_Desc', "Program_Code='" + program + "'") + "-";
				description += UC.Data.getDataValue('uc_ac_Project_PS', 'Project_Desc', "Project_Code='" + proj + "'") + "-";
				description += UC.Data.getDataValue('uc_ac_Proj_Activity_PS', 'Activity_Desc', "Activity_ID='" + activity + "'");
				
				var dsAC = new Ab.data.createDataSourceForFields({
				   id: 'acDS',
				   tableNames: ['ac'],
				   fieldNames: ['ac.ac_id', 'ac.description']
				});
				

				var saveRecord = new Ab.data.Record();
				saveRecord.isNew = true;
				saveRecord.setValue('ac.ac_id', newacct);
                saveRecord.setValue('ac.description', description.toUpperCase());

				dsAC.saveRecord(saveRecord);
				//alert("Account Created");
				this.accountPanel.setFieldValue("activity_log.ac_id", newacct);
				
				//alert (description.toUpperCase());
			} else {
				//if the merged account code is in archibus, we are OK! Send a positive message
				this.accountPanel.setFieldValue("activity_log.ac_id", newacct);
				//alert("Account Found");
			}
			...
			this.accountPanel.setFieldValue("activity_log.ac_id", "error");
			//return a failure message if account isn't valid
    ...
    checkBasicForm: function(){
		...
		//JJYCHAN - ADD A CHECK FOR THE ACCOUNT CODE.
		this.accountPanel_onCheckacct();
		
		
		//UC-JJYCHAN - New field for professorValue
		var professorValue = this.accountPanel.getFieldValue("activity_log.prof");
		if(professorValue == '' || professorValue == undefined || professorValue == null)
		{
				View.showMessage(getMessage("noAccountHolder"));
				
				//KB3039310 - change dom value and class name because core UI change in v21.1
			  	var fieldInputTd = $("activity_log.prof").parentNode.parentNode;
			...
			// add the funder information to the description
			
		}
		var professorNumber = this.accountPanel.getFieldValue("activity_log.phone_prof");
		var professorEmail = this.accountPanel.getFieldValue("activity_log.email_prof");
        ...
        var jobSubjectValue = this.descriptionPanel.getFieldValue("activity_log.job_subject");
		var description_box = this.descriptionPanel.getFieldValue("activity_log.description_temp");
		
		if (description_box == undefined || description_box == '' || description_box == null) {
			//if there is no description, throw an invalid field.
			this.descriptionPanel.addInvalidField("activity_log.description_temp", getMessage("noDescription"));
            this.descriptionPanel.displayValidationResult();
            return false;
		} else {
			var full_description = "Job Subject: " + jobSubjectValue + "\r\n" + " " + description_box + "\r\n" + " Account Holder: " + professorValue + "\r\n" + " Account Phone: " + professorNumber + "\r\n" + " Account Email: " + professorEmail;
			this.descriptionPanel.setFieldValue("activity_log.description", full_description);
		}
...
function showSlaParameters() {
	...
	var recordValues = ABHDC_getDataRecordValues("basicDs1"); 
