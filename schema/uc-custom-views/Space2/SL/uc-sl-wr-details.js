...
var managerDetailsController = View.createController('managerDetailsController', {
	...
	afterViewLoad: function() {
		...
		this.details_tabs.setTabRestriction('det_dwg', 'wr_id='+wr_id);

		this.details_tabs.setTabRestriction('det_report', 'exists (select 1 from wr where wr_id='+wr_id+' AND wr.bl_id = rm.bl_id and wr.fl_id = rm.fl_id and wr.rm_id = rm.rm_id)');
	...
	details_tabs_beforeTabChange: function(tabPanel, currentTabName, newTabName)
	{
		...
		if (currentTabName == "det_info")
		{
			...
			if (thisController.infoTabController.checkFormChanged())
			{
				changeTab = false;
