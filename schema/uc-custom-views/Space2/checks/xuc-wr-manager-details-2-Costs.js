...
{
	var panel = View.getControl('', 'wrcfEditPanel');
	var record = costTabController.gettingRecordsData(panel, 'wrcf');
	var parameters = {
		fields: record
	};
	var result = Workflow.runRuleAndReturnResult("AbBldgOpsOnDemandWork-saveWorkRequestCraftsperson",parameters);
	
	// refresh Grid (done here so we can pass in the restriction)
	View.getControl('', 'wrcfReportGrid').refresh(View.restriction);
}
...
{
	var panel = View.getControl('', 'wrtlEditPanel');
	var record = costTabController.gettingRecordsData(panel, 'wrtl');
	var parameters = {
		fields: record
	};
	var result = Workflow.runRuleAndReturnResult("AbBldgOpsOnDemandWork-saveWorkRequestTool",parameters);
	
	// refresh Grid (done here so we can pass in the restriction)
	View.getControl('', 'wrtlReportGrid').refresh(View.restriction);
}
...
{
	var panel = View.getControl('', 'wrOtherEditPanel');
	var record = costTabController.gettingRecordsData(panel);
	
	var parameters = {
		fields: record
	};
	var result = Workflow.runRuleAndReturnResult("AbBldgOpsOnDemandWork-saveOtherCosts",parameters);
	
	// refresh Grid (done here so we can pass in the restriction)
	View.getControl('', 'wrOtherReportGrid').refresh(View.restriction);
}
	...
	if (deleteOK) {
		...
		View.confirm(getMessage('confirmDelete'), function(button) {
			if (button == 'yes') {
				...
				var result = Workflow.runRuleAndReturnResult('AbBldgOpsOnDemandWork-deleteItems', parameters);
				if (result.code == 'executed') {
						grid.refresh(View.restriction);
				}
				else {
