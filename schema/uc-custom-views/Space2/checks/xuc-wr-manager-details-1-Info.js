...
var infoTabController = View.createController('infoTabController', {
	...
	updateAcctDesc: function() {
		...
		$('Shownav_details_info_ac.description').innerHTML = acctDescription;
	},
	...
	cancelWorkRequest: function() {
		...
		try {
			...
			/* NOT SURE WHY THIS IF STATEMENT IS NECESSARY
			...
			*/
			...
			setTimeout("View.getControl('','wr_details_frame').frame.dom.contentWindow.location.href = '';",2000);
			*/
	...
	completeWorkRequest: function() {
		...
		var rest = "wr_id="+wrId+" AND cost_total <= 0";
		var rest_other = "wr_id="+wrId+" AND fulfilled='0'";

		// check wrcf
		if (readyToComplete) {
			record = UC.Data.getDataRecord('wrcf', ['wr_id'], rest);
			if (record != null) {
		...
		// check wrpt
		if (readyToComplete) {
			record = UC.Data.getDataRecord('wrtl', ['wr_id'], rest);
			if (record != null) {
				readyToComplete = false;
			}
		}
	...
	issueWorkRequest: function() {
		...
		if (this.nav_details_info.record.getValue('wr.activity_log_id') == "" &&
			...
			try {
				Workflow.call('AbBldgOpsOnDemandWork-issueWorkorder', parameters);
				this.refreshSearchGrid();
				success = true;
			}
			catch (e) {
	...
	createSubReq_callback: function(subWrId) {
		if (subWrId != undefined && subWrId != null) {
			...
			this.ds_wr_details.saveRecord(newRec);
		...
		*/
		...
		if (sendEmail && sendToEmail != null) {
			...
			var emailBody = "The following Work Request has been Rejected.<br>" +
							...
							"contact FMD Customer Care at fmdcustomercare@ucalgary.ca or 403-220-7777.<br><br>" + emailMessage;
			//email Footer
