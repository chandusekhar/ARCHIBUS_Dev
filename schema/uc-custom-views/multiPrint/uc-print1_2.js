	...
	if(result.code == 'executed'){
		...
		for(var j=0;j<results.length;j++){
			...
			for(var i = 0, len = aValidFields.length; i < len; i++){
				...
				if(aValidFields[i]=="wr.date_requested"){
					...
					document.getElementById(aValidFields[i]).innerHTML = replaceLFwBR(record[aValidFields[i]]);
				}
			}

			if(record['wr.eq_id']=="" || record['wr.work_team_id']=="FLEET") {//2012.02.29 hide the eqInfoRow if the wr.eq_id not exists
				document.getElementById('eqInfoRow').style.display = "none";
                document.getElementById('eqQuestionRow').style.display = "none";
			}
			if(record['wr.work_team_id']!="FLEET") {
				document.getElementById('vehicleInfoRow').style.display = "none";
                document.getElementById('tblFleet').style.display = "none";

			}
			else {
				document.getElementById('tblMaint').style.display = "none";
				document.getElementById('facMgmtTitle').innerHMTL = "Fleet Management";
				document.getElementById('locTitle').innerHMTL = "Vehicle:";
				document.getElementById('facMgmtTitle').innerText = "Fleet Management";
				document.getElementById('locTitle').innerText = "Vehicle:";
			}

			arrDs = View.dataSources.items


			var wr_id=record['wr.wr_id'];
			rest="wr.wr_id='" +wr_id+"' "
...
function buildChildTableFromDataSource(rest,dsItem){
	...
	if (typeof(rest) == "string") {
		...
		restriction = "wr.wr_id =" + rest['wr.wr_id']
	}
	var divrest
	...
	var restriction = "exists (Select 1 from wr where " + divrest + " AND  " + restriction + ")"
...
*/
	...
	if(tablName == 'wrcf' && records.length < 4){
		createChildTableBlankRows(table, 4 - records.length, cSpan);
	}
