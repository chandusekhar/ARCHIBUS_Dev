...
var infoTabController = View.createController('infoTabController', {
	...
	completeWorkRequest: function() {
		...
		//////////////////////////////////////////////////////////////////////////////////////////////
		var restCF = "wr_id="+wrId+" AND cost_total <= 0 AND (SELECT tr_id FROM wr WHERE wr.wr_id = wrcf.wr_id) <> 'RECORDS' AND entry_type = 'Timecard' AND (SELECT rate_hourly + rate_double + rate_over FROM cf WHERE cf.cf_id = wrcf.cf_id) > 0";
	...
	removeStatusEnum: function(statusField) {
		if (statusField != undefined) {
			...
			switch (statusValue) {
				...
				case "AA":
				case "Prj":
					UC.UI.restrictDropDown(statusDropDown, 'AA', 'I');
					UC.UI.removeOption(statusDropDown, 'Prj');
					break;
				case "I":
				case "HL":
				case "HA":
				...
				case "FWC":
                case "HD":
					UC.UI.restrictDropDown(statusDropDown, 'I', 'FWC');
					...
					UC.UI.removeOption(statusDropDown, 'S');
					UC.UI.removeOption(statusDropDown, 'Com');
				...
				case "Com":
					...
					UC.UI.restrictDropDown(statusDropDown, 'FWC', 'Com');
					UC.UI.removeOption(statusDropDown, 'Can');
