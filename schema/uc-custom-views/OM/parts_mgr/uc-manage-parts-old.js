...
//  **
	ucManageParts_topPanel_afterRefresh: function() {
		...
		this.ucManageParts_topPanel.gridRows.each(function(row) {
			...
			// set the restriction
			var rest = "wr_id="+wrId+" AND fulfilled IN (0,1,2,3)";
			...
			if (!record.isNew) {
				...
				if (record.getValue('wr_other.fulfilled') == 0) {
					changeColor= true;
					color = '#cf7d7d'; //red
					weight='bold';
				}
				else if (record.getValue('wr_other.fulfilled') == 2) {
					changeColor = true;
					color = '#cfcc7d';	//yellow
					weight='normal';
				}
				//if parts are on order,
				else if (record.getValue('wr_other.fulfilled') == 3){
					changeColor = true;
					color = '#7d9ecf';	//blue
					weight='normal';
				}
				...
				var date_requested = record.getValue('wr_other.date_used').getTime();
				var today = new Date();
...
//     */
    ucManageParts_bottomPanel_afterRefresh: function() {
        ...
        this.ucManageParts_bottomPanel.gridRows.each(function(row) {
            ...
            var partType = row.getRecord().getValue('wr_other.other_rs_type');
			...
			else if (row.getRecord().getValue('wr_other.fulfilled') == '1') {
				color = '#7dcf87'; //green
				weight='normal';
			}
			//if it's a part that is on hold for quote, color orange
			else if (row.getRecord().getValue('wr_other.fulfilled') == '2') {
				color = '#cfcc7d';	//yellow
				weight='normal';
			}
			//if parts are on order,
			else if (row.getRecord().getValue('wr_other.fulfilled') == '3') {
				color = '#7d9ecf';	//blue
				weight='normal';
			}
			...
			var cell1 = row.cells.get('other_rs_type');
			var cell2 = row.cells.get('date_used');
            var cell3 = row.cells.get('qty_used');
			var cell4 = row.cells.get('description');
