function setFilterAndRender() {
	...
	if (date_from != '') {
		restriction += " AND wrhwr.date_completed >= "+restLiteral(date_from);
	}

	var date_to = console.getFieldValue('wrhwr.date_completed.to');
	if (date_to != '') {
		restriction += " AND wrhwr.date_completed <= "+restLiteral(date_to);
	}
    ...
    report.addParameter('date_rest', restriction);
	report.refresh(null);
