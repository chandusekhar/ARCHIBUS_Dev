...
// 2015/11/06 - MSHUSSAI - Added code in function errorReportPanel_beforeSave to add location to the Subject and Body of the email message
...
var errorReportController = View.createController('errorReportController', {
	...
	errorReportPanel_beforeSave: function()
	{
		...
		if (this.errorReportPanel.getFieldValue("request_rm_id") == "")	{
			specialString = "";
		}
		else {
			specialString = "/";
		...
		location = this.errorReportPanel.getFieldValue("request_bl_id") + "/" + this.errorReportPanel.getFieldValue("request_fl_id") + specialString + this.errorReportPanel.getFieldValue("request_rm_id");
		...
		if (errorType == "WR") {
			...
			subject = "A Room Error has been reported - " + location;
			//email header
    ...
    createWorkRequest: function() {
		...
		if (this.errorReportPanel.getFieldValue("request_fl_id") == "") {
			View.showMessage("Floor must be entered.");
			return false;
		}
