...
var ucWrhwrExec =  View.createController("ucWrhwrExec",{
	...
	requestConsole_onSelectCf:function(){
		var filter = "";
		if (this.requestConsole.getFieldValue("wr.cf_id") != "") {
			filter = "&filter=" + this.requestConsole.getFieldValue("wr.cf_id")
		}

		View.openDialog('uc-select-cf-vn-dialog.axvw?parentPanel=requestConsole&tragetTbl=wr&tragetFld=cf_id'+filter, "", false, {
				closeButton: true,
				maximize: false,
...
function apply_console_restriction() {
    ...
    var description = console.getFieldValue('wrhwr.description');
    ...
    if (description != "") { restriction = restriction + " AND wrhwr.description " + literalOrNullLike(description); }
    ...
    if(cf_id != "") {
        restriction += " AND (EXISTS(select 1 from wrcfhwrcf where wrcfhwrcf.wr_id=wrhwr.wr_id AND wrcfhwrcf.cf_id = "+literalOrNull(cf_id)
...
function literalOrNullLike(val, emptyString) {
    if (val == undefined || val == null)
        return "NULL";
    else if (!emptyString && val == "")
        return "NULL";
