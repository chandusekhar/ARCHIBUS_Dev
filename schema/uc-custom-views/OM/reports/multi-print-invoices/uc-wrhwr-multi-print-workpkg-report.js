var printWoCtrl = View.createController('printWoCtrl', {
   ...
   printRow: function(para,rest,list) {
		...
		ds1=this.view.dataSources.get('workpkg_ds');
		if(rest!=' '){
			...
			ds=View.dataSources.get('workpkg_ds');
			if((selectedRows.length>0)&&(list==true)){
				var wo_id = ' ( ';
				for(i=0;i<selectedRows.length;i++){
					currRec = selectedRows[i];
					if(i!=(selectedRows.length-1)){
						wo_id+= "'"+currRec.values['wrhwr.wo_id']+"',";
					}
					else{
						wo_id+= "'"+currRec.values['wrhwr.wo_id']+"' )";
					}
				}
				restriction = restriction +" and wohwo.wo_id in "+wo_id;
...
function buildPrint2(restrictionClause,oIframe1,print,ds1,styles,stylesheets){
	...
	if(result.code == 'executed'){
		...
		for(var j=0;j<results.length;j++){
			...
			var wo_id=record['wohwo.wo_id'];
			rest="wohwo.wo_id='" +wo_id+"' "
...
function buildChildTableFromDataSource(rest,dsItem){
	...
	if(tablName=="wrhwr"){
		var restriction = "exists (Select 1 from wohwo where " + divrest + " AND  " + restriction + ")"
	}
	else{
		var restriction = "exists (Select 1 from wrhwr,wohwo where wrhwr.wo_id=wohwo.wo_id AND " + divrest + " AND  " + restriction + ")"
	}
