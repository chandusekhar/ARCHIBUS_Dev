...
//
//2016/06/30 - MSHUSSAI - Added new function called sendEmailOnHP() in the javascript in order to handle emailing On Hold for Parts
//
//2016/07/19 - MSHUSSAI - Added condition to saveWorkRequestOther() function to not send emails if more then one part has been added
...
var costTabController = View.createController('costTabController', {
    ...
    wrcfReportGrid_afterRefresh: function () {
        ...
        if (this.firstLoad && rest != null) {
            ...
            this.wrOtherReportGrid.refresh("wr_other." + rest);

            //
            var openerView = View.getOpenerView();
            if (openerView != undefined) {
                var openerCtrl = openerView.controllers.get("managerDetailsController");
                if (openerCtrl.infoTabControllerVehicle != null) {
                    this.infoController = openerCtrl.infoTabControllerVehicle;
                    this.infoPanel = this.infoController.nav_details_info_vehicle;
                }
                else {
                    this.infoController = openerCtrl.infoTabController;
                    this.infoPanel = this.infoController.nav_details_info;
                }
            }

            if (this.infoPanel != null) {
                var wr_tr_id = this.infoPanel.getFieldValue('wr.tr_id');
                if (wr_tr_id == 'FLEET') {
                    this.isFleet = true;
                    this.wrOtherReportGrid.setTitle("Contractors/Other");
                    this.wrptReportGrid.refresh(rest);
                }
            }
        }
    },
    ...
    wrOtherEditPanel_afterRefresh: function () {
        ...
        if (other_type == 'KEYSHOP' || other_type == 'P-CARD') {
    ...
    wrOtherEditPanel_beforeSave: function () {
        ...
        var dataSourceWROther = View.dataSources.get(View.panels.get('wrOtherEditPanel').dataSourceId);
        ...
        if (this.wrOtherEditPanel.record.isNew ||
            ...
            var rest = "wr_other.wr_id = " + wrId
									...
									+ dataSourceWROther.formatValue("wr_other.date_used", insertDate, true) + "')";


            if (dataSourceWROther.getRecord(rest).values["wr_other.date_used"] != undefined) {
                ...
                insertDate = dataSourceWROther.getRecord(rest).values["wr_other.date_used"];
    ...
    onOtherChanged: function (newVal) {
        if (newVal == 'KEYSHOP' || newVal == 'P-CARD') {
...
function fillRequiredInfo(button) {
    ...
    switch (button.id) {
        ...
        case "btnWrOtherAdd":
            ...
            break;
        case "btnWrOtherCLRAdd":
            tablename = "wr_other_clr";
            editPanelName = "wrOtherCLREditPanel";
            break;
    }
    ...
    if (tablename != undefined) {
        ...
        if (tablename == "wr_other_clr") {
            var tablename_real = "wr_other";
            panel.setFieldValue(tablename_real + ".wr_id", wrId);
            panel.setFieldValue('wr_other.other_rs_type', 'CONTRACTOR');
            panel.setFieldValue('wr_other.fulfilled', 'No', '0');
            panel.setFieldValue('wr_other.po_line', '1');
            panel.setFieldValue('wr_other.qty_used', '1');
        }
        ...
        View.confirm(getMessage('confirmSetOnHoldForParts'), function (button) {
            if (button == 'yes') {
                ...
                try {
                    ...
                    result = Workflow.callMethod('AbBldgOpsOnDemandWork-WorkRequestService-updateWorkRequestStatus', 'wr', 'wr_id', wr_id, recordValues, status);
                    ...
                    costTabController.infoController.refreshSearchGrid();
                } catch (e) {
                    Workflow.handleError(e);
                }
            }
...
// *****************************************************************************************
// Used when a new CLR is created
// Refreshes the grid panel 
//******************************************************************************************
function saveCLR() {
    View.getControl('', 'wrOtherReportGrid').refresh();
}
...
// ******************************************************************************************
function sendEmailOnHPNew() {

    var sendEmail = false;

    var totalCount = null;

    var infopanel = View.panels.get('wrOtherEditPanel');

    var record = UC.Data.getDataRecordValuesFromForm(infopanel);

    // check for the main wr
    var thisWrId = infopanel.getFieldValue('wr_other.wr_id');
    var thisWoId = infopanel.record.getValue('wr.wo_id');
    var workTeamId = infopanel.record.getValue('wr.work_team_id');
    var mainReqRest = "wr.wr_id = (SELECT MIN(t.wr_id) FROM wr t WHERE t.wo_id = " + thisWoId + ")";
    ...
    var rest = "em_id = '" + mainRequestor.replace(/\'/g, "''") + "'";
    var sendToEmail = UC.Data.getDataValue("em", "email", rest);
...
function cfSelectVal() {
    var editPanel = View.panels.get('wrcfEditPanel');
    var restriction = new Ab.view.Restriction();

    var wrRecords = UC.Data.getDataRecords('wr', ['wr_id'], "wr.tr_id='FLEET' and wr.wr_id=" + editPanel.getFieldValue('wrcf.wr_id'));
    if (wrRecords != undefined && wrRecords.length > 0) {
        restriction.addClause('cf.work_team_id', 'FLEET');
    }

    View.openDialog('uc-wr-manager-select-cf.axvw', new Ab.view.Restriction(), false, { width: 800, height: 600, fleetRestriction: restriction });
}
