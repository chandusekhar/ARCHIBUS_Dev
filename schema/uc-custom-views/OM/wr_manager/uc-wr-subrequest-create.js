	...
	copyInformation: function() {
		if (this.mainWrId != null) {
			...
			var fieldNames = ['wr_id', 'wo_id', 'site_id', 'bl_id', 'fl_id', 'rm_id',
					...
					'incident_no', 'ins_no', 'driver', 'comments_mgr','comments_risk'
					];
			...
			this.subReqForm.setFieldValue('wr.work_team_id', mainRecord["wr.work_team_id"]['l']);
			...
			//this.subReqForm.setFieldValue('wr.cf_notes', mainRecord["wr.cf_notes"]);
...
if (!brgTest){
	...
	subReqForm_beforeSave: function() {
		...
		//var origDesc = this.subReqForm.record.values['wr.description'];

		var origDesc = this.subReqForm.getFieldValue('wr.description');

		var parentId = "Parent Request ID: WR#" + this.mainWrId + "\r\n";
		...
		var newDesc = "REASON FOR SUB-REQUEST: (Assign To: "+ selectedWorkUnit +") " + replaceLF(this.subReqForm.getFieldValue('subwrcomments')) + "\r\n\r\n" + parentId + origDesc;

		// Need to set the record directly also, otherwise, the line feeds does
		// not save correctly.
		this.subReqForm.record.setValue('wr.description', newDesc);
		//alert(newDesc.replace(/\n/g,"[n]").replace(/\r/g,"[r]"));
		//return false;
