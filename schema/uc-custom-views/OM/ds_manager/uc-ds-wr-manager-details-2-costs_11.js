...
//
//2016/06/27 - MSHUSSAI - Added new function called sendEmailOnHPNew() in order to handle emailing On Hold for Parts
//
...
var costTabController = View.createController('costTabController', {
	...
	wrcfReportGrid_afterRefresh: function() {
		...
		if (this.firstLoad && rest != null) {
			...
			this.pjhaReportGrid.refresh(rest);
...
//	 */
	...
	wrOtherEditPanel_afterRefresh: function() {
		...
		if (other_type == 'KEYSHOP' || other_type == 'P-CARD' || other_type == 'FLEET' || other_type == 'INSTOCK') {
...
//	 */
	wrOtherEditPanel_beforeSave: function() {
		var continueSave = true;
		var otherds = this.wrOtherEditPanel.getDataSource();
		...
		if (this.wrOtherEditPanel.record.isNew ||
			...
			var rest = "wr_other.wr_id = "+wrId
									...
									+ this.ds_wr_other2.formatValue("wr_other.date_used", insertDate, true) + "')";


			if (this.ds_wr_other2.getRecord(rest).values["wr_other.date_used"] != undefined ) {
				...
				insertDate = this.ds_wr_other2.getRecord(rest).values["wr_other.date_used"];
	...
	{
		if (newVal == 'KEYSHOP' || newVal == 'P-CARD' || newVal == 'FLEET' || newVal == 'INSTOCK') {
...
function saveWorkRequestCraftsperson()
{
	...
	costTabController.infoController.saveWrAuditValuesIssuedFunc = null;
	...
	if (costTabController.infoPanel.getFieldValue('wr.status') == 'RA') {
		var oldStatus = costTabController.infoPanel.getFieldValue('wr.status');
		View.confirm(getMessage('confirmSetIssued'), function(button) {
			if (button == 'yes') {
				var result = {};
				try {
					...
					costTabController.infoController.saveWrAuditValuesIssued();					
...
function saveWorkRequestOther()
{
	...
	var record = UC.Data.getDataRecordValuesFromForm(panel);
	costTabController.infoController.saveWrAuditValuesHPFunc = null;
		...
		View.confirm(getMessage('confirmSetOnHoldForParts'), function(button) {
			if (button == 'yes') {
				...
				try {
					...
					recordValues["wr.wr_id"] = wr_id;
					recordValues["wr.status"] = oldStatus;
					costTabController.infoController.saveWrAuditValuesHP();
					result = Workflow.callMethod('AbBldgOpsOnDemandWork-WorkRequestService-updateWorkRequestStatus', recordValues, status);					
					costTabController.infoController.afterSaveWorkflowFunc = null;
					costTabController.infoController.afterSaveWorkflow();
					costTabController.infoPanel.refresh();
...
function sendEmailOnHPNew()
{
		...
		var infopanel = View.panels.get('nav_details_info');
		
		var record = UC.Data.getDataRecordValuesFromForm(infopanel);
		
		// check for the main wr
		var thisWrId = infopanel.getFieldValue('wr.wr_id');
...
function deleteItems(gridName, tableName)
{
	...
	if (gridName=="wrOtherReportGrid") {
		for (var i = 0; i < dataRows.length; i++) {
			...
			dataRowCopy['wr_other.actual_cost'] = dataRow['wr_other.actual_cost'];
			var actual_cost = toJSON(dataRowCopy['wr_other.cost_total']);
			dataRowCopy['wr_other.other_rs_type'] = dataRow['wr_other.other_rs_type'];
			var rs_type = toJSON(dataRowCopy['wr_other.other_rs_type']);
			
			alert(fulfilled + '/' + actual_cost + '/' + rs_type);

			//remove quotes using a global regular expression
			var fulfilled = fulfilled.replace(/\"/g, "");
			var rs_type = rs_type.replace(/\"/g, "");

			if (fulfilled != "No" && rs_type != "CONTRACTOR" && actual_cost) {
				deleteOK = false;
