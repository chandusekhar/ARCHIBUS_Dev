...
// 2015/01/13 - MSHUSSAI - Commented ac_desc_value Line 177 in order to fix an issue where the Account Description field was displaying the Work Request Description, this is now fixed
...
// 2016/02/23 - MSHUSSAI - WR370556 - Fixed Vehicle Barcode, Budget Owner and Vehicle Driver 'Info' pop up screens
...
// 2012/10 - DC - added to handle vehicle request.
		//alert(this.nav_details_info.getFieldValue('wr.work_team_id'));
		//alert(this.nav_details_info.getFieldValue('wr.tr_id'));
		if (this.nav_details_info.getFieldValue('wr.work_team_id')=='FLEET') {
			//Vehicle record
...
// 2012/10 - DC - added to handle vehicle request.
	...
	odometerDetailsPanel_beforeSave:function() {
		//update odometer panel
		this.odometerDetailsPanel.setFieldValue("uc_fleet_odo.vehicle_id", this.nav_details_info.getFieldValue("vehicle.vehicle_id"));
		this.odometerDetailsPanel.setFieldValue("uc_fleet_odo.wr_id", this.nav_details_info.getFieldValue("wr.wr_id"));
...
// 2012/10 - DC - added to handle vehicle request.
	...
	//		var restriction = "vehicle.vehicle_id = '"+this.nav_details_info.getFieldValue("eq.eq_std").replace(/'/g,"''")+"'";
			var restriction = "vehicle.eq_id = '"+this.nav_details_info.getFieldValue("wr.eq_id").replace(/'/g,"''")+"'";
...
// 2012/10 - DC - added to handle vehicle request.
	...
	dvrInfoVehicle_afterRefresh: function() {
		...
		var thisDriver = this.nav_details_info.getFieldValue('wr.driver');
	...
	boInfoVehicle_afterRefresh: function() {
		...
		var thisBo = this.nav_details_info.getFieldValue('wr.budget_owner');
...
// *****************************************************************************
// eMail Generic Vehicle message
// *****************************************************************************
function eMailVehicleGeneric(recipient)
{
	var subject = encodeURIComponent("Regarding unit " + View.getControl('','vehicleInfo').getFieldValue('vehicle.vehicle_id'));
	var body    = encodeURIComponent("\n\nRef WR: " + View.getControl('','nav_details_info').getFieldValue('wr.wr_id'));
	var mailto = "";
	switch(recipient) {
		case "budget_owner":
			mailto = View.getControl('','vehicleInfo').getFieldValue('vehicle.budget_owner');
		case "org_admin":
			mailto = View.getControl('','vehicleInfo').getFieldValue('vehicle.org_admin');
		case "org_contact":
			mailto = View.getControl('','vehicleInfo').getFieldValue('vehicle.org_contact');
		default:
			mailto = "";
	}
...
// *****************************************************************************
// eMail Maint Vehicle message
// *****************************************************************************
function eMailVehicleMaint(recipient)
{
	var subject = encodeURIComponent("Vehicle maintenance on your unit " + View.getControl('','vehicleInfo').getFieldValue('vehicle.vehicle_id'));
	var body    = encodeURIComponent("Hello. \n\n It shows in our records that your vehicle is due for maintenance. " 
