...
var costTabController = View.createController('costTabController', {
	...
	wrcfReportGrid_afterRefresh: function() {
		...
		if (this.firstLoad && rest != null) {
			...
			this.vmrsReportGrid.refresh(rest);
...
//	 */
	...
	odometerDetailsPanel_afterRefresh: function() {
		if (this.isFleet) {
		...
		if (other_type == 'KEYSHOP' || other_type == 'FLEET') {
...
//	 */
	...
	{
		if (newVal == 'KEYSHOP' || newVal == 'FLEET') {
	...
	add_vmrs_form_beforeSave:function() {
	
		this.add_vmrs_form.setFieldValue("uc_vmrs_record.wr_id", this.nav_details_info.getFieldValue("wr.wr_id"));
...
function refreshVMRS()
{
	var panel = View.panels.get('vmrsReportGrid');
	View.getControl('', 'vmrsReportGrid').refresh();
...
function cfSelectVal(){
	...
	View.selectValue('', "Craftpersons", ['wrcf.cf_id'], 'cf', ['cf.cf_id'],
				['cf.cf_id','cf.name','cf.tr_id','cf.position','cf.rate_hourly','cf.rate_over','cf.rate_double','cf.status'],
		        restriction, null, false, true, '');
}
...
function selectValueWrOther() {
	var rest = "other_rs.other_rs_type like 'FLEET%' or other_rs.other_rs_type like 'VEHICLE%' or other_rs.other_rs_type like 'CONTRACTOR'";
	//alert(rest);
