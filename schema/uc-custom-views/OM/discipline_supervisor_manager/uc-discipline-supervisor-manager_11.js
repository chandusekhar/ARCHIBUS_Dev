...
// 2016/07/20 - MSHUSSAI - Added condition to afterAssignCF() function to prompt user to Issue request when a new Craftsperson is added
...
var disciplineSupervisorManagerController = View.createController('disciplineSupervisorManagerController', {
    ...
    afterViewLoad: function() {
        ...
        //If the USER is in a restricted work team (ie they shouldn't be able to see other work teams) then disable the work team field.
        if(View.user.role == 'UC-FACULTYENG') {
            this.consolePanel.enableField("wr.work_team_id", false);
            $('selectbox_work_team_id').disabled=true;
        }
    ...
    wrListPanel_afterRefresh: function(){
        ...
        this.wrListPanel.gridRows.each(function(row){
            var hazardCount = parseInt(row.record['wr.hazard_count']);
            if(hazardCount == 0){
                row.actions.get('hazard_icon').show(false);
            }
        });
    },
    ...
    consolePanel_afterRefresh: function(){
        ...
        if(View.user.role == 'UC-FACULTYENG') {
            alert("Hello");
            this.consolePanel.enableField("wr.work_team_id", true);
        }
    }, 
    ...
    consolePanel_onShow: function(){
        ...
        var prob_cat = document.getElementById("prob_cat_input").value;
        var vehicle_id = this.consolePanel.getFieldValue("vehicle.vehicle_id");
        ...
        if(vehicle_id != ""){	restriction = restriction + " AND EXISTS(select 1 from vehicle where wr.eq_id = vehicle.eq_id and vehicle.vehicle_id="+this.literalOrNull(vehicle_id)+" )";	}
    ...
    wrListPanelhideShowButton: function(status){
        ...
        panel.actions.get(expire).button.setVisible(true);
        ...
        switch(status){
            case 'Requested':
                ...
                panel.actions.get(expire).button.setVisible(false);
                break;
            case 'For Re-assignment':
                panel.actions.get(assignCF).button.setVisible(true);
                panel.actions.get(fieldWorkComplete).button.setVisible(false);
            ...
            case 'Stopped':
                ...
                panel.actions.get(fieldWorkComplete).button.setVisible(false);
                panel.actions.get(sendToCCC).button.setVisible(false);
                panel.actions.get(reIssueRequests).button.setVisible(false);
                panel.actions.get(expire).button.setVisible(false);
                break;
    ...
    showHazardsByRoom: function(row) {
        var restriction = {};
        restriction['uc_env_hazard_by_room.bl_id'] = row['wr.bl_id'];
        restriction['uc_env_hazard_by_room.fl_id'] = row['wr.fl_id'];
        restriction['uc_env_hazard_by_room.rm_id'] = row['wr.rm_id'];
    ...
    afterAssignCF: function(selectedRecords) {
		...
		for (var i = 0; i < selectedRecords.length; i++) {
			// check if the request is in the "AA" state
			if (selectedRecords[i].getValue("wr.status") != "AA" || selectedRecords[i].getValue("wr.status") != "RA") {
				allAA = false;
				break;
			}
		}*/
        ...
        for (var i = 0; i < selectedRecords.length; i++) {
            ...
            if (selectedRecords[i].getValue("wr.status") == "RA") {
                allRA = true;
                break;
            }
        }
        ...
        if (allAA) {
            ...
            View.confirm("Do you want to Issue the requests?", function(button) {
                if (button == 'yes') {
                    disciplineSupervisorManagerController.wrListPanel_onReIssueRequests();
                }
    ...
    wrListPanel_onExpire: function () {
        ...
        var txt = expireWR(count);

        if (txt == 'Ok') {
            View.openProgressBar("Expiring Requests. Please wait...");
            var dataSource = View.dataSources.get("subreq_add_ds");
            ...
            for (var i = 0; i < selectedRecords.length; i++) {
                // Save audit info
                this.saveWrAuditValue(selectedRecords[i], "Exp");
                ...
                try {
                    ...
                    result = Workflow.callMethod('AbBldgOpsOnDemandWork-WorkRequestService-updateWorkRequestStatus', recordValues, "Exp");
                    success = true;
                    View.closeProgressBar();
                } catch (e) {
                    if (e.code == 'ruleFailed') {
            ...
            alert("Expired " + selectedRecords.length + " work request(s)");
...
// 2012/10 - DC - added to handle vehicle request.
	...
	wrListPanel_onReIssueRequests: function() {
		...
		for (var i = 0; i < selectedRecords.length; i++) {
			...
			var status = selectedRecords[i].getValue("wr.status");
			if ( (status != "HL") && (status != "HA") && (status != "PC") && (status != "FWC") && (status != "RA") ){
				View.showMessage("Not all selected records are in the On Hold for Labor, On Hold for Access, Parts Complete, For Re-assignment or Field work Complete status.");
				return false;
			}
...
function expireWR(count) {
    var txt;
    var r = confirm("Are you sure you want to expire " + count + " work request(s)?");
    if (r == true) {
        txt = "Ok";
    } else {
        txt = "Cancel";
