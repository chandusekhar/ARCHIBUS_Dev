...
//  2015/11/17 - MSHUSSAI - Added code to display the Change Requestor button to the UC-SYSADMIN role
...
var detailsTabController = View.createController('detailsTabController', {
	...
	flrmHousingRest: "bl_id in ('AU','BR','CA','CD','CR','VCA','GL','IH','KA','NO','OL','RU','YA')",
	blCT:"",
	flCT:"",
	blProgram:"",
	blZone:"",
	...
	wr_create_details_afterRefresh: function() {
		...
		// show "Change Requestor" button for CCC
		var userRole = View.user.role;
		if (userRole == 'UC-CSC' || userRole == 'UC-SYSDEV' || userRole == 'UC-SYSADMIN') {
			var changeReqBtn = document.getElementById('changeReq');
			changeReqBtn.style.display = 'block';
		}
...
*/
	...
	submitRequest: function(sTab) {
		...
		if (this.wr_create_details.getFieldValue('activity_log.work_team_id') == "RESIDENCE" && ($('ac_id_part1').value=="" || $('ac_id_part2').value==""|| $('ac_id_part3').value==""|| $('ac_id_part4').value==""|| $('ac_id_part5').value==""|| $('ac_id_part6').value=="")){
			...
			document.getElementById("ac_id_part3").value = '50161';
			...
			try {
				...
				if (!isProj) {
					...
					if (isRES) {
						...
						//JJYCHAN - problem with account codes. Always set charge_type to TBD
						//wrRecord.setValue('wr.charge_type', 'TBD');
					...
					if (isCore) {
				        // If any account code fields are filled in, we will ignore the auto check and have user's input override
				        if (document.getElementById("ac_id_part2").value != '' ||
						...
						//if (CoreWT!='CCC') {
							...
							if (DPID != null && DPID != "" &&
								(this.probtype.AID != null && this.probtype.AID != "") &&
								(this.blProgram != null && this.blProgram != "")
								)
							{
					...
					//	}
						...
						if (ac == ""){
							...
							//	return;
							//}
							alert('You have not entered an Account Code for this request. You may be contacted by Customer Care Center to supply an Account.');
				...
				if (!routingOn) {
					if (!isRES) {
						//copy the work team into option1 so we can track what the code would have done.
						wrRecord.setValue('wr.option1', wrRecord.getValue('wr.work_team_id'));
						if (CoreWT !='FMD_IT') {
							wrRecord.setValue('wr.work_team_id', 'CCC');
						}
					}
				...
				View.dataSources.get('ds_wr_save2').saveRecord(wrRecord);
				...
				//Email the Building Code
				//The below code is used to send an email to the Building Contacts Email Address once the work request has been created successfully.
				var bl_id2 = this.wr_create_details.getFieldValue('activity_log.bl_id');
				var em_id2 = this.wr_create_details.getFieldValue('activity_log.requestor');
				
				var rest2 = "bl_id = '" + bl_id2 +"'";
				var rest3 = "em_id = '" + em_id2 +"'";
				
				var DPContactEmail = UC.Data.getDataValue('bl', 'contact_phone', rest2);
				var DPRequestorEmail = UC.Data.getDataValue('em', 'email', rest3);
				var DPRequestorFirstName = UC.Data.getDataValue('em', 'name_first', rest3);
				var DPRequestorLastName = UC.Data.getDataValue('em', 'name_last', rest3);
				var DPRequestorFullName = DPRequestorFirstName + " " + DPRequestorLastName;
				var DPBuildingName = UC.Data.getDataValue('bl', 'name', rest2);
				var DPFloorName = this.wr_create_details.getFieldValue('activity_log.fl_id');
				var DPRoomName = this.wr_create_details.getFieldValue('activity_log.rm_id');
				var DPDescription = this.wr_create_details.getFieldValue('activity_log.description');
				var DPRequestorPhone = UC.Data.getDataValue('em', 'phone', rest3);
				

				if (DPContactEmail != null && DPContactEmail != "" && wrId!= null && wrId != "" && bl_id2 != null && bl_id2 != "")					
				{					
					...
					try {
						...
						var result = Workflow.callMethod('AbCommonResources-ucEmailService-sendEmailCustomTags', 'AbBldgOpsOnDemandWork', 'UC_EMAIL_WFR',
							'UC_WRCREATE_REQCONTACT_BODY','UC_WRCREATE_REQCONTACT_SUBJECT','','','',
							'', DPContactEmail, {"wrid": wrId, "ticketdescription": DPDescription, "requestorphone": DPRequestorPhone, "requestorname": DPRequestorFullName, "requestoremail": DPRequestorEmail, "blid": DPBuildingName, "flid": DPFloorName, "rmid": DPRoomName});
						
						/*var result = Workflow.callMethod('AbCommonResources-ucEmailService-sendEmail', 'AbBldgOpsOnDemandWork', 'UC_EMAIL_WFR',
							'UC_WRCREATE_REQCONTACT_BODY','UC_WRCREATE_REQCONTACT_SUBJECT','wr','wr_id',wrId,
							'', DPContactEmail);*/
					
						//alert("Your Email has been sent to the building contact.");
					}
					catch (ex) {
		...
		if (building == 'BR' ||	building == 'CA' ||	building == 'CD' ||	building == 'GL' ||
			...
			switch(building) {
				...
				case 'CR':
					...
					resdept='50161';
					break;	
				case 'VCA','VCB','VCC','VCD','VCE','VCF','VCG','VCH','VCI','VCJ','VCK','VCL','VCM','VCN','VCO','VCP','VCQ','VCR','VCS','VCT','VCU','VCV','VCW','VCX','VCY','VCZ':
					internal='RES000600';
					resdept='50161';
				default:
					internal='000000000';
			...
			document.getElementById("ac_id_part1").value = 'UCALG';
			document.getElementById("ac_id_part2").value = '15';
			document.getElementById("ac_id_part3").value = resdept;
			document.getElementById("ac_id_part4").value = '66115';
			document.getElementById("ac_id_part5").value = account;
...
function sendRequestorEmail()
{
    ...
    var emailAddress = infoForm.getFieldValue('em.email');
	var emRecord = UC.Data.getDataRecord('em', ['em_id', 'name_first', 'name_last', 'email'], "em_id='"+infoForm.getFieldValue('em.em_id').replace(/'/g, "''")+"'");

	if (emRecord != null) {
		eFullName = emRecord['em.name_first'] + " " + emRecord['em.name_last'];
        emailAddress = emRecord['em.email'];
	}
...
{
	var selectPanel = View.panels.get('changeRequestorSelect');
	selectPanel.refresh();
	selectPanel.isCollapsed = false;
	selectPanel.showIndexAndFilter();

	selectPanel.showInWindow({});
}

function changeRequestorSelect(row)
{
	var em_id = row['uc_d_person_ps.person_eid'];
	var email = row['uc_d_person_ps.person_campus_email'];
	var phone = row['uc_d_person_ps.person_campus_phone'];
	var name_first = row['uc_d_person_ps.person_first_name'];
	var name_last = row['uc_d_person_ps.person_last_name'];
	var em_number = row['uc_d_person_ps.emplid'];

	// check if em_id exists, if not create em record;
	var emRecord = UC.Data.getDataRecord('em', ['em_id'], "em_id='"+em_id.replace(/'/g, "''")+"'");
	if (emRecord == null) {
		var newEmRecord = new Ab.data.Record();
		newEmRecord.isNew = true;
		newEmRecord.setValue('em.em_id', em_id);
		newEmRecord.setValue('em.email', email);
		newEmRecord.setValue('em.phone', phone);
		newEmRecord.setValue('em.name_first', name_first);
		newEmRecord.setValue('em.name_last', name_last);
		newEmRecord.setValue('em.em_number', em_number);
		View.dataSources.get('saveEm_ds').saveRecord(newEmRecord);
	}

	var panel = View.panels.get('wr_create_details');
	panel.setFieldValue('activity_log.requestor', em_id);
	panel.setFieldValue('activity_log.phone_requestor', phone);

	BRG.UI.addNameField('requestor_name', panel, 'activity_log.requestor', 'em', ['name_first','name_last','phone','email'],
        {'em.em_id' : 'activity_log.requestor'},
        emNameLabelConfig);

	View.panels.get('changeRequestorSelect').closeWindow();
}