...
// 2016/03/23 - MSHUSSAI - Added code to Update AFM_USERS and CF table when the Save button is clicked.  Also added code to refresh the parent view based on the requestor details which have changed.
...
function saveEmRecord()
{
	...
	*/
	...
	/*uc_email(newEmail,
			...
			'standard.template');
    */
	var mainPanelView = View.getOpenerView();
	
	var formNew = mainPanelView.panels.get('wr_create_details')
	
	//var restrictionReq = {'wr_create_details.activity_log.bl_id': bl_id};
		
	//formNew.refresh(restrictionReq);
	
	BRG.UI.refreshNamedField(formNew, 'activity_log.requestor');
