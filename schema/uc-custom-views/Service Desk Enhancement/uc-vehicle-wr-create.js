...
var detailsTabController = View.createController('detailsTabController', {
	...
	afterInitialDataFetch: function() {
		...
		//this.wr_create_details.setFieldValue("activity_log.prob_cat", probCat);
		this.wr_create_details.setFieldValue("activity_log.prob_type", probType);
		this.wr_create_details.setFieldValue("activity_log.requestor", View.user.employee.id);
		this.wr_create_details.setFieldValue("activity_log.phone_requestor", View.user.employee.phone);
		
		var parameters = {
			tableName: 'probtype',
			fieldNames: toJSON(['probtype.priority']),
			restriction: "probtype.prob_type = '" + probType + "' AND probtype.prob_cat = '" + probCat + "'"
		};
	...
	checkForm: function(acct) {
		if (this.validateForm(acct)) {
			...
			try {
				...
				//Driver and cause code doesn't get saved in workflow so add after the fact
				var driver = this.wr_create_details.getFieldValue("activity_log.driver");
				var causeCode = this.wr_create_details.getFieldValue("activity_log.cause_type");

				var wrRecord= new Ab.data.Record();
				wrRecord.isNew = false;
				wrRecord.setValue('wr.wr_id', wrId);
				wrRecord.setValue('wr.driver', driver);
				wrRecord.setValue('wr.cause_type', causeCode);
	...
	/* 			var tabsPanel = View.getControl('', 'wr_create_tabs');
				...
				if(this.sTab=='docs'){
					viewPanel.loadView("uc-vehicle-wr-create-review.axvw?doc=y", "wr_id='"+wrId+"'", false);
				} else if(this.sTab=='report') {
					viewPanel.loadView("uc-vehicle-wr-create-review.axvw", "wr_id='"+wrId+"'", false);
					//sendRequestorEmail();
				}
...
function updateVehicle() {
	...
	var vRecord =  UC.Data.getDataRecord('vehicle', ['eq_id', 'dv_id', 'dp_id','budget_owner','em_id','ac_id'], "vehicle_id='"+vId.replace(/'/g, "''")+"'");
	if (vRecord != null) {
		...
		if (vRecord['vehicle.ac_id'].l !="") {
			var acct = vRecord['vehicle.ac_id'].l.split('-')
