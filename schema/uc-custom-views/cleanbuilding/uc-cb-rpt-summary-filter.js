var abCbRptSummaryFilterController = View.createController('abCbRptSummaryFilterCtrl',{
	...
	abCbRptSummaryFilter_console_onFilter:function(){
        ...
        var startDateAbated = this.abCbRptSummaryFilter_console.getFieldValue("activity_log.date_installed");
        var endDateAbated = this.abCbRptSummaryFilter_console.getFieldValue("activity_log.date_abated_end");
        if(!validateDates(startDateAbated, endDateAbated))
            return;
	...
	addConsoleRestriction: function() {
        ...
        this.restriction += getRestrictionForField(console, "activity_log.vf_assessment", this.instrLabels, this.printableRestriction);
        ...
        var startDateAbated = this.abCbRptSummaryFilter_console.getFieldValue("activity_log.date_installed");
        if (valueExistsNotEmpty(startDateAbated)) {
			var title = getTitleOfConsoleField(console, "activity_log.date_installed");
			this.restriction += " AND activity_log.date_installed >= ${sql.date('" + startDateAbated + "')}";
			this.instrLabels.push({field: "activity_log.date_installed", title: title, value: startDateAbated});
			this.printableRestriction.push({'title': title, 'value': startDateAbated});
        }
        
        var endDateAbated = this.abCbRptSummaryFilter_console.getFieldValue("activity_log.date_abated_end");
        if (valueExistsNotEmpty(endDateAbated)) {
			var title = getTitleOfConsoleField(console, "activity_log.date_abated_end");
			this.restriction += " AND activity_log.date_installed <= ${sql.date('" + endDateAbated + "')}";
			this.instrLabels.push({field: "activity_log.date_abated_end", title: title, value: endDateAbated});
			this.printableRestriction.push({'title': title, 'value': endDateAbated});
        }
	}
});
