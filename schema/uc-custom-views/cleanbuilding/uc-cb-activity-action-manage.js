var abCbActivityActionsCtrl = View.createController('abCbActivityActionsCtrl', {
	...
	getFilterRestriction: function(){
		...
		*/
		...
		this.addRestrictionClause(console, 'activity_log.project_id', 'activity_log.project_id', restriction, '=', 'AND');
		if (valueExistsNotEmpty(console.getFieldValue('activity_log.project_id'))) {
			sRestriction += getSqlRestrictionForField(console, 'activity_log.project_id');
			pRestriction.push({'title': getTitleOfConsoleField(console, 'activity_log.project_id'), 'value': console.getFieldValue('activity_log.project_id')});
		}
	...
	*/
	...
	abCbActItemsActionList_edit_onClick: function(row){
		...
		//var projectId = row['activity_log.project_id'];
		...
		var controller = this;
		alert (projectId);
		View.openDialog('uc-cb-action-add-edit.axvw', null, false, {
			width: 1024,
			height: 900,
			projectId: projectId,
			selKeys: [],
