...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<html>
			...
			<head>
				...
				<script language="JavaScript" src="#Attribute%//@relativeFileDirectory%/brg-proj-projects-org-impact-by-building.js">
					<xsl:value-of select="$whiteSpace"/>
				</script>
								...
								<xsl:for-each select="$afmTableGroup/dataSource/data/recordIndices/recordIndex">
									...
									<tr>
										<td>
											...
											<xsl:choose>
												<xsl:when test="//rowDimension/member[@name=$rowDimensionName]/@isAll='true'">false</xsl:when>
											...
											<xsl:call-template name="showingResult">
												...
												<xsl:with-param name="dv_dp_id" select="$rowDimensionName"/>
											</xsl:call-template>
										</td>
										...
										<xsl:variable name="editable">
											<xsl:choose>
												<xsl:when test="1=1">false</xsl:when>
												<xsl:otherwise>true</xsl:otherwise>
											</xsl:choose>
											</xsl:variable>
												<xsl:call-template name="showingResult">
	...
	<xsl:template name="showingResult">
		...
		<xsl:variable name="claimedImageID">IMG_C<xsl:value-of select="$dv_dp_id"/></xsl:variable>
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:choose>
							...
							<xsl:otherwise>
							<a href="#" title="{$actionTip}" onclick='settingValues("{$dv_dp_id}","{$actionSerialized}","{$actionTarget}")'>N/A</a>
							</xsl:otherwise>
							</xsl:choose>
						</xsl:otherwise>
				</xsl:choose>
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:choose>
							...
							<xsl:otherwise>
							<xsl:variable name="claimedImageID">IMG_C<xsl:value-of select="$dv_dp_id"/></xsl:variable>
									<a href="#" title="{$actionTip}" onclick="ChangeItToActiveItem('{$claimedImageID}'); settingValues('{$dv_dp_id}','{$actionSerialized}','{$actionTarget}')">
									<img alt="{$claimedImageID}" id="{$claimedImageID}" src="{$abSchemaSystemGraphicsFolder}/ab-icon-task-dflt.gif" border="0" width="7" height="7" />
									<xsl:value-of select="concat('  ',$value)"/></a>
							</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
