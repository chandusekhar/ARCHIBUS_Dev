...
{ 
	...
	if ($('status'))
	{
	    ...
	    if (status == 'In Planning') {
	    	restriction += "status IN (\'Approved\',\'Approved-In Design\')";
	    } else if (status == 'In Execution') {
	    	restriction += "status IN (\'Issued-In Process\',\'Issued-On Hold\',\'Completed-Pending\',\'Completed-Not Ver\')";
		} else restriction += "brg_project_view.status LIKE \'%\'";
	} else restriction += "project_id IS NOT NULL";
