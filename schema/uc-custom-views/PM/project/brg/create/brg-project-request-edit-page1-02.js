...
// 2016/01/13  MSHUSSAI - WR298692: Now TR_ID will remain as PROJECTS but WORK_TEAM_ID will be changed to CCC
...
var projectRequestEditPage1Controller = View.createController('projectRequestEditPage1', {
	...
	projectCreateForm_afterRefresh: function()
	{
		...
		if (wrRecord != null) {
			this.projectCreateForm.setFieldValue('project.bl_id', wrRecord["wr.bl_id"]["l"]);
			this.projectCreateForm.setFieldValue('project.requestor', wrRecord["wr.requestor"]["l"]);
			this.projectCreateForm.setFieldValue('project.ac_id', wrRecord["wr.ac_id"]["l"]);
			this.projectCreateForm.setFieldValue('project.description', wrRecord["wr.description"]["l"]);
			this.projectCreateForm.setFieldValue('project.scope', wrRecord["wr.cf_notes"]["l"].substring(0,1999));
		}
	...
	projectRequestEditPage1CreatedGrid_onWithdrawProjectButton: function(row)
	{
		...
		if (wrDataValues == undefined) {
			...
			try {
				Workflow.callMethod('AbBldgOpsOnDemandWork-WorkRequestService-cancelWorkRequest',wr_id);
				View.showMessage("The Project ("+projectId+") has been Rejected");
			}
			catch (e) {
	...
	createProjectFromRequest: function()
	{
		...
		if (this.createButtonWR != null) {
			...
			if (this.projectCreateForm.save()) {
				...
				var old_cf_notes = BRG.Common.getDataValues('wr',['cf_notes'],'wr_id='+wr_id)["wr.cf_notes"]["l"];
	...
	returnRequest: function()
	{
		...
		this.projectRequestReturnForm.setFieldValue("wr.work_team_id", "CCC");
