...
var ucProjQuickDetails2Ctrl = View.createController('ucProjQuickDetails2Ctrl', {
    ...
    formPanelCustomActions_form_afterRefresh: function() {
		...
		case "UC-PMOADMIN":
			...
			this.formPanelCustomActions_form.enableField("project.proj_acct", true);
			this.formPanelCustomActions_form.enableField("project.cost_likely", true);
			this.formPanelCustomActions_form.enableField("project.cfi_num", true);
			this.formPanelCustomActions_form.enableField("project.incl_noncfi", true);
			break;
		case "UC-PMOPM":
		...
		default:
			...
			this.formPanelCustomActions_form.enableField("project.proj_acct", false);
			this.formPanelCustomActions_form.enableField("project.cost_likely", false);
			this.formPanelCustomActions_form.enableField("project.cfi_num", false);
			this.formPanelCustomActions_form.enableField("project.incl_noncfi", false);
    ...
    formPanelCustomActions_form_onSaveInfoTab: function() {
        var autoGeneratedWorkRequest;
        var oldPhase = this.formPanelCustomActions_form.record.oldValues['project.proj_phase'];
        var newPhase = this.formPanelCustomActions_form.getFieldValue('project.proj_phase');
        ...
        if(!oldPhase.match(closeoutExpression) && newPhase.match(closeoutExpression)){
            var wfResults = Workflow.runRuleAndReturnResult('AbCommonResources-getDataRecords', {
                ...
                restriction: toJSON("project_id='" + this.formPanelCustomActions_form.getFieldValue('project.project_id').replace(/'/g,"''") + "'")
            });
            ...
            if(wfResults.data != 'undefined'){
                ...
                if(openWorkRequests.length > 0){
                    ...
                    this.view.alert('Please complete the following work requests before closing the project:\n' + workRequestList);
                    return false;
                }
            }
        }
        
        if(this.formPanelCustomActions_form.save())
        {
            ...
            //Refresh the grid panel(s) in the opener view
            var openerView = this.view.getOpenerView();
            var openerPanels = openerView.panels;
            for(var i = 0; i < openerPanels.items.length; i++){
                if(openerPanels.items[i]['type'] == 'grid') {
                    openerPanels.items[i].refresh();
                }
            }
