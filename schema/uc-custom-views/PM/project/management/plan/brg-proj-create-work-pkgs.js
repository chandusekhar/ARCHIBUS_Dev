var projCreateWorkPkgsController = View.createController('projCreateWorkPkgs', {
	...
	projCreateWorkPkgsAddWorkPkgForm_beforeSave : function() {
		var form = View.panels.get('projCreateWorkPkgsAddWorkPkgForm'); 
		var curDate = new Date();
		var date_start = getDateObject(this.projCreateWorkPkgsAddWorkPkgForm.getFieldValue('work_pkgs.date_est_start'));//note that getFieldValue returns date in ISO format
		var date_end = getDateObject(this.projCreateWorkPkgsAddWorkPkgForm.getFieldValue('work_pkgs.date_est_end'));
		if (date_end < date_start) {
			this.projCreateWorkPkgsAddWorkPkgForm.addInvalidField('work_pkgs.date_est_end', getMessage('endBeforeStart'));
			return false;
		}
		if ((curDate - date_start)/(1000*60*60*24) >= 1){
	    	if (!confirm(getMessage('dateBeforeCurrent'))) return false;
		}
		this.projCreateWorkPkgsAddWorkPkgForm.setFieldValue('work_pkgs.date_act_start', this.projCreateWorkPkgsAddWorkPkgForm.getFieldValue('work_pkgs.date_est_start'));
		this.projCreateWorkPkgsAddWorkPkgForm.setFieldValue('work_pkgs.date_act_end', this.projCreateWorkPkgsAddWorkPkgForm.getFieldValue('work_pkgs.date_est_end'));
	    return true;
	},
