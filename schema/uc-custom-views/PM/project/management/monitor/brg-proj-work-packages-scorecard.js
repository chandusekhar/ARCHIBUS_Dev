var projWorkPackagesScorecardController = View.createController('projWorkPackagesScorecard', {
	projRestriction : '',
	...
	selectProjectReport_afterRefresh: function(){
		this.projRestriction = getConsoleRestriction();
	...
	selectProjectReport_afterGetData: function(panel, dataSet){
		var defaultColumnSubtotal = new Ab.data.Record({
			'work_pkgs.count_work_pkg_id': {l: '', n: ''}
        });
        ...
        dataSet.totals[0].localizedValues['work_pkgs.count_work_pkg_id'] = '';
		for (var c = 0; c < dataSet.columnSubtotals.length; c++) {
			dataSet.columnSubtotals[c].localizedValues['work_pkgs.count_work_pkg_id'] = '';
		}
		for (var r = 0; r < dataSet.rowSubtotals.length; r++) {
			dataSet.rowSubtotals[r].localizedValues['work_pkgs.count_work_pkg_id'] = '';
		}
	},
...
function colorBlock(row, column, class_name){
    document.getElementById('selectProjectReport_cell_r' + row + '_' + 'c' + column + '_f0').parentNode.className = class_name;
}
