...
function onSelectRow(row) {
    var rest = "activity_log.project_id = '"+row["work_pkgs.project_id"].replace(/'/g, "''")+"' AND activity_log.work_pkg_id = '"+row["work_pkg_bids.work_pkg_id"].replace(/'/g, "''")+"'";
    View.panels.get('projUpdateActionsGrid').refresh(rest);
}