...
var vehicleCreateBasicController = View.createController("vehicleBasicReportController", {
	quest : null,
	currentStatus: null,
	oIframe:null,
	afterViewLoad: function() {
		...
		BRG.UI.setNameField('haz_doc_span', this.vehicle_details1, 'vehicle.haz_doc','<span style="color:#FF0000;">(if applicable)</span>')
		...
		//var vehicleId = this.vehicle_details.getFieldValue("vehicle.vehicle.id");
		//this.vehicle_details1.refresh("vehicle.vehicle_id='"+vehicleId+"'");
	},

/* 	afterInitialDataFetch: function() {
	//	var selectBox = $("vehicle_details1_vehicle.status");
...
/* 	vehicle_details1_onPrint: function() {
	    document.getElementById("sectionLabels1").style.display = "inline";
		var oContent1 = frames["vehicle_details1_form"].innerHTML;
	...
	vehicle_details1_onPrint: function() {
		if (this.oIframe == null) {
			if (!frames["print_iframe"]) { this.print.createFrame()}
			this.oIframe = frames["print_iframe"]
		}
		var browser=navigator.appName;
		if ((browser=="Microsoft Internet Explorer")) {
				this.oIframe.document.execCommand('print', false, null);
			} else {
	...
	showVehicle: function(row) {
		...
		vehicleCreateBasicController.vehicle_details1.show(false);
	...
	vehicle_type_drilldown_onRefresh: function(){
		...
		this.vehicle_details1.show(false);
	...
	vehicle_drilldown_onVehicle_addNew : function(){
		this.vehicle_details1.refresh(null,true);
	...
	vehicle_details1_afterRefresh: function() {
		var status = this.vehicle_details1.getFieldValue("vehicle.status");
		this.vehicle_details1.setFieldValue("vstatus",status);
		var q_id = 'FLEET - VEHICLE SAFETY';
		this.quest = new Ab.questionnaire.Quest(q_id, 'vehicle_details1', !(View.user.role=='UC_FleetManager' || View.user.role=='UC-SYSDEV'));
	    document.getElementById("sectionLabels1").style.display = "none";
	    document.getElementById("vehicle_details1_field_gen0_labelCell").style.display = "none";

		BRG.UI.addNameField('vehicle_type_info', this.vehicle_details1, 'vehicle.vehicle_type_id', 'vehicle_type', ['description','class_id'], {'vehicle_type.vehicle_type_id' : 'vehicle.vehicle_type_id'}, vehicleClassLabelConfig);
		BRG.UI.addNameField('perm_em', this.vehicle_details1, 'vehicle.em_id', 'em', ['name_first','name_last','phone','email'], {'em.em_id' : 'vehicle.em_id'}, emNameLabelConfig);
		BRG.UI.addNameField('org_contact_info', this.vehicle_details1, 'vehicle.org_contact', 'em', ['name_first','name_last','phone','email'], {'em.em_id' : 'vehicle.org_contact'}, emNameLabelConfig);
		BRG.UI.addNameField('org_admin_info', this.vehicle_details1, 'vehicle.org_admin', 'em', ['name_first','name_last','phone','email'], {'em.em_id' : 'vehicle.org_admin'}, emNameLabelConfig);
		BRG.UI.addNameField('budget_owner_info', this.vehicle_details1, 'vehicle.budget_owner', 'em', ['name_first','name_last','phone','email'], {'em.em_id' : 'vehicle.budget_owner'}, emNameLabelConfig);
		BRG.UI.addNameField('inspected_by_info', this.vehicle_details1, 'vehicle.inspected_by', 'em', ['name_first','name_last','phone','email'], {'em.em_id' : 'vehicle.inspected_by'}, emNameLabelConfig);
		BRG.UI.addNameField('disposal_requestor_info', this.vehicle_details1, 'vehicle.disposal_requestor', 'em', ['name_first','name_last','phone','email'], {'em.em_id' : 'vehicle.disposal_requestor'}, emNameLabelConfig);
		BRG.UI.addNameField('disposal_authorizer_info', this.vehicle_details1, 'vehicle.disposal_authorizer', 'em', ['name_first','name_last','phone','email'], {'em.em_id' : 'vehicle.disposal_authorizer'}, emNameLabelConfig);

		//this.replaceDisabledEnumFields('vehicle.color','vehicle.drive_type','vehicle.transmission');
	    vehicleCreateBasicController.replaceDisabledEnumFields([this.vehicle_details1.fields.get('vstatus').dom]);
		vehicleCreateBasicController.replaceDisabledEnumFields([this.vehicle_details1.fields.get('vehicle.color').dom]);
		vehicleCreateBasicController.replaceDisabledEnumFields([this.vehicle_details1.fields.get('vehicle.drive_type').dom]);
		vehicleCreateBasicController.replaceDisabledEnumFields([this.vehicle_details1.fields.get('vehicle.transmission').dom]);
		vehicleCreateBasicController.replaceDisabledEnumFields([this.vehicle_details1.fields.get('vehicle.occ').dom]);
		vehicleCreateBasicController.replaceDisabledEnumFields([this.vehicle_details1.fields.get('vehicle.engine').dom]);
		vehicleCreateBasicController.replaceDisabledEnumFields([this.vehicle_details1.fields.get('vehicle.meter_units').dom]);
		vehicleCreateBasicController.replaceDisabledEnumFields([this.vehicle_details1.fields.get('vehicle.avail_fdo').dom]);
		vehicleCreateBasicController.replaceDisabledEnumFields([this.vehicle_details1.fields.get('vehicle.condition').dom]);
		vehicleCreateBasicController.replaceDisabledEnumFields([this.vehicle_details1.fields.get('vehicle.hailstorm_writeoff').dom]);
		vehicleCreateBasicController.replaceDisabledEnumFields([this.vehicle_details1.fields.get('vehicle.hailstorm_repair').dom]);
		vehicleCreateBasicController.replaceDisabledEnumFields([this.vehicle_details1.fields.get('vehicle.condition').dom]);
		...
		document.getElementById("vehicle_details1_vehicle.po_doc").style.background="transparent";
	    document.getElementById("vehicle_details1_vehicle.po_doc").style.border="none";
		...
		//document.getElementById("vehicle_details1_vehicle.haz_doc").style.background="transparent";
	    //document.getElementById("vehicle_details1_vehicle.haz_doc").style.border="none";

		//var quest0 = this.vehicle_details1.fields.get('vehicle_details1_question0.answer_field';
		//vehicleCreateBasicController.replaceDisabledEnumFields([this.vehicle_details1.fields.get('vehicle_details1_question0.answer_field').dom]);
...
//		makeFieldReadOnly(document.getElementById('vehicle_details1_question0.answer_field'));
//		vehicleCreateBasicController.replaceDisabledEnumFields([document.getElementById('vehicle_details1_question0.answer_field')]);
	    ...
	    if( this.oIframe.document ) {
			if (this.oIframe.document.body != null) {
				this.oIframe.document.body.innerHTML = ""; //Chrome, IE
			}
		}else {
			this.oIframe.contentDocument.body.innerHTML = ""; //FireFox
		}

		var oDoc1
		if (this.oIframe.document) oDoc1 = this.oIframe.document;
		this.getPrintTxt(oDoc1,oContent1)
		oDoc1.close()
	   frames["vehicle_details1_form"].style.display = ""
	},

	vehicle_details1_beforeSave : function() {
...
function getEquipmentDetails() {
	...
	var vehicle_details1 = View.panels.get("vehicle_details1");

	var vehicle_restriction = {"vehicle.vehicle_id":row["vehicle.vehicle_id"]};

	vehicle_details1.refresh(vehicle_restriction, false);
	vehicle_details1.show(true);
	...
	document.getElementById("vehicle.description.new").value = vehicle_details1.getFieldValue("vehicle.description");
	document.getElementById("vehicle.comments.new").value = vehicle_details1.getFieldValue("vehicle.comments");
	...
	docCntrl.docPkey= vehicle_details1.getFieldValue("vehicle.vehicle_id");
...
function getNewEquipmentDetails(eqId) {
	...
	// Risk tab panels
	var vehiclerisk_details = View.panels.get("vehiclerisk_details");
	vehicle_restriction = {"vehicle.vehicle_id":vehicle_details1.getFieldValue("vehicle.vehicle_id")};
	vehiclerisk_details.refresh(vehicle_restriction, false);
...
function saveVehicleData() {
	...
	vehicle_details1.clearValidationResult();

	var status = vehicle_details1.getFieldValue("vstatus");
	vehicle_details1.setFieldValue("vehicle.status",status);
	vehicle_details1.setFieldValue("vehicle.description",document.getElementById("vehicle.description.new").value);
	vehicle_details1.setFieldValue("vehicle.comments",document.getElementById("vehicle.comments.new").value);
	var invalid = "false";

	if(status=='DISP'){
	//	vehicle_details1.addInvalidField('vehicle.status','');
		vehicle_details1.addInvalidField('vstatus','');
		invalid = "true";
	}
	...
	var eqId = vehicle_details1.getFieldValue('vehicle.eq_id');

	if(invalid=="true"){
		vehicle_details1.displayValidationResult();
	} else {

		if(document.getElementById('update_or_insert').value=='update') {

			eq_details.setFieldValue('eq.eq_id',eqId);
			eq_details.setFieldValue('eq.dv_id',vehicle_details1.getFieldValue('vehicle.dv_id'));
			eq_details.setFieldValue('eq.dp_id',vehicle_details1.getFieldValue('vehicle.dp_id'));
			eq_details.setFieldValue('eq.option1',vehicle_details1.getFieldValue('vehicle.mfr_id'));
			eq_details.setFieldValue('eq.option2',vehicle_details1.getFieldValue('vehicle.model_id'));
			eq_details.setFieldValue('eq.use1','FLEET VEHICLE');
			eq_details.setFieldValue('eq.eq_std','VEHCL-XXXX-XXXXX');
			eq_details.setFieldValue('eq.meter',vehicle_details1.getFieldValue('vehicle.meter'));
			eq_details.setFieldValue('eq.meter_units',vehicle_details1.getFieldValue('vehicle.meter_units'));
			eq_details.setFieldValue('eq.comments',vehicle_details1.getFieldValue('vehicle.description'));

			if (eq_details.save()) {
				if (!vehicle_details1.save()){
					return false
				}
			}
			else {
			...
			eq_details.setFieldValue('eq.eq_id',vehicle_details1.getFieldValue('vehicle.eq_id'));
			eq_details.setFieldValue('eq.dv_id',vehicle_details1.getFieldValue('vehicle.dv_id'));
			eq_details.setFieldValue('eq.dp_id',vehicle_details1.getFieldValue('vehicle.dp_id'));
			eq_details.setFieldValue('eq.option1',vehicle_details1.getFieldValue('vehicle.mfr_id'));
			eq_details.setFieldValue('eq.option2',vehicle_details1.getFieldValue('vehicle.model_id'));
			...
			eq_details.setFieldValue('eq.meter',vehicle_details1.getFieldValue('vehicle.meter'));
			eq_details.setFieldValue('eq.meter_units',vehicle_details1.getFieldValue('vehicle.meter_units'));
			eq_details.setFieldValue('eq.comments',vehicle_details1.getFieldValue('vehicle.comments'));
...
function dispVehicle() {
	...
	if(vehicle_details1.getFieldValue("vehicle.disposal_no")==""){
		invalid = "true";
		vehicle_details1.addInvalidField('vehicle.disposal_no','');
	}
	if(vehicle_details1.getFieldValue("vehicle.date_excessed")==""){
		invalid = "true";
		vehicle_details1.addInvalidField('vehicle.date_excessed','');
	}
	if(vehicle_details1.getFieldValue("vehicle.disposal_requestor")==""){
		invalid = "true";
		vehicle_details1.addInvalidField('vehicle.disposal_requestor','');
	}
	if(vehicle_details1.getFieldValue("vehicle.disposal_authorizer")==""){
		invalid = "true";
		vehicle_details1.addInvalidField('vehicle.disposal_authorizer','');
	}
	if(vehicle_details1.getFieldValue("vehicle.date_reg_cancelled")==""){
		invalid = "true";
		vehicle_details1.addInvalidField('vehicle.date_reg_cancelled','');
	}
	if(vehicle_details1.getFieldValue("vehicle.date_ins_cancelled")==""){
		invalid = "true";
		vehicle_details1.addInvalidField('vehicle.date_ins_cancelled','');
	}
	...
	if(invalid=="true") {
		...
		View.confirm(message, function(button) {
			if (button == 'yes') {
				...
				vehicle_details1.setFieldValue("vehicle.status","DISP");

				if(vehicle_details1.save()) {
					eq_details.save();
					vehicleCreateBasicController.checkDisablePMS()
					vehicle_details1.show(false);
					pms_grid.show(false);
...
function setVehicleBarcode(){
	...
	schedule_edit.setFieldValue("pms.eq_id",vehicle_details1.getFieldValue("vehicle.eq_id"));
}
...
function mfrOnchange(fieldName, selectedValue, previousValue){
	View.panels.get('vehicle_details1').setFieldValue("vehicle.model_id","");
}

function modelOnchange(){
	var model = View.panels.get('vehicle_details1').getFieldValue("vehicle.model_id");
	var mfr = "";
	if (model != "") {
	...
	View.panels.get('vehicle_details1').setFieldValue("vehicle.mfr_id",mfr);
}

function regType(){
	var pnl = View.panels.get('vehicle_details1')
