...
var helpDeskCreateBasicController = View.createController("helpDeskBasicController", {
	...
	afterInitialDataFetch: function() {
		...
		//if we opened from a vehicle detail (work requests tab), fill in the basic vehicle information
	...
	requestPanel_afterRefresh: function(){
		...
		$("activity_log.prob_type").readOnly = false;
	...
	doPrepareWorks: function(){
		...
		var openerVehiclePanel = View.getOpenerView().panels.get("vehicle_form");
		//alert(openerVehiclePanel.getFieldValue("vehicle.loc_vehicle"));
		this.equipmentPanel.setFieldValue("vehicle_id", openerVehiclePanel.getFieldValue("vehicle.vehicle_id"));
		
		
		this.equipmentPanel.setFieldValue("activity_log.eq_id", openerVehiclePanel.getFieldValue("vehicle.eq_id"));
		this.equipmentPanel.setFieldValue("activity_log.driver", openerVehiclePanel.getFieldValue("vehicle.org_contact"));
		this.equipmentPanel.setFieldValue("activity_log.location", openerVehiclePanel.getFieldValue("vehicle.loc_vehicle"));
		this.equipmentPanel.setFieldValue("activity_log.budget_owner", openerVehiclePanel.getFieldValue("vehicle.budget_owner"));
		this.equipmentPanel.setFieldValue("activity_log.ac_id", openerVehiclePanel.getFieldValue("vehicle.ac_id"));
		this.datePanel.setFieldValue("activity_log.date_scheduled", openerVehiclePanel.getFieldValue("vehicle.date_assign_em"));
		
		//alert(this.equipmentPanel.getFieldValue("activity_log.eq_id"));
	...
	doWorkForPanels: function(activityTypeValue){
		...
		if(activityLogId != 0 && activityLogId != undefined){
			...
			this.locationPanel.setFieldValue("activity_log.site_id", "C");
			this.locationPanel.setFieldValue("activity_log.bl_id", "PP");
		...
		if(this.mainTabs != null){
			...
			*/
			...
			if(result.code == 'executed'){
				...
				//JJYCHAN ADDED TO INSERT NON-WORKFLOW FIELDS (driver, date_assigned)
				var wr_id=UC.Data.getDataValue("activity_log", "wr_id", "activity_log.activity_log_id=" + res.activity_log_id);
				var driver=this.equipmentPanel.getFieldValue("activity_log.driver");
				var budget_owner = this.equipmentPanel.getFieldValue("activity_log.budget_owner");
				var date_scheduled = this.datePanel.getFieldValue("activity_log.date_scheduled");
				var towing = this.descriptionPanel.getFieldValue("activity_log.towing");

				var wrRestriction = new Ab.view.Restriction();
				wrRestriction.addClause("wr.wr_id", wr_id, "=");

				var dsWR = new Ab.data.createDataSourceForFields({
					id: 'dsWRSave',
					tableNames: ['wr'],
					fieldNames: ['wr.wr_id', 'wr.driver', 'wr.date_assigned', 'wr.budget_owner', 'wr.towing']
				});
				var wrRecord=dsWR.getRecord(wrRestriction);
				wrRecord.setValue("wr.driver", driver);
				wrRecord.setValue("wr.budget_owner", budget_owner);
				wrRecord.setValue("wr.date_assigned", date_scheduled);
				wrRecord.setValue("wr.towing", towing);
				dsWR.saveRecord(wrRecord);
...
function showSlaParameters() {
	...
	var recordValues = ABHDC_getDataRecordValues("basicDs4"); 
