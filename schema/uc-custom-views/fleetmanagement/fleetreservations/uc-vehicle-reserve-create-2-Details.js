...
var detailsTabController = View.createController('detailsTabController', {
	...
	prefillInfo: function() {
		...
		this.wr_create_details_afterRefresh()
	},
	...
	generateCosts: function() {
		var pnl = detailsTabController.wr_create_details
			...
			wrSql += "," + pnl.getFieldValue('activity_log.distance_est') + " distance_est"
	...
	submitRequest: function(sTab) {
		...
		if (this.validateForm()) {
			...
			try {
				...
				//View.dataSources.get(ds).saveRecord(actLogRecord);
				ds.saveRecord(actLogRecord)	
				...
				wrRecord.oldValues['wr.date_assigned'] = '2000-01-01';
	...
	*/		
				...
				/*var reportTabController = View.controllers.get('reportTabController');
				if (reportTabController != undefined) {
					reportTabController.prefillReportInfo();
				}
				*/
