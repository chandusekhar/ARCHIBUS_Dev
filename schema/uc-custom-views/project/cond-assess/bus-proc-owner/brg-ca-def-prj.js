...
var defProjCtrl = View.createController('defProjCtrl',{
	...
	projectDetailsPanel_onSave: function() {
		...
		var status = this.projectDetailsPanel.getFieldValue("project.status");
		//BRG 2015-05-12
		if (status == "Closed"){
			this.countItemDS.addParameter("projId", this.projectDetailsPanel.getFieldValue("project.project_id"));
			var rec = this.countItemDS.getRecord();
			var itemsCount = parseInt(rec.getValue("activity_log.countItems"));
			if (itemsCount > 0){
				View.showMessage('error',getMessage("cannotClose"));
				return;
			}

		} 

		this.projectDetailsPanel.save();
		this.projectsTreePanel.refresh();
	},
