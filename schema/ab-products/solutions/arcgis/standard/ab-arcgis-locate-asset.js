...
var mapController = View.createController('mapController', {
	...
	afterViewLoad: function(){
		...
		this.assetPanel.addEventListener('onMultipleSelectionChange', function(row) {
			...
			if( selection[0] ){
				View.controllers.get('mapController').toggleAssetPanelControls(true);
			}
			else {
				View.controllers.get('mapController').toggleAssetPanelControls(false);
			}
		})
	},
	...
	onLocateAsset: function(row) {
		...
		var lon = row['bl.lon.raw'] || row['bl.lon'] ;
		var lat = row['bl.lat.raw'] || row['bl.lat'];
		if (!!lon && !!lat) {
