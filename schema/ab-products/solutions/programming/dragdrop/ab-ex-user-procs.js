...
var userProcessAssignmentController = View.createController('userProcessAssignments', {
    ...
    afterInitialDataFetch: function() {
        ...
        for (var nodeCounter = 0; nodeCounter < this.userProcs_activityTree._nodes.length; nodeCounter++) {
            var node = this.userProcs_activityTree._nodes[nodeCounter];
            new UserProcessAssignmentControllerDropTarget(node);
        }
    },
    ...
    unassignUnlicensed: function() {
				try {
    			var result = Workflow.call('AbSystemAdministration-unassignUnlicensed');
 ...
 */
var UserProcessAssignmentControllerDropTarget = function(node) {
    this.node = node;
    UserProcessAssignmentControllerDropTarget.superclass.constructor.call(this, node.contentElId);
};

Ext.extend(UserProcessAssignmentControllerDropTarget, Ext.dd.DropTarget, {
    notifyDrop: function(dd, e, data) {
        // this.node is the drop target TreeNode
        // data is the drag source TreeNode
    },
    notifyOver: function(dd, e, data) {
        return 'x-dd-drop-ok';
        // to prevent drop, return 'x-dd-drop-nodrop'
    }
});
