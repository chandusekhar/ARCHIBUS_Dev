...
var definePMPController = View.createController('definePMP', {
    ...
    curTreeNode: null,
    modelName: 'nb',//must be in lower case
    //The tree panel 
    treeview: null,
    ...
    afterViewLoad: function(){
    	...
    	this.treeview.setTreeNodeConfigForLevel(1,           		
                [{fieldName: 'pmps.pmps_or_eq_id', pkCssClass: 'true'}, 
                 {fieldName: 'pmps.pmps_instructions', length: 40}]);    
    	...
    	//Isolate selector
    	var titleDiv = document.getElementById('procsFilterPanel_title');
    	...
    	bim3dViewer = this.bim3d;
        this.bim3d.setObjectClickEventListener(this.drillDown.bind(this));
        this.bim3d.setSuperCategories([{title:'Exterior/Core', name:'Exterior', superCategories:[ {title:'Site', name: 'Site', codes:['Topography','Site','Roads','Parking', 'Property Lines','Pads'], json: this.modelName + '-site'}, {title:'Shell',  name: 'Shell', codes:['Roofs','Walls','Curtain Panels', 'Doors'], functions:['Exterior', 'Foundation', 'Retaining', 'Coreshaft'], json: this.modelName + '-shell'}, {title:'Core',  name: 'Core',  codes:['Floors','Stairs','Railings','Ceilings'], json: this.modelName + '-core'}]},
                                       {title:'Disciplines',  name: 'Disciplines', superCategories:[{title:'Equipment',  name: 'HVAC',   codes:['Mechanical Equipment', 'Ducts','Duct Fittings','Duct Systems','Flex Ducts','Air Terminals'], json:''}, {title:'Pipes',  name: 'Pipes',   codes:['Flex Pipes','Piping Systems','Plumbing','Plumbing Fixtures', 'Pipe Accessories', 'Pipe Fittings', 'Pipes'], json: this.modelName +'-plumbing'}]}
                  	 	            ]);
        this.bim3d.setObjectDoubleClickable(false);
        var scope = this;
       
        //load mdoel
    	this.loadModel.defer(10,  this, [this.bim3d]);
    	
    	//html panel resize
		this.procsFilterPanel.syncHeight = function(){
			scope.bim3d.resize();
		}
        
    },
    
    loadModel: function(bim3d){
    	bim3d.load(this.modelName +'-hvac', true, function(scenes){
		});
    },
    
    drillDown: function(id, category, level, assetType, pk, userData){
    	//drill-down
    	this.bim3d.clearHighlight();
    ...
    procsFilterPanel_onIsolate_Eqs: function(){
    	this.bim3d.isolate(['Mechanical Equipment', 'Ducts']);
    },
    
    procsFilterPanel_onClear_isolate: function(){
...
function onClickTreeNode(){
    ...
    if(pmpNode.level.levelIndex === 0){
		//eq_system first???
		var eqSystemDatasource   = View.dataSources.get('ds_eq_system_demo')
		
		var restriction = new Ab.view.Restriction();
 		restriction.addClause('pms.pmp_id', pmpNode.data['pmp.pmp_id'], '=');
 		var pmsDatasource  = View.dataSources.get('ds_pms_demo');
 		var records = pmsDatasource.getRecords(restriction);
 		var restriction_eq_system = new Ab.view.Restriction();
 		var colors = {}, eq_id, records_eq_system;
 		for(var i=0, ln=records.length; i < ln ;i++){
 			eq_id = records[i].getValue('pms.eq_id');
 			colors[eq_id] = 'rgb(255, 0, 0)';
 			{
 				restriction_eq_system.addClause('eq_system.eq_id_master', eq_id, '=');
 				records_eq_system = eqSystemDatasource.getRecords(restriction_eq_system);
 				for(var j=0, lnj=records_eq_system.length; j<lnj; j++){
 					colors[records_eq_system[j].getValue('eq_system.eq_id_depend')] = 'rgb(255, 0, 0)';
 				}
 			}
 		}
 		bim3dViewer.clearSelection();
 		bim3dViewer.clearHighlight();
 		bim3dViewer.highlightByPrimaryKeys(colors);
 		bim3dViewer.render();
    }else{
    	
    }
    
    if (pmpNode.level.levelIndex == 1) {
    	var restriction = new Ab.view.Restriction();   	   	
    	var resType = pmpNode.data['pmps.res_type'];   	
    	if (resType == 'Step') {
        	var pmpId = pmpNode.data['pmps.pmp_id'];
        	var pmpsId = pmpNode.data['pmps.pmps_id'];  
        	restriction.addClause("pmps.pmp_id", pmpId, '=');
        	restriction.addClause("pmps.pmps_id", pmpsId, '=', 'AND');
        	View.controllers.get('definePMP').psrDetailTabs.selectTab("stepTab", restriction, false, false, false);    		
    	} else {
        	var restriction = new Ab.view.Restriction();  
    		var eqId = pmpNode.data['pmps.pmps_or_eq_id'];
        	View.panels.get("woGrid").addParameter("eqId", eqId);
        	//restriction.addClause("wo.wo_type", 'EQPM', '='); 
        	//restriction.addClause("wr.eq_id", eqId, '=', 'AND');
    		View.controllers.get('definePMP').psrDetailTabs.selectTab("woTab", restriction, false, false, false);
    	}
    }
...
function showPmpTree(eqId){
   ...
   // var eqId = definePMPController.getEqId();
    if (typeof eqId !== 'undefined' && eqId != '') {
        report.addParameter("eqId", eqId); 
    	report.addParameter("eqRest", "AND pms.eq_id = '" + eqId + "'");    	
    } else {
        report.addParameter("eqId", "");
    	report.addParameter("eqRest", ""); 
    }
