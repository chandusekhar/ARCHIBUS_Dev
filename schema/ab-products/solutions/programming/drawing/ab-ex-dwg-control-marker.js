/**
 * called by ab-ex-dwg-control-marker.axvw
 */
var exampleController = View.createController('example', {
    ...
    loadSvg: function(bl_id, fl_id) {
    	...
    	// layer control
    	this.layerControl = this.drawingControl.getAddOn('LayersPopup');
    ...
    drawingPanel_onShowExistingMarkers: function() {
		...
		// zoom extents
    	this.drawingControl.getAddOn('NavigationToolbar').onZoomExtents();
		...
		if(records == null || records.length == 0){
			...
			// refresh the list of layers; only need to do this once
			if (this.refreshLayerPopup) {
				this.refreshLayerPopup = false;
				this.layerControl.showLayers();				
			}
		}
    ...
    afterPlaceMarker: function(marker) {
		...
		// refresh the list of layers; only need to do this once
		if (self.refreshLayerPopup) {
			self.refreshLayerPopup = false;
			self.layerControl.showLayers();			
		}
...
function onClickFloorTreeNode(){
	...
	if (rm_id) {
    	...
    	exampleController.drawingControl.getAddOn('AssetLocator').findAssets([bl_id+';'+fl_id+';'+rm_id], opts);
	} 
...
function onClickMarkerTreeNode(){
	...
	controller.drawingControl.getAddOn('AssetLocator').findAssets([controller.layerName + '_' + auto_number], { zoomFactor: 10 });
}