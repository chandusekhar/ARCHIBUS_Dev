  ...
  <xsl:template match="/">
    <html>
      <head>
        ...
        <script language="javascript" src="#Attribute%//@relativeFileDirectory%/ab-eq-locate.js">
          <xsl:value-of select="$whiteSpace" />
        </script>
      </head>
      ...
      <body  onload='preparingLoadPage()' class="body" leftmargin="0" rightmargin="0" topmargin="0">
		...
		<script language="javascript">
			<xsl:for-each select="/*/afmTableGroup/dataSource/data/records/record">
				<xsl:variable name="bl_fl" select="concat(@eq.bl_id,'-',@eq.fl_id)"/>
				<xsl:variable name="bl_fl_rm" select="concat($bl_fl, ';', @eq.rm_id)"/>
				arrBLFLRM['<xsl:value-of select="$bl_fl_rm"/>']='<xsl:value-of select="@eq.dwgname"/>';
				arrBLFL['<xsl:value-of select="$bl_fl"/>']='<xsl:value-of select="@eq.dwgname"/>';
			</xsl:for-each>
