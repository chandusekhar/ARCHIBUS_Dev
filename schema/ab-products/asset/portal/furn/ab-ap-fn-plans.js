 ...
 */
var abFnPlansController = View.createController('abFnPlansCtrl',{
	afterViewLoad: function(){
        //specify a handler for when drawing is fully loaded; to be able to manually set highlights after load
        View.panels.get('ab-fn-plans_DrawingPanel').addEventListener('ondwgload', onDwgLoaded);
		//add event listener fro clicking on a room
		View.panels.get('ab-fn-plans_DrawingPanel').addEventListener('onclick', onDwgPanelClicked);
    }
});
...
function onDwgLoaded(){
	...
	var records = View.dataSources.get('ds_ab_fn_plans_darwing_highlight').getRecords(restriction);
	...
	if(records.length > 0){
		var dwgCtrl = View.panels.get('ab-fn-plans_DrawingPanel');
...
function onDwgPanelClicked(pk){
	...
	var restriction = new Ab.view.Restriction();
	restriction.addClause('fn.bl_id' , pk[0]);
	restriction.addClause('fn.fl_id' , pk[1]);
	restriction.addClause('fn.rm_id' , pk[2]);
	
	
	
	View.panels.get('ab-fn-plans-summary').refresh(restriction);
	View.panels.get('ab-fn-plans-list').refresh(restriction);
}
...
function loadDrawingPanel(node){
	...
	View.panels.get('ab-fn-plans-summary').show(false);
	View.panels.get('ab-fn-plans-list').show(false);
	
	//get selected building and floor
	var currentNode = View.panels.get('ab-fn-plans-tree-bl').lastNodeClicked;
	...
	//draw the floor map
    var drawingPanel = View.panels.get('ab-fn-plans_DrawingPanel');
