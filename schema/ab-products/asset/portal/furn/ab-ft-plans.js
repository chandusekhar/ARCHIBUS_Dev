...
var abFtPlansController = View.createController('abFtPlansCtrl',{
	 ...
	 */
	ab_ft_plans_console_onPaginatedReport: function(){
		var site_id = this.ab_ft_plans_console.getFieldValue("bl.site_id");
		var bl_id = this.ab_ft_plans_console.getFieldValue("bl.bl_id");
		var parameters = null;
		var consoleRestriction = "";

		if (valueExistsNotEmpty(site_id)) {
			consoleRestriction += "(rm.bl_id IN (SELECT DISTINCT bl_id FROM bl WHERE site_id='" + site_id + "'))";
		}
		
		if (valueExistsNotEmpty(bl_id)) {
			consoleRestriction += (consoleRestriction != "" ? " AND " : "");
			consoleRestriction += "rm.bl_id = '" + bl_id + "'";
		...
		View.openPaginatedReportDialog('ab-ft-plans-pgrp.axvw', null, parameters);
	}
});
...
function loadDrawingPanel(node){
    ...
    selectedFl = currentNode.data['bl.state_id'];
	selectedDwgName = currentNode.data['bl.dwgname'];
