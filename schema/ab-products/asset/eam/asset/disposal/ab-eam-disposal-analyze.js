...
var dispAnalyseFilterConfig = new Ext.util.MixedCollection();

dispAnalyseFilterConfig.addAll(
...
var abEamDisposalAnalyzeCtrl = View.createController('abEamDisposalAnalyzeCtrl', {
	...
	afterViewLoad: function(){
		...
		this.filterController.initializeConfigObjects(dispAnalyseFilterConfig);
		var controller = this;
	...
	afterInitialDataFetch: function(){
		var controller = this;
		var selectedTabName = this.abEamAssetDisposalAnalyzeTree.getSelectedTabName();
		this.abEamAssetDisposalAnalyzeTree.refreshTab(selectedTabName);
		...
		if(valueExists(this.assetRegistryController)){
			...
			// was not loaded yet
			this.abAssetDispAnalyzeRegistryByType.parameters = {'assetByTypeConfig': assetByTypeConfig};
		}
	...
	onFilter: function(restriction){
		...
		this.abEamAssetDisposalAnalyzeTree.findTab('abEamAssetDisposalAnalyzeTreeGeo').parameters = geoTreeParameters;
		...
		this.abEamAssetDisposalAnalyzeTree.findTab('abEamAssetDisposalAnalyzeTreeLocation').parameters = locationTreeParameters;

		var orgTreeParameters = getOrganizationTreeRestrictionFromAssetFilter(this.filterRestriction, parameters);
		this.abEamAssetDisposalAnalyzeTree.findTab('abEamAssetDisposalAnalyzeTreeOrg').parameters = orgTreeParameters;

		var projTreeParameters = getProjectTreeRestrictionFromAssetFilter(this.filterRestriction, parameters);
		this.abEamAssetDisposalAnalyzeTree.findTab('abEamAssetDisposalAnalyzeTreeProj').parameters = projTreeParameters;

		_.extend(parameters, {
			'filterRestriction': this.filterRestriction
		});
		this.abEamAssetDisposalAnalyzeTree.findTab('abEamAssetDisposalAnalyzeEqSystems').parameters = parameters;
	...
	onClickMarkForAction: function(parentPanel, context){
		...
		this.abAnalyzePendingActionSelect.clear();
		this.abAnalyzePendingActionSelect.showInWindow({
			anchor: context,
			width: 400,
			height: 200,
			closeButton: false
	...
	abAnalyzePendingActionSelect_onSave: function(){
		var pendingAction = this.abAnalyzePendingActionSelect.getFieldValue('pending_action');
		if (updateAssetField(this.assetRegistryController.view, this.selectedRecords, 'pending_action', pendingAction)) {
			...
			this.abAnalyzePendingActionSelect.closeWindow();
		}
	},
	...
	getAssetRegistryController: function(){
		...
		} else if(valueExists(this.abAssetDispAnalyzeRegistryByType.contentView)
				&& valueExists(this.abAssetDispAnalyzeRegistryByType.contentView.controllers)
				&& valueExists(this.abAssetDispAnalyzeRegistryByType.contentView.controllers.get('abEamAssetByTypeCtrl'))){
			result = this.abAssetDispAnalyzeRegistryByType.contentView.controllers.get('abEamAssetByTypeCtrl');
		}
		return result;
	},
