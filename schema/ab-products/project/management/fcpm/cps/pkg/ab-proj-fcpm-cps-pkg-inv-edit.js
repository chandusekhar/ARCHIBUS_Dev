var projFcpmCpsPkgInvEditController = View.createController('projFcpmCpsPkgInvEdit', {
	project_id: '',
	taxRate: 0,
	
	afterInitialDataFetch: function() {		
		var amount_lien = replaceNullValueWithZero(this.projFcpmCpsPkgInvEdit_inv.getFieldValue('invoice.amount_lien'));
		if (amount_lien == 0) $('apply_lien_holdback_no').checked = true;
		else $('apply_lien_holdback_yes').checked = true;
	},
	
	projFcpmCpsPkgInvEdit_inv_afterRefresh: function() {
		var status = this.projFcpmCpsPkgInvEdit_inv.getFieldValue('invoice.status');
		if (status != 'ISSUED') {
			this.projFcpmCpsPkgInvEdit_inv.actions.get('save').show(false);
			this.projFcpmCpsPkgInvEdit_inv.actions.get('next').show(true);
			$('apply_lien_holdback_no').disabled = true;
			$('apply_lien_holdback_yes').disabled = true;
		} else {
			this.projFcpmCpsPkgInvEdit_inv.actions.get('save').show(true);
			this.projFcpmCpsPkgInvEdit_inv.actions.get('next').show(false);
			$('apply_lien_holdback_no').disabled = false;
			$('apply_lien_holdback_yes').disabled = false;
		}
	},
	
	projFcpmCpsPkgInvEdit_inv2_afterRefresh: function() {
		/*var default_value = getMessage('na');
		if (this.projFcpmCpsPkgInvEdit_inv2.getFieldValue('invoice.fac_org_level_01') == '') this.projFcpmCpsPkgInvEdit_inv2.setFieldValue('invoice.fac_org_level_01', default_value);
		if (this.projFcpmCpsPkgInvEdit_inv2.getFieldValue('invoice.fac_org_level_02') == '') this.projFcpmCpsPkgInvEdit_inv2.setFieldValue('invoice.fac_org_level_02', default_value);
		if (this.projFcpmCpsPkgInvEdit_inv2.getFieldValue('invoice.fac_org_level_03') == '') this.projFcpmCpsPkgInvEdit_inv2.setFieldValue('invoice.fac_org_level_03', default_value);
		if (this.projFcpmCpsPkgInvEdit_inv2.getFieldValue('invoice.fac_org_level_04') == '') this.projFcpmCpsPkgInvEdit_inv2.setFieldValue('invoice.fac_org_level_04', default_value);
		if (this.projFcpmCpsPkgInvEdit_inv2.getFieldValue('invoice.fac_org_level_05') == '') this.projFcpmCpsPkgInvEdit_inv2.setFieldValue('invoice.fac_org_level_05', default_value);
		if (this.projFcpmCpsPkgInvEdit_inv2.getFieldValue('invoice.fac_org_level_06') == '') this.projFcpmCpsPkgInvEdit_inv2.setFieldValue('invoice.fac_org_level_06', default_value);
		 */
		var status = this.projFcpmCpsPkgInvEdit_inv2.getFieldValue('invoice.status');
		if (status != 'ISSUED') {
	...
	enableEditButtons: function(enable) {
		this.projFcpmCpsPkgInvEdit_inv2.actions.get('save').show(enable);
		this.projFcpmCpsPkgInvEdit_inv2.actions.get('approve').show(enable);
		this.projFcpmCpsPkgInvEdit_inv2.actions.get('reject').show(enable);
		this.projFcpmCpsPkgInvEdit_inv2.actions.get('saveDis').show(!enable);
		this.projFcpmCpsPkgInvEdit_inv2.actions.get('approveDis').show(!enable);
		this.projFcpmCpsPkgInvEdit_inv2.actions.get('rejectDis').show(!enable);
	},
	
	projFcpmCpsPkgInvEdit_inv2_onReject: function() {
		this.projFcpmCpsPkgInvEdit_inv2.clearValidationResult();
		var invoice_id = this.projFcpmCpsPkgInvEdit_inv2.getFieldValue('invoice.invoice_id');
		var controller = this;
        var message = getMessage('confirmReject');
        View.confirm(message, function(button){
        	if (button == 'yes') {
        		if (!controller.projFcpmCpsPkgInvEdit_inv2.canSave()) return;
	        	controller.projFcpmCpsPkgInvEdit_inv2.setFieldValue('invoice.status', 'REJECTED');
	        	controller.projFcpmCpsPkgInvEdit_inv2.save();
	        	controller.enableEditButtons(false);
				View.getOpenerView().panels.get('projFcpmCpsPkgInvGrid').refresh();
        	}
        });
	},
	
	projFcpmCpsPkgInvEdit_inv2_onApprove: function() {
		this.projFcpmCpsPkgInvEdit_inv2.clearValidationResult();
		var invoice_id = this.projFcpmCpsPkgInvEdit_inv2.getFieldValue('invoice.invoice_id');
		var controller = this;
        ...
        View.confirm(message, function(button){
        	if (button == 'yes') {
				if (!controller.projFcpmCpsPkgInvEdit_inv2.canSave()) return;
				controller.projFcpmCpsPkgInvEdit_inv2.setFieldValue('invoice.status', 'SENT');
				controller.projFcpmCpsPkgInvEdit_inv2.save();
				controller.enableEditButtons(false);
				View.getOpenerView().panels.get('projFcpmCpsPkgInvGrid').refresh();
        	}
...
function onChangeApplyLien() {
	var form = View.panels.get('projFcpmCpsPkgInvEdit_inv');
...
function calculateLienTax() {
	var controller = View.controllers.get('projFcpmCpsPkgInvEdit');
	var form = View.panels.get('projFcpmCpsPkgInvEdit_inv');
...
function truncToTwoDecimals(number, fieldName) {
	...
	if (fieldName) number = View.dataSources.get('projFcpmCpsPkgInvEditDs1').formatValue(fieldName, number, true);
