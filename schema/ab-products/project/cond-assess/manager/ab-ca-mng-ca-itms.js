...
var mngCondAssessController = View.createController('mngCondAssessCtrl',{
    ...
    // KB 3046065:  View Service Requests
...
//		titleObj.on('click', this.showPDAMenu, this, null);
	...
	repMngCondAssess_onPaginatedReport: function(){
		...
		if (this.view.taskInfo.activityId == 'AbCapitalPlanningCA') {
			...
			View.openPaginatedReportDialog('ab-comm-mng-ca-itms-rpt.axvw', {
	...
	repMngCondAssess_onCreateWorkReq: function(row){
        ...
        // KB 3046065:  View Service Requests
        if (row.getFieldValue('activity_log.hasRequests')=='1') {
          this.assessmentRecord = row;
		  var restriction = new Ab.view.Restriction();
          restriction.addClause('activity_log.assessment_id',row.getFieldValue('activity_log.activity_log_id'),'=');
          this.repMngCondAssessSvcReq.refresh(restriction);
          this.repMngCondAssessSvcReq.appendTitle(row.getFieldValue('activity_log.activity_log_id'));
          this.repMngCondAssessSvcReq.showInWindow({});    
        }
		else if(!createWorkRequest(this.view, row, function(){
	 ...
	 */
    // KB 3046065:  View Service Requests
	repMngCondAssessSvcReq_onAddSvcReq: function(){
		var controller = this;
		var rowIndex = this.assessmentRecord.getIndex();
        createWorkRequest(this.view, this.assessmentRecord, function(){
			controller.mngCondAssessFilterPanel_onShow();
			controller.repMngCondAssess.selectRow(rowIndex);
            controller.repMngCondAssessSvcReq.refresh();
		});
	},
	...
	repMngCondAssess_onAddCondAssess: function(){
		...
		this.consoleRestriction = this.mngCondAssessFilterPanel.getRecord().toRestriction();
		restriction.addClauses(this.consoleRestriction, false);
		View.openDialog('ab-ca-edit-ca-itm.axvw', null, true, { 
