...
var editCondAssessController = View.createController('editCondAssessCtrl',{
	    ...
	    if (matchAssessor) {
			if (this.validateBlAndSite(this.editCondAssessPanel)) {
				if (this.editCondAssessPanel.canSave()) {
					...
					if (!this.setActivitylogActQuestValue()) {
						View.showMessage(getMessage('invalidQuestValues'));
						return;
					}
	...
	setDefaultValues: function(){
		...
		} else if (valueExists(this.restriction)) {
			for(var i=0; i< this.restriction.clauses.length; i++){
				panel.setFieldValue(this.restriction.clauses[i].name, this.restriction.clauses[i].value);
			}
		}
	},
