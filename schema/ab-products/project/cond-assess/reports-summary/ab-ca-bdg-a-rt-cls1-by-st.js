...
var caSumCls1BySiteController = View.createController('caSumCls1BySiteCtrl', {
	// selected projects
	selectedProjectIds: [],
	//console restriction
	consoleRestriction: '',
	...
	caFilterPanel_onShow: function(){
		...
		this.listCASumCls1BySiteSites.refresh(this.consoleRestriction + this.csi_parameter_site);
		this.repCASumCls1BySite.show(false);
		this.repCASumCls1BySiteDetails.addParameter("consoleRestriction", this.consoleRestriction + this.csi_parameter_site);
	},
	/** 
	...
	caFilterPanel_onPaginatedReport: function(){
		...
		View.openPaginatedReportDialog("ab-ca-bdg-a-rt-cls1-by-st-prnt.axvw",
		{
...
function showSummary(row){
	caSumCls1BySiteController.site_id = row['activity_log.site_id'];
	var restriction  = caSumCls1BySiteController.consoleRestriction;
	restriction += " AND activity_log.site_id = '"+ row['activity_log.site_id'] +"'";
	caSumCls1BySiteController.repCASumCls1BySiteDetails.addParameter("siteId", " AND activity_log.site_id = '"+ row['activity_log.site_id'] +"'");
	caSumCls1BySiteController.repCASumCls1BySite.addParameter('consoleRestriction', restriction);
	caSumCls1BySiteController.repCASumCls1BySite.addParameter('classification_id', caSumCls1BySiteController.csi_parameter_summary);
	caSumCls1BySiteController.repCASumCls1BySite.refresh();
}
...
function setRestrictionToXLS(){
	var controller = caSumCls1BySiteController;
	...
	controller.repCASumCls1BySiteDetails.restriction = restriction;
}
