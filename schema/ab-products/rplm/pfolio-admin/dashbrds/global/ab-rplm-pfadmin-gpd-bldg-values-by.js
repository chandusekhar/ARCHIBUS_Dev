var abRplmPfadminGpdBldgValuesByCtrl = View.createController('abRplmPfadminGpdBldgValuesByCtrl', {
	...
	afterInitialDataFetch: function(){
		...
		if(this.view.parameters){
			if(this.view.parameters.maximize){
				this.abRplmPfadminGpdBldgValuesByLocation_chart.setInstructions(instructionLabel);
			}
		}
		
		this.chartLevel = this.getChartLevel(this.objFilter);
		
		var objMap = View.panels.get("abRplmPfadminGpdBldgValuesByLocation_chart");
		var detailsPanelId = bldgByValues_detailsPanelsObj.get(this.chartLevel).panelId;
		...
		objMap.refresh();
		objMap.setTitle(getMessage("labelBldgValuesBy") + " " + this.getTitleByLevel(this.chartLevel));
