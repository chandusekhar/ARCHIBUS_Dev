var abRplmPfadminGpdOccupancyByCtrl = View.createController('abRplmPfadminGpdOccupancyByCtrl', {
	...
	afterInitialDataFetch: function(){
		...
		if(this.view.parameters){
			if(this.view.parameters.maximize){
				this.abRplmPfadminGpdOccupancyByLocation_chart.setInstructions(instructionLabel);
			}
		}
		
		this.chartLevel = this.getChartLevel(this.objFilter);
		
		var objMap = View.panels.get("abRplmPfadminGpdOccupancyByLocation_chart");
		var detailsPanelId = occupancyBy_detailsPanelsObj.get(this.chartLevel).panelId;
		...
		objMap.refresh();
		objMap.setTitle(getMessage("labelOccupancyBy") + " " + this.getTitleByLevel(this.chartLevel));
