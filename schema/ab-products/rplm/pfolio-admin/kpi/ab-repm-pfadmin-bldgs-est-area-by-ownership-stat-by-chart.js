// filter settings
var bldgEstAreaByOwnershipFilter = new Ext.util.MixedCollection();
bldgEstAreaByOwnershipFilter.addAll(
 ...
 */
var abRepmBldgEstAreaByOwnershipController = View.createController('abRepmBldgEstAreaByOwnershipController', {
	...
	afterViewLoad: function(){
		...
		this.abRepmBldgEstAreaByOwneshipChart.controllerId = 'abRepmBldgEstAreaByOwnershipController';
		this.abRepmBldgEstAreaByOwneshipChart.config.showLegendOnLoad = (!isInDashboard(this.view));
		// hide title when is in dashboard
		if (isInDashboard(this.view)) {
			// we must read some config parameters from dashboard view
			this.dashConfig = getDashboardConfigObject(this, 'abRepmBldgEstAreaByOwneshipChart');
		...
		if (valueExists(this.view.parameters) && valueExists(this.view.parameters["crtFilter"]) ) {
			...
			this.crtFilter = bldgEstAreaByOwnershipFilter.clone();
		}
	 ...
	 * Open filter dialog 
	 */
	abRepmBldgEstAreaByOwneshipChart_onFilter: function(){
		onOpenFilter(this);
	},
	
	abRepmBldgEstAreaByOwneshipChart_onReport: function(){
		var gridPanel = this.abRepmBldgEstAreaByOwnershipByReport;
		...
		gridPanel.update();
		gridPanel.setTitle(getMessage("titleBldgAreaByOwnershipBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
	...
	refreshChart: function() {
		var chartPanel = this.abRepmBldgEstAreaByOwneshipChart;
		// get group by parameter
		this.crtGroupBy = this.crtFilter.get('group_by').value;
		var groupByField = getGroupByField(this.crtGroupBy, 'bl');
		...
		if (isInDashboard(this.view)) {
			chartPanel.setTitle(getMessage("titleBldgAreaByOwnershipBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
			chartPanel.actions.get('filter').show(false);
			chartPanel.actions.get('report').show(false);
			chartPanel.actions.get('export').show(false);
		} else{
			this.view.setTitle(getMessage("titleBldgAreaByOwnershipBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
			chartPanel.title = getMessage("titleBldgAreaByOwnershipBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy));
...
//			});
			...
			if (!isInDashboard(this.view)){
				...
				opener.openDialog('ab-repm-pfadmin-bldgs-est-area-by-ownership-stat-by-chart.axvw', null, false, {
				    closeButton: true,
				    maximize: true,
