// filter settings
var landEstAreaByOwnershipFilter = new Ext.util.MixedCollection();
landEstAreaByOwnershipFilter.addAll(
 ...
 */
var abRepmLandEstAreaByOwnershipController = View.createController('abRepmLandEstAreaByOwnershipController', {
	...
	afterViewLoad: function(){
		...
		this.abRepmLandEstAreaByOwneshipChart.controllerId = 'abRepmLandEstAreaByOwnershipController';
		this.abRepmLandEstAreaByOwneshipChart.config.showLegendOnLoad = (!isInDashboard(this.view));
		// hide title when is in dashboard
		if (isInDashboard(this.view)) {
			// we must read some config parameters from dashboard view
			this.dashConfig = getDashboardConfigObject(this, 'abRepmLandEstAreaByOwneshipChart');
		...
		if (valueExists(this.view.parameters) && valueExists(this.view.parameters["crtFilter"]) ) {
			...
			this.crtFilter = landEstAreaByOwnershipFilter.clone();
		}
	 ...
	 * Open filter dialog 
	 */
	abRepmLandEstAreaByOwneshipChart_onFilter: function(){
		onOpenFilter(this);
	},
	
	abRepmLandEstAreaByOwneshipChart_onReport: function(){
		var gridPanel = this.abRepmLandEstAreaByOwnershipByReport;
		...
		gridPanel.update();
		gridPanel.setTitle(getMessage("titleLandAreaByOwnershipBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
	...
	refreshChart: function() {
		var chartPanel = this.abRepmLandEstAreaByOwneshipChart;
		// get group by parameter
		this.crtGroupBy = this.crtFilter.get('group_by').value;
		var groupByField = getGroupByField(this.crtGroupBy, 'property');
		...
		if (isInDashboard(this.view)) {
			chartPanel.setTitle(getMessage("titleLandAreaByOwnershipBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
			chartPanel.actions.get('filter').show(false);
			chartPanel.actions.get('report').show(false);
			chartPanel.actions.get('export').show(false);
		} else{
			this.view.setTitle(getMessage("titleLandAreaByOwnershipBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
			chartPanel.title = getMessage("titleLandAreaByOwnershipBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy));
...
//			});
			...
			if (!isInDashboard(this.view)){
				...
				opener.openDialog('ab-repm-pfadmin-land-est-area-by-ownership-stat-by-chart.axvw', null, false, {
				    closeButton: true,
				    maximize: true,
