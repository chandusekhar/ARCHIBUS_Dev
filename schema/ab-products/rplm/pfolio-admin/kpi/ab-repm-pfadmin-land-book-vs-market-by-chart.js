// filter settings
var landBookMarketValFilter = new Ext.util.MixedCollection();
landBookMarketValFilter.addAll(
 ...
 */
var abRepmLandBookMarketValController = View.createController('abRepmLandBookMarketValController', {
	...
	afterViewLoad: function(){
		...
		this.abRepmLandBookMarketValChart.controllerId = 'abRepmLandBookMarketValController';
		this.abRepmLandBookMarketValChart.config.showLegendOnLoad = (!isInDashboard(this.view));
		// hide title when is in dashboard
		if (isInDashboard(this.view)) {
			// we must read some config parameters from dashboard view
			this.dashConfig = getDashboardConfigObject(this, 'abRepmLandBookMarketValChart');
		...
		if (valueExists(this.view.parameters) && valueExists(this.view.parameters["crtFilter"]) ) {
			...
			this.crtFilter = landBookMarketValFilter.clone();
		}
	 ...
	 * Open filter dialog 
	 */
	abRepmLandBookMarketValChart_onFilter: function(){
		onOpenFilter(this);
	},
	
	abRepmLandBookMarketValChart_onReport: function(){
		var gridPanel = this.abRepmLandBookMarketValReport;
		...
		gridPanel.update();
		gridPanel.setTitle(getMessage("titleLandBookMarketValBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
	...
	refreshChart: function() {
		var chartPanel = this.abRepmLandBookMarketValChart;
		// get group by parameter
		this.crtGroupBy = this.crtFilter.get('group_by').value;
		var groupByField = getGroupByField(this.crtGroupBy, 'property');
		...
		if (isInDashboard(this.view)) {
			chartPanel.setTitle(getMessage("titleLandBookMarketValBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
			chartPanel.actions.get('filter').show(false);
			chartPanel.actions.get('report').show(false);
			chartPanel.actions.get('export').show(false);
		} else{
			this.view.setTitle(getMessage("titleLandBookMarketValBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
			chartPanel.title = getMessage("titleLandBookMarketValBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy));
...
//			});
			...
			if (!isInDashboard(this.view)){
				...
				opener.openDialog('ab-repm-pfadmin-land-book-vs-market-by-chart.axvw', null, false, {
				    closeButton: true,
				    maximize: true,
