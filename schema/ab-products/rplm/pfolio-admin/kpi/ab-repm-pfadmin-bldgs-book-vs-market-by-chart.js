// filter settings
var bldgBookMarketValFilter = new Ext.util.MixedCollection();
bldgBookMarketValFilter.addAll(
		...
		{id: 'ownership', custom: true, columnName:null, visible: true, disabled: true, defaultValue: 'owned', value: null, values:['owned', 'leased', 'all'], restriction:{
			...
			'leased': "EXISTS(SELECT 1 FROM ls WHERE ls.use_as_template = 0 AND ls.bl_id = bl.bl_id AND ls.signed = 1 AND (ls.date_end >= ${sql.currentDate} OR ls.date_end IS NULL) AND ls.date_start <= ${sql.currentDate})",
			'all':''
		}, labelId: 'label_ownership', optionLabel: 'labelOwnership_'},
		{id: 'time_span', custom: true, columnName:null, visible: false, disabled: false, defaultValue: 'past1', value: null, values:['past5', 'past3', 'past1', 'next1', 'next3', 'next5'], labelId: 'label_timespan', optionLabel: 'labelTimeSpan_'},
 ...
 */
var abRepmBldgBookMarketValController = View.createController('abRepmBldgBookMarketValController', {
	...
	afterViewLoad: function(){
		...
		this.abRepmBldgBookMarketValChart.controllerId = 'abRepmBldgBookMarketValController';
		this.abRepmBldgBookMarketValChart.config.showLegendOnLoad = (!isInDashboard(this.view));
		// hide title when is in dashboard
		if (isInDashboard(this.view)) {
			// we must read some config parameters from dashboard view
			this.dashConfig = getDashboardConfigObject(this, 'abRepmBldgBookMarketValChart');
		...
		if (valueExists(this.view.parameters) && valueExists(this.view.parameters["crtFilter"]) ) {
			...
			this.crtFilter = bldgBookMarketValFilter.clone();
		}
	 ...
	 * Open filter dialog 
	 */
	abRepmBldgBookMarketValChart_onFilter: function(){
		onOpenFilter(this);
	},
	
	abRepmBldgBookMarketValChart_onReport: function(){
		var gridPanel = this.abRepmBldgBookMarketValReport;
		...
		gridPanel.update();
		gridPanel.setTitle(getMessage("titleBldgBookMarketValBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
	...
	refreshChart: function() {
		var chartPanel = this.abRepmBldgBookMarketValChart;
		// get group by parameter
		this.crtGroupBy = this.crtFilter.get('group_by').value;
		var groupByField = getGroupByField(this.crtGroupBy, 'bl');
		...
		chartPanel.groupingAxis[0].title = getMessage("labelGroupBy_" +  this.crtGroupBy);
		chartPanel.config.groupingAxis[0].title = getMessage("labelGroupBy_" +  this.crtGroupBy);
		chartPanel.configObj.groupingAxis[0].title = getMessage("labelGroupBy_" +  this.crtGroupBy);
		//Kb 3051960 Commented this line to fix Chrome error
		//chartPanel.loadChartSWFIntoFlash();
		chartPanel.refresh();
		
		if (isInDashboard(this.view)) {
			chartPanel.setTitle(getMessage("titleBldgBookMarketValBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
			chartPanel.actions.get('filter').show(false);
			chartPanel.actions.get('report').show(false);
			chartPanel.actions.get('export').show(false);
		} else{
			this.view.setTitle(getMessage("titleBldgBookMarketValBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
			chartPanel.title = getMessage("titleBldgBookMarketValBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy));
...
//			});
			...
			if (!isInDashboard(this.view)){
				...
				opener.openDialog('ab-repm-pfadmin-bldgs-book-vs-market-by-chart.axvw', null, false, {
				    closeButton: true,
				    maximize: true,
