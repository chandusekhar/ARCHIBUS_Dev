// filter settings
var pfadminSalesFilter = new Ext.util.MixedCollection();
pfadminSalesFilter.addAll(
 ...
 */
var abRplmPfadminSalesController = View.createController('abRplmPfadminSalesController', {
	...
	afterViewLoad: function(){
		...
		this.chartSales.controllerId = 'abRplmPfadminSalesController';
		this.chartSales.config.showLegendOnLoad = (!isInDashboard(this.view));
		this.crtFilter = pfadminSalesFilter.clone();
	 ...
	 * Open filter dialog 
	 */
	chartSales_onFilter: function(){
		onOpenFilter(this);
	},
	
	chartSales_onReport: function(){
		var gridPanel = this.reportSales;
		// time span
		...
		gridPanel.update();
		gridPanel.setTitle(getMessage("titlePfadminSales"));
	...
	refreshChart: function() {
		var chartPanel = this.chartSales;
		...
		if (isInDashboard(this.view)) {
			chartPanel.setTitle(getMessage("titlePfadminSales"));
			chartPanel.actions.get('filter').show(false);
			chartPanel.actions.get('report').show(false);
			chartPanel.actions.get('export').show(false);
		} else{
			this.view.setTitle(getMessage("titlePfadminSales"));
			chartPanel.title = getMessage("titlePfadminSales");
