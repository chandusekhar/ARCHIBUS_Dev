// filter settings
var landMonthlyCapTransFilter = new Ext.util.MixedCollection();
landMonthlyCapTransFilter.addAll(
 ...
 */
var abRepmLandMonthlyCapTransController = View.createController('abRepmLandMonthlyCapTransController', {
	...
	afterViewLoad: function(){
		...
		this.abRepmLandMonthlyCapTransChart.controllerId = 'abRepmLandMonthlyCapTransController';
		this.abRepmLandMonthlyCapTransChart.config.showLegendOnLoad = (!isInDashboard(this.view));
		// hide title when is in dashboard
		if (isInDashboard(this.view)) {
			// we must read some config parameters from dashboard view
			this.dashConfig = getDashboardConfigObject(this, 'abRepmLandMonthlyCapTransChart');
		...
		// when is called from drilldown in dashboard
		if (valueExists(this.view.parameters) && valueExists(this.view.parameters["crtFilter"]) ) {
			...
			this.crtFilter = landMonthlyCapTransFilter.clone();
		}
	 ...
	 * Open filter dialog 
	 */
	abRepmLandMonthlyCapTransChart_onFilter: function(){
		onOpenFilter(this);
	},
	
	abRepmLandMonthlyCapTransChart_onReport: function(){
		var gridPanel = this.abRepmLandMonthlyCapTransReport;
		...
		gridPanel.update();
		gridPanel.setTitle(getMessage("titleLandMonthlyCapTransBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
	...
	refreshChart: function() {
		var chartPanel = this.abRepmLandMonthlyCapTransChart;
		// get group by parameter
		this.crtGroupBy = this.crtFilter.get('group_by').value;
		var groupByField = getGroupByField(this.crtGroupBy, 'property');
		...
		if (isInDashboard(this.view)) {
			chartPanel.setTitle(getMessage("titleLandMonthlyCapTransBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
			chartPanel.actions.get('filter').show(false);
			chartPanel.actions.get('report').show(false);
			chartPanel.actions.get('export').show(false);
		} else{
			this.view.setTitle(getMessage("titleLandMonthlyCapTransBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
			chartPanel.title = getMessage("titleLandMonthlyCapTransBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy));
...
//			});
			...
			if (!isInDashboard(this.view)){
				...
				opener.openDialog('ab-repm-pfadmin-land-monthly-capital-trans-by-chart.axvw', null, false, {
				    closeButton: true,
				    maximize: true,
