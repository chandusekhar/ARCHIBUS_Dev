// filter settings
var structBookMarketValFilter = new Ext.util.MixedCollection();
structBookMarketValFilter.addAll(
 ...
 */
var abRepmStructBookMarketValController = View.createController('abRepmStructBookMarketValController', {
	...
	afterViewLoad: function(){
		...
		this.abRepmStructBookMarketValChart.controllerId = 'abRepmStructBookMarketValController';
		this.abRepmStructBookMarketValChart.config.showLegendOnLoad = (!isInDashboard(this.view));
		// hide title when is in dashboard
		if (isInDashboard(this.view)) {
			// we must read some config parameters from dashboard view
			this.dashConfig = getDashboardConfigObject(this, 'abRepmStructBookMarketValChart');
		...
		if (valueExists(this.view.parameters) && valueExists(this.view.parameters["crtFilter"]) ) {
			...
			this.crtFilter = structBookMarketValFilter.clone();
		}
	 ...
	 * Open filter dialog 
	 */
	abRepmStructBookMarketValChart_onFilter: function(){
		onOpenFilter(this);
	},
	
	abRepmStructBookMarketValChart_onReport: function(){
		var gridPanel = this.abRepmStructBookMarketValReport;
		...
		gridPanel.update();
		gridPanel.setTitle(getMessage("titleStructBookMarketValBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
	...
	refreshChart: function() {
		var chartPanel = this.abRepmStructBookMarketValChart;
		// get group by parameter
		this.crtGroupBy = this.crtFilter.get('group_by').value;
		var groupByField = getGroupByField(this.crtGroupBy, 'property');
		...
		if (isInDashboard(this.view)) {
			chartPanel.setTitle(getMessage("titleStructBookMarketValBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
			chartPanel.actions.get('filter').show(false);
			chartPanel.actions.get('report').show(false);
			chartPanel.actions.get('export').show(false);
		} else{
			this.view.setTitle(getMessage("titleStructBookMarketValBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy)));
			chartPanel.title = getMessage("titleStructBookMarketValBy").replace("{0}", getMessage("labelGroupBy_" +  this.crtGroupBy));
...
//			});
			...
			if (!isInDashboard(this.view)){
				...
				opener.openDialog('ab-repm-pfadmin-struc-book-vs-market-by-chart.axvw', null, false, {
				    closeButton: true,
				    maximize: true,
