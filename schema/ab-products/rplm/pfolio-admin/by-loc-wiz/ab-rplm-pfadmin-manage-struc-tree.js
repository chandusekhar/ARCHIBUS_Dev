var structTreeController = View.createController('structTree',{
	// filter settings
	filter:{
				'city_id':'',
				'state_id':'',
	...
	applyFilter: function(filter){
		controller = View.controllers.get('structTree');
	...
	ctryTree_onShowSelected: function(){
		...
		var structNodes = this.tree.getSelectedNodes(2);
		if(cityNodes.length == 0 && structNodes.length == 0){
			View.showMessage(getMessage('error_noselection'));
			return;
		}
		
		// we need to pass an array with selected structures id's
		var maxItemsNo = getMaxItemsNo();
		var items = new Array();
		var showNotGeocodedMessage = false;
		// add selected structures from tree
		for(var i = 0; i < structNodes.length; i++ ) {
			var node = structNodes[i];
			if(! valueExistsNotEmpty(node.data['property.lat']) || !valueExistsNotEmpty(node.data['property.lon'])) {
				showNotGeocodedMessage = true;
			}
			var structId = "'"+ node.data['property.pr_id.key']+"'";
			items.push(structId);
		}
		// get structures for selected cities that don't have child nodes displayed
		var dsStruct = View.dataSources.get('dsStrucStructures');
		dsStruct.addParameter('filterRestriction', this.filterRestriction );
		dsStruct.addParameter('activeClause', this.activeRestriction );
		for(var i = 0; i < cityNodes.length; i++){
			var node = cityNodes[i];
			if(node.children.length == 0){
				// childrens are not displayed. Read from database
				var records = dsStruct.getRecords(node.restriction);
				for ( var j=0; j< records.length; j++ ){
					var rec = records[j];
					var structId = "'"+ rec.getValue('property.pr_id')+"'";
					if(! valueExistsNotEmpty(rec.getValue('property.lat')) || !valueExistsNotEmpty(rec.getValue('property.lon'))) {
						showNotGeocodedMessage = true;
						this.notGeocodedItems.push(structId);
					}
					items.push(structId);
				}
			}
		...
		var isValidGisLicense = this.panel_row1col2.contentView.controllers.get('abStructureMangementTab').isValidGisLicense;
		...
		if (ctrlMap != null) {
			ctrlMap.showSelectedStructures(items, showNotGeocodedMessage);
		}
		var ctrlDetails = this.panel_row1col2.contentView.controllers.get('structureManagementTableDetails');
...
function afterGeneratingTreeNode(node){
	var controller = View.controllers.get('structTree');
