 ...
 */
var landManagementTableDetailsController = View.createController('landManagementTableDetails', {
	//report button
	menuParent: null,
	// report menu
	...
	afterInitialDataFetch: function(){
		if(this.view.parameters){
			if(this.view.parameters.maximize){
				...
				if(openerView){
					this.items = openerView.controllers.get('landTree').selectedItems;
	...
	initializeView: function(){
		...
		this.landGrid.refresh(restriction);
        
	},
	/**
	 * Click "Details" event handler
	 */
	landGrid_detail_onClick:function(row){
		var row = row;
