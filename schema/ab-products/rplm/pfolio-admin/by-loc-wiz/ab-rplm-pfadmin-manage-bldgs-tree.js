var bldgTreeController = View.createController('bldgTree', {
	...
	getRestriction: function(filter){
		...
		if ( valueExistsNotEmpty(filter['su_manual_area_max'])) {
			restriction += " AND ((SELECT SUM(su.area_manual) FROM su WHERE su.bl_id = bl.bl_id) * " + this.areaUnitsConversionFactor + ") < " + filter['su_manual_area_max'];
		}
		if ( valueExistsNotEmpty(filter['su_manual_area_min'])) {
			restriction += " AND ((SELECT SUM(su.area_manual) FROM su WHERE su.bl_id = bl.bl_id) * " + this.areaUnitsConversionFactor + ") > " + filter['su_manual_area_min'];
		}
		
		if ( valueExistsNotEmpty(filter['su_usable_area_max'])) {
			restriction += " AND ((SELECT SUM(su.area_usable) FROM su WHERE su.bl_id = bl.bl_id) * " + this.areaUnitsConversionFactor + ") < " + filter['su_usable_area_max'];
		}
		if ( valueExistsNotEmpty(filter['su_usable_area_min']) ) {
			restriction += " AND ((SELECT SUM(su.area_usable) FROM su WHERE su.bl_id = bl.bl_id) * " + this.areaUnitsConversionFactor + ") > " + filter['su_usable_area_min'];
		}

		if ( valueExistsNotEmpty(filter['purchasing_cost_max']) ) {
			restriction += " AND (SELECT cost_purchase * ${sql.exchangeRateFromBudgetToUserForDate('Budget', 'ot.date_purchase')} FROM ot WHERE ot.ot_id = (SELECT MAX(c.ot_id) FROM ot c WHERE c.bl_id =  bl.bl_id AND c.status = 'Owned')) < " + filter['purchasing_cost_max'];
		}
		if ( valueExistsNotEmpty(filter['purchasing_cost_min']) ) {
			restriction += " AND (SELECT cost_purchase * ${sql.exchangeRateFromBudgetToUserForDate('Budget', 'ot.date_purchase')} FROM ot WHERE ot.ot_id = (SELECT MAX(c.ot_id) FROM ot c WHERE c.bl_id =  bl.bl_id AND c.status = 'Owned')) > " + filter['purchasing_cost_min'];
		}

		if ( valueExistsNotEmpty(filter['book_value_max']) ) {
			restriction += " AND (bl.value_book * ${sql.exchangeRateFromBudgetToUserForDate('Budget', 'bl.date_book_val')}) < " + filter['book_value_max'];
		}

		if ( valueExistsNotEmpty(filter['book_value_min']) ) {
			restriction += " AND (bl.value_book * ${sql.exchangeRateFromBudgetToUserForDate('Budget', 'bl.date_book_val')}) > " + filter['book_value_min'];
		}

		if ( valueExistsNotEmpty(filter['market_value_max']) ) {
			restriction += " AND (bl.value_market * ${sql.exchangeRateFromBudgetToUserForDate('Budget', 'bl.date_market_val')}) < " + filter['market_value_max'];
		}

		if ( valueExistsNotEmpty(filter['market_value_min']) ) {
			restriction += " AND (bl.value_market * ${sql.exchangeRateFromBudgetToUserForDate('Budget', 'bl.date_market_val')}) > " + filter['market_value_min'];
		}
	...
	ctryTree_onShowSelected: function(){
		...
		if(valueExists(this.panel_row1col2.contentView) && valueExists(this.panel_row1col2.contentView.controllers.get('abBldgMangementTab'))){
			isValidGisLicense = this.panel_row1col2.contentView.controllers.get('abBldgMangementTab').isValidGisLicense;
		}
