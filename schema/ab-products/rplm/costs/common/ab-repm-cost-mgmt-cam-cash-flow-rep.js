var controller = View.createController('calculateCashFlow', {
	...
	console_onFilter : function() {
		...
		var camCost = this.requestParameters["cam_cost"] == ""?"ALL":this.requestParameters["cam_cost"];
		var camCostLabel = getMessage("camCost_" + camCost.replace("-", "_"));
		if (valueExistsNotEmpty(camCostLabel)) {
			panelTitle += " - " +camCostLabel;
		}
	...
	getConsoleRestriction: function(){
		...
		// CAM COST
        var camCostSelectObj = document.getElementById("camCosts");
        if (camCostSelectObj) {
        	var cam_cost = camCostSelectObj.value;
        	if (valueExistsNotEmpty(cam_cost) && cam_cost != "ALL") {
        		requestParameters["cam_cost"] = cam_cost;
        	}
        }
...
function onClickCashFlowDetails(event){
	...
	View.openDialog('ab-rplm-cost-mgmt-cam-cash-flow-rep-details.axvw', null, true, {
            width: 1000,
            height: 400,
            closeButton: false,
