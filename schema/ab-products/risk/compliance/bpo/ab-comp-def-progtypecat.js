...
var defProgramController = View.createController('defRMTypeCat', {
    ...
    afterViewLoad: function(){
    	...
    	this.treeview.setTreeNodeConfigForLevel(1,           			// index of the tree level
                [{fieldName: 'regprogtype.regprog_type'},   					// field value, full length
                 {fieldName: 'regprogtype.summary', length: 40}]);    	    // field value, trimmed
    },
    ...
    onAddNewButtonPush: function(menuItemId){
        ...
        if (this.curTreeNode) {
            ...
            if (nodeLevelIndex == 0) {
                cateId = this.curTreeNode.data["regprogcat.regprog_cat"];
            }
            else {
                cateId = this.curTreeNode.data["regprogtype.regprog_cat"];
            }
        }
        else {
        ...
        switch (menuItemId) {
            case "CATEGORY":
                ...
                break;
            case "TYPE":
                restriction.addClause("regprogtype.regprog_cat", cateId, '=');
    ...
    cate_detail_onDelete: function(){
        ...
        this.commonDelete("regCatTab1DS", "cate_detail", "regprogcat.regprog_cat");
    },
    type_detail_onDelete: function(){
        this.operDataType = "TYPE";
        this.commonDelete("reGTypeTab2DS", "type_detail", "regprogtype.regprog_type");
    },
    ...
    setCurTreeNodeAfterUpdate: function(curEditPanel, parentNode){
        if (this.operType == "DELETE") {
            ...
            switch (this.operDataType) {
                case "CATEGORY":
                    var pkFieldName = "regprogcat.regprog_cat";
                    break;
                case "TYPE":
                    var pkFieldName = "regprogtype.regprog_type";
                    break;
            }
