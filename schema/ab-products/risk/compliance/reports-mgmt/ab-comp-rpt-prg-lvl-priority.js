var abCompRptRegLvlPriorityController = View.createController('abCompRptRegLvlPriorityController', {
	...
	afterViewLoad : function() {
		...
		var scoreBoard = new ManageReportControl('scoreBoard', '');
	...
	getProgramResForPopup:function(consoleController,consoleRestriction,ob,control ,firstGroupFieldName,firstGroupFieldValue,secondGroupFieldName,secondGroupFieldValue){
		...
		if(firstGroupFieldValue.indexOf('no value') != -1){
			...
			clickObectrestriction=' ${sql.isNull(\'(case when regprogram.comp_level IS  NULL THEN regprogram.comp_level_calc ELSE regprogram.comp_level END)\', "\'Not Entered\'")} IS NULL ';
		}else{
			
			clickObectrestriction=  ' ${sql.isNull(\'(case when regprogram.comp_level IS  NULL THEN regprogram.comp_level_calc ELSE regprogram.comp_level END)\', "\'Not Entered\'")} =\''  + firstGroupFieldValue+'\'';
		}
		if(secondGroupFieldValue.indexOf('no value') != -1){
			clickObectrestriction=clickObectrestriction+" and regprogram."+secondGroupFieldName+" IS NULL";
		}else{
			clickObectrestriction=clickObectrestriction+" and regprogram."+secondGroupFieldName+"='"+secondGroupFieldValue+"'";
