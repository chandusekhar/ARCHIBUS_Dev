		...
		<table align="center" width="100%">
			<tr align="center"><td>
				<xsl:for-each select="$afmTableGroup/afmAction">
					...
					<!-- no delete action -->
					<xsl:if test="$type!='addnew' and $type!='render' and $type!='addNew'">
						<xsl:choose>
							<!-- javascript variables and functions used here are in edit-forms.js -->
								...
								<xsl:choose>
									<xsl:when test="$type='delete'">
										<!--xsl:if test="not(@enabled)">
											<input class="AbActionButtonFormStdWidth" type="button" value="{title}" title="{tip}" onclick='return onDelete("{$afmTableGroupID}","{@serialized}","{@frame}",true)'/>
										</xsl:if-->
									</xsl:when>
									<xsl:otherwise>
										<xsl:choose>
											...
											<xsl:otherwise>
												<input class="AbActionButtonFormStdWidth" type="button" value="{title}" title="{tip}" onclick='return onSave("{$afmTableGroupID}","{@serialized}","{@frame}",true)'/>
											</xsl:otherwise>
										</xsl:choose>
									</xsl:otherwise>
								</xsl:choose>
							...
							<xsl:otherwise>
								<xsl:choose>
									<xsl:when test="$type='addnew'">
										<!--xsl:if test="not(@enabled)">
											<input  class="AbActionButtonFormStdWidth"  type="button" value="{title}" title="{tip}" onclick='return onAddNew("{$afmTableGroupID}","{@serialized}","{@frame}",false)'/>
										</xsl:if-->
									</xsl:when>
									<xsl:otherwise>
										<input  class="AbActionButtonFormStdWidth"  type="button" value="{title}" title="{tip}" onclick='onCancel("{$afmTableGroupID}","{@serialized}","{@frame}",false)'/>
									</xsl:otherwise>
								</xsl:choose>
							</xsl:otherwise>
						</xsl:choose>
