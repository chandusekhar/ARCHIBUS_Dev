...
var controller = View.createController('abEpHighltSystemAndZonesController', {
	...
	afterViewLoad: function() {
        ...
        this.abSystemAndZone_DrawingPanel.addEventListener('onclick', this.onDwgPanelClicked, this);   
        // add event listener for check box click
        this.abSystemZone_rmdetailGrid.addEventListener('onMultipleSelectionChange', this.onRoomGridPanelClicked, this);   
        this.abSystemZone_eqdetailGrid.addEventListener('onMultipleSelectionChange', this.onEquipmentGridPanelClicked, this);   
    ...
    onDwgPanelClicked: function(pk, selected) {     
    	...
    	if (radioValue == "EQ") {
    		this.abSystemZone_eqdetailGrid.gridRows.each(function(row){
        		if (row.getFieldValue("eq.eq_id") == pk[0]) {
        			if (selected) {
        				row.select();
        			} else {
        				row.unselect();
		...
		} else if (radioValue == "RM") {
			this.abSystemZone_rmdetailGrid.gridRows.each(function(row){
    ...
    onRoomGridPanelClicked: function(row) {    	
    	var roomArray = [];
		var room = [row.row.getFieldValue('rm.bl_id'), row.row.getFieldValue('rm.fl_id'), row.row.getFieldValue('rm.rm_id')];
		roomArray.push(room);
    	
    	if (row.row.isSelected()) {
    		this.abSystemAndZone_DrawingPanel.selectAssets(roomArray);
    	} else {
    		this.abSystemAndZone_DrawingPanel.unselectAssets(roomArray);
    	}     	
    },
    
    onEquipmentGridPanelClicked: function(row) {    	
    	var eqArray = [];
		var eq = [row.row.getFieldValue('eq.eq_id')];
		eqArray.push(eq);
    	
    	if (row.row.isSelected()) {
    		this.abSystemAndZone_DrawingPanel.selectAssets(eqArray);
    	} else {
    		this.abSystemAndZone_DrawingPanel.unselectAssets(eqArray);
    	}     	
    }, 
   ...
   onChangeRoomStatus: function(menuItemId) {
	   ...
	   try {
		   ...
		   this.abSystemAndZone_DrawingPanel.refresh();
...
function showDrawing(){ 
    ...
    dwgName = flGrid.rows[flGrid.selectedRowIndex]["fl.dwgname"];
	...
	if (whiPanel == 2){
		...
		ABEP_appendRuleSetForRecoveryStatus("eq", 'abSystemAndZone_DrawingPanel', 'abSystemZones_drawing_eqHighlight');
	}
    //room
	if (whiPanel == 3){
		...
		ABEP_appendRuleSetForRecoveryStatus("rm", 'abSystemAndZone_DrawingPanel', 'abSystemZones_drawing_rmHighlight');
	}
...
function addDrawingByType(assetTypesSuffix, tablename, backgroundSuffix, currentHighlightDS, currentLabelsDS){
    ...
    restriction.addClause(tablename + '.dwgname', dwgName, '=');
    ...
    if (assetType == "rm" || assetType == "eq" ) {
    	drawingPanel.assignMode = "2";
    	drawingPanel.multipleSelectionEnabled = true;
    } else {
    	drawingPanel.assignMode = "";
    	drawingPanel.multipleSelectionEnabled = false;
    }
