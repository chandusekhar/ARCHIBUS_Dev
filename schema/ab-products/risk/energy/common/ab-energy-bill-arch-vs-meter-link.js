var energyBillVsMeterLinkController = View.createController('energyBillVsMeterLink',{
	...
	energyBillVsMeterLink_grid_onShowGrid: function() {
		if (View.parameters.vn_id) {
			...
			try { 
				var jobId = Workflow.startJob('AbRiskEnergyManagement-BillMeterReconciliationService-getLinkedMetersForGrid', View.parameters.vn_id, View.parameters.bill_id, "bill_archive");					
	...
	energyBillVsMeterLink_grid_onAdd: function() {
		...
		restriction.addClause('bill_archive.vn_id', View.parameters.vn_id);
		restriction.addClause('bill_archive.bill_id', View.parameters.bill_id);
		var record = this.energyBillVsMeterCommon_dsBillBaseArch.getRecord(restriction);
		var vn_ac_id = record.getValue('bill_archive.vn_ac_id');
