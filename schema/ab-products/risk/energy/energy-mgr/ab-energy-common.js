...
function getConsoleConversionFactor(consoleId) {
	var console = View.panels.get(consoleId);
	var billTypeId = console.getFieldValue('bill_archive.bill_type_id');
	var billUnitId = View.controllers.get('abEnergyCostConsole').consoleBillUnits;
	var convFactor = View.controllers.get('abEnergyBillCommonController').getConversionFactor(billTypeId,billUnitId);
	return convFactor;
}
...
/*
 * Drill down function for charts that have year to year compare.
 * 
 * Used for:
 * ab-energy-consumption-compare-billed.axvw 
 * ab-energy-cost-by-year.axvw * 
 * 
 */
function onDrillDownToBillArchiveYearCompare(item){  
	...
	var unitsConversionFactor = this.getConsoleConversionFactor('energyConsole');
	
	//apply also the restriction to the grid panel to be showed with the details if clicking on the chart	 
	// don't include the time period restriction
	var finalRestriction = controller.getFinalRestriction(false);	 
	...
	var selectedTimePeriod = item.selectedChartData['bill_archive.time_period']; 
	if (selectedTimePeriod != undefined && selectedTimePeriod != '') {
		// if the first column is selected, is will be the previous year
		if (item.graph.columnIndex == 0) {
			selectedTimePeriod = controller.getPreviousYear(selectedTimePeriod);
		}		
		finalRestriction += " AND bill_archive.time_period = '" + selectedTimePeriod + "' ";
	}        
	var drillDownView = View.panels.get("energyDrillDownBillArchive"); 	
	drillDownView.addParameter('finalRestriction', finalRestriction); 
	if(valueExistsNotEmpty(unitsConversionFactor)){
		drillDownView.addParameter('unitsConversionFactor', unitsConversionFactor); 
	}
	drillDownView.refresh();
	drillDownView.showInWindow( {width: 1000, height: 600, closeButton: true}); 
	View.controllers.get('abEnergyBillCommonController').setQtyEnergyTitle(drillDownView);
	// set the drill down title 
	controller.energyDrillDownBillArchive.setTitle(View.title + " - " + getMessage("billDetails"));
}
 

function onDrillDownToBillArchive(item){  
	// use the first controller	
	var controller = View.controllers.get(0); 
	var selectedLocationValue = item.selectedChartData['bill_archive.locDtl'];
	var selectedLocationOption = controller.detailOptionSelected;
	...
	View.controllers.get('abEnergyBillCommonController').setQtyEnergyTitle(View.panels.get('energyDrillDownBillArchive'));
...
//	
	// set the drill down title
//	controller.energyDrillDownBillLines.setTitle(View.title + " - " + getMessage("lineDetails"));
}
