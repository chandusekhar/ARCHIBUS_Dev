var ctrlAbEnergyDefBillsCommon = View.createController('ctrlAbEnergyDefBillsCommon',{
	...
	bill_AbEnergyDefBills_afterRefresh: function(){
		if(!this.bill_AbEnergyDefBills.newRecord){
			var billId = $('bill_AbEnergyDefBills_bill.bill_id').value;
			var vnId = $('bill_AbEnergyDefBills_bill.vn_id').value;	
			var billCount,childBillCount;
			var controller = this;
			var result = Workflow.callMethod('AbRiskEnergyManagement-ProcessBills-getBillLineCount', billId, vnId);
			if(result.code == 'executed') {
				billCount = result.value;
			}		
			result = Workflow.callMethod('AbRiskEnergyManagement-ProcessBills-getChildrenBillCountForProratedByLocationBills', billId);
			if(result.code == 'executed') {
				childBillCount = result.value;
			}		
			if((billCount < 1) || (childBillCount < 1 )){
				this.bill_AbEnergyDefBills.enableButton("sendForApproval", false);	
			}
			if((billCount < 1) || (childBillCount > 0 )){
				this.bill_AbEnergyDefBills.enableButton("generateBills",false);
			}
			if(childBillCount < 1){
				this.bill_AbEnergyDefBills.enableButton("viewProratedBills",false);
			}
		} else {
		...
		if(billCount < 1){
			...
			View.confirm(msg, function(button) {
				if (button == 'yes') {
					...
					var records = controller.bill_ds.getRecords(res);
					for(var i=0; i<records.length; i++){
						var record = records[i];
						record.setValue('bill.status','Pending Approval');
						record.isNew = false;
						controller.bill_ds.saveRecord(record);
					}
	...
	bill_grid_onSendForApproval: function(billId, vnId, billLineId){
			this.bill_AbEnergyDefBills_onSendForApproval(billId, vnId, billLineId);
	...
	bill_AbEnergyDefBills_grid_onSendForApprovalMulti: function(){
		...
		for ( var i = 0; i < selectedRows.length; i++) {
			...
			var result = Workflow.callMethod('AbRiskEnergyManagement-ProcessBills-getChildrenBillCountForProratedByLocationBills', billId);
			if(result.code == 'executed') {
				childBillCount = result.value;
			}
			if(childBillCount < 1){
				View.showMessage(getMessage('msg_error_no_children'));
				return false;
			}
		}
