...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">	
	...
	<xsl:template match="/">
		...
		<head>	
			...
			<script language="JavaScript" src="#Attribute%//@relativeFileDirectory%/ab-em-setup-or-move-edit.js"><xsl:value-of select="$whiteSpace"/></script>
					...
					<td colspan="4">
						<table align="left" valign="top"><tr>
							<td  class="inputFieldLabel">
								<input type="checkbox" name="bIssued"/>
								<span  translatable="true">Issue</span>
							</td>
							<td>
								<!-- new -->
								<input class="AbActionButtonFormStdWidth" type="button" value="{$mo_tgrp/message[@name='create']}" onclick='onCreateMoveRequest("{$afmInputsForm}", "{$SQLAction/@serialized}","{$renderAction/@serialized}","{$warning_message}")'/>
							</td>
							<td>
								<!-- reset -->
				...
				<tr>
					...
					<td colspan="4">
						<table align="left" valign="top"><tr>
							...
							<td>
								<!-- update -->
								<input class="AbActionButtonFormStdWidth" type="button" value="{$mo_tgrp/message[@name='modify']}" onclick='onUpdateMoveRequest("{$afmInputsForm}", "{$SQLAction/@serialized}","{$renderAction/@serialized}","{$warning_message}")'/>
							</td>
							
							<td>
								<!-- cancel action -->
								<input class="AbActionButtonFormStdWidth" type="button" value="{$mo_tgrp/message[@name='cancel']}" onclick='onCancelMoveRequest("{$afmInputsForm}", "{$SQLAction/@serialized}","{$renderAction/@serialized}","{$warning_message}")'/>
							</td>
							<td>
								<!-- reset -->
								<input  class="AbActionButtonFormStdWidth" type="RESET" value="{/*/message[@name='reset']}"/>
					...
					<td COLSPAN="4">
						<table align="center">
							<tr>
								<td>
									<!-- close action -->
									<input class="AbActionButtonFormStdWidth" type="button" value="{$mo_tgrp/message[@name='close']}" onclick='onCloseMoveRequest("{$afmInputsForm}", "{$SQLAction/@serialized}","{$renderAction/@serialized}","{$warning_message}")'/>
								</td>
								<td>
									<!-- cancel action -->
									<input class="AbActionButtonFormStdWidth" type="button" value="{$mo_tgrp/message[@name='cancel']}" onclick='onCancelMoveRequest("{$afmInputsForm}", "{$SQLAction/@serialized}","{$renderAction/@serialized}","{$warning_message}")'/>
	...
	<xsl:template name="common_create_update_mo">
		...
		<tr>
			<td class="inputFieldLabel_validated" nowrap="1">
				<span  translatable="true">Contact:</span>
			</td>
			<td  nowrap="1">
				<input type="hidden" name="mo.mo_id" value="{$mo_tgrp/dataSource/data/records/record/@mo.mo_id}"/>
				<input type="hidden" name="mo.em_id" value="{$em_name}"/>
				<input class="inputField" maxlength="{$mo_tgrp/dataSource/data/fields/field[@name='requestor']/@size}" type="text" name="mo.requestor" value="{$mo_tgrp/dataSource/data/records/record/@mo.requestor}" onchange='validationInputs("{$afmInputsForm}", "mo.requestor")' onblur='validationInputs("{$afmInputsForm}", "mo.requestor")'/>
			...
			<td class="inputFieldLabel_validated"  nowrap="1">
				<span  translatable="true">Move Project:</span>
			</td>
			<td   nowrap="1">
				<input class="inputField" type="text"  maxlength="{$mo_tgrp/dataSource/data/fields/field[@name='mp_id']/@size}" name="mo.mp_id" value="{$mo_tgrp/dataSource/data/records/record/@mo.mp_id}"  onchange='validationInputs("{$afmInputsForm}", "mo.mp_id")' onblur='validationInputs("{$afmInputsForm}", "mo.mp_id")'/>
				<xsl:call-template name="SelectValue">
					<xsl:with-param name="afmTableGroup" select="$mo_tgrp"/>
					<xsl:with-param name="formName" select="$afmInputsForm"/>
		...
		<tr>
			...
			<td  class="inputFieldLabel" nowrap="1">
				<span  translatable="true">Time to Move:</span>
			</td>
			<td >
				...
				<input class="inputField" type="text" name="mo.time_to_perform" SIZE="20" MAXLENGTH="10" VALUE="{$trucated_time_value}" onchange='validationAndConvertionTimeInput(this, "mo.time_to_perform",null,"false",true, false)' onblur='validationAndConvertionTimeInput(this, "mo.time_to_perform", null,"false",true,true)'/>
		...
		<tr>
			...
			<td  class="{$inputFieldLabelCSS_to_bl_id}" nowrap="1">
				<span  translatable="true">To Building:</span><font face="Arial" size="-1" color="red">*</font>
			</td>
			<td   nowrap="1">
				<input class="inputField"  maxlength="{$mo_tgrp/dataSource/data/fields/field[@name='to_bl_id']/@size}"  type="text" name="mo.to_bl_id" value="{$mo_tgrp/dataSource/data/records/record/@mo.to_bl_id}" onkeyup='checkVacanyRoomsButton("{$afmInputsForm}","mo.to_bl_id","mo.to_fl_id","vacanyRoomsButton");' onfocus='checkVacanyRoomsButton("{$afmInputsForm}","mo.to_bl_id","mo.to_fl_id","vacanyRoomsButton");' onchange='checkVacanyRoomsButton("{$afmInputsForm}","mo.to_bl_id","mo.to_fl_id","vacanyRoomsButton"); validationInputs("{$afmInputsForm}", "mo.to_bl_id")' onblur='checkVacanyRoomsButton("{$afmInputsForm}","mo.to_bl_id","mo.to_fl_id","vacanyRoomsButton"); validationInputs("{$afmInputsForm}", "mo.to_bl_id")'/>
				<xsl:call-template name="SelectValue">
					<xsl:with-param name="afmTableGroup" select="$mo_tgrp"/>
					<xsl:with-param name="formName" select="$afmInputsForm"/>
		...
		<tr>
			...
			<td  class="{$inputFieldLabelCSS_to_fl_id}"  nowrap="1">
				<span  translatable="true">To Floor:</span><font face="Arial" size="-1" color="red">*</font>
			</td>
			<td  nowrap="1">
				<input class="inputField"  maxlength="{$mo_tgrp/dataSource/data/fields/field[@name='to_fl_id']/@size}" type="text" name="mo.to_fl_id" value="{$mo_tgrp/dataSource/data/records/record/@mo.to_fl_id}" onkeyup='checkVacanyRoomsButton("{$afmInputsForm}","mo.to_bl_id","mo.to_fl_id","vacanyRoomsButton");' onfocus='checkVacanyRoomsButton("{$afmInputsForm}","mo.to_bl_id","mo.to_fl_id","vacanyRoomsButton");' onchange='checkVacanyRoomsButton("{$afmInputsForm}","mo.to_bl_id","mo.to_fl_id","vacanyRoomsButton"); validationInputs("{$afmInputsForm}", "mo.to_fl_id")' onblur='checkVacanyRoomsButton("{$afmInputsForm}","mo.to_bl_id","mo.to_fl_id","vacanyRoomsButton"); validationInputs("{$afmInputsForm}", "mo.to_fl_id")'/>
				<xsl:call-template name="SelectValue">
					<xsl:with-param name="afmTableGroup" select="$mo_tgrp"/>
					<xsl:with-param name="formName" select="$afmInputsForm"/>
		...
		<tr>
			...
			<td   class="{$inputFieldLabelCSS_to_rm_id}" nowrap="1">
				<span  translatable="true">To Room:</span><font face="Arial" size="-1" color="red">*</font>
			</td>
			<td   nowrap="1">
				<input class="inputField"   maxlength="{$mo_tgrp/dataSource/data/fields/field[@name='to_rm_id']/@size}" type="text" name="mo.to_rm_id" value="{$mo_tgrp/dataSource/data/records/record/@mo.to_rm_id}" onkeyup='checkVacanyRoomsButton("{$afmInputsForm}","mo.to_bl_id","mo.to_fl_id","vacanyRoomsButton");' onfocus='checkVacanyRoomsButton("{$afmInputsForm}","mo.to_bl_id","mo.to_fl_id","vacanyRoomsButton");' onchange='validationInputs("{$afmInputsForm}", "mo.to_rm_id")' onblur='validationInputs("{$afmInputsForm}", "mo.to_rm_id")'/>
				<xsl:call-template name="SelectValue">
					<xsl:with-param name="afmTableGroup" select="$mo_tgrp"/>
					<xsl:with-param name="formName" select="$afmInputsForm"/>
		...
		<tr>
			<td valign="top"  class="inputFieldLabel"  >
				<span  translatable="true">Move Description:</span>
			</td>
			<td colspan="3" rowspan="2" >
				<textarea class="textareaABData" name="mo.description" cols="60" rows="4" wrap="PHYSICAL">
		...
		<tr>
			<td  class="inputFieldLabel"  nowrap="1">
				<input type="checkbox" name="bSendEmail" onclick='EnableCopyTo("{$afmInputsForm}")'/>
		...
		<tr>
			...
			<td colspan="3">
				...
				<input class="inputField_box" type="text" name="copyEmailAddress" value="{$ccEmailAddress}" size="60" disabled="disabled"/>
