...
public class ReservationMessagesService {
    ...
    public ReservationMessage createCancelledConfirmation(final RoomReservation reservation) {
        ...
        final NotificationDataModel dataModel = createDataModelWithRoom(reservation, getRoomForMessage(reservation));
    ...
    private RoomArrangement getRoomForMessage(final RoomReservation reservation) {
        ...
        if (reservation.getRoomAllocations().isEmpty()) {
            final String notApplicable = EventHandlerBase.localizeString(ContextStore.get()
                .getEventHandlerContext(), NOT_APPLICABLE, this.getClass().getName());
            room =
                    new RoomArrangement(reservation.determineBuildingId(), notApplicable,
