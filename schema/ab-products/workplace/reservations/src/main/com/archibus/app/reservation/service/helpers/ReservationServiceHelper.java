        ...
        for (final RoomReservation existingReservation : existingReservations) {
            ...
            if (roomArrangements.isEmpty()) {
                throw new ReservableNotAvailableException(roomReservation.getRoomAllocations()
                    .get(0).getRoomArrangement(), roomReservation.getReserveId(),
                    ROOM_NOT_AVAILABLE, ReservationServiceHelper.class);
            }
    ...
    private static void saveFirstNewOccurrence(final RoomReservation reservation,
        ...
        final List<RoomArrangement> roomArrangements =
                roomArrangementDataSource.findAvailableRooms(reservation, null, false, null,
                    false, false);
        if (roomArrangements.isEmpty()) {
            Logger.getLogger(ReservationServiceHelper.class).debug(
                "The room is not available on the first occurrence date "
                        + reservation.getStartDate());

            // Don't allow a conflict if the reservation already existed without a conflict.
            ReservationConflictsHelper.checkAlreadyConflicted(reservation);
            ReservationConflictsHelper.createConflictedFirstOccurrence(reservation,
