 ...
 * @since 21.3
 */
public class ConferenceCallReservationDataSource extends RoomReservationDataSource implements
        IConferenceCallReservationDataSource {
     ...
     */
    public List<RoomReservation> getConferenceCallOccurrences(final RoomReservation roomReservation) {
        final Integer conferenceParentId = this.getParentId(roomReservation.getConferenceId());
        final Integer[] conferenceIds =
                this.getRecurringConferenceIds(conferenceParentId,
    ...
    public Integer[] getRecurringConferenceIds(final Integer conferenceParentId,
        ...
        final DataSource dataSource =
                DataSourceFactory.createDataSourceForFields(Constants.RESERVE_TABLE_NAME,
                    new String[] { Constants.RES_ID, Constants.RES_PARENT,
    ...
    public Integer getParentId(final Integer reservationId) {
        final DataSource dataSource =
                DataSourceFactory.createDataSourceForFields(Constants.RESERVE_TABLE_NAME,
                    new String[] { Constants.RES_ID, Constants.RES_PARENT,
                            Constants.DATE_START_FIELD_NAME, Constants.RES_CONFERENCE });
        dataSource.addRestriction(Restrictions.eq(Constants.RESERVE_TABLE_NAME, Constants.RES_ID,
            reservationId));
        final DataRecord primaryRecord = dataSource.getRecord();
        return primaryRecord.getInt(Constants.RESERVE_TABLE_NAME + Constants.DOT
                + Constants.RES_PARENT);
    }
    ...
    public List<RoomReservation> getConferenceSeries(final Integer conferenceId,
        ...
        dataSource.addRestriction(Restrictions.in(Constants.RESERVE_TABLE_NAME, Constants.STATUS,
            allowedStatuses));
        if (startDate != null) {
            dataSource.addRestriction(Restrictions.gte(Constants.RESERVE_TABLE_NAME,
        ...
        dataSource
            .addSort(Constants.RESERVE_TABLE_NAME, Constants.DATE_START_FIELD_NAME, sortOrder);
    ...
    public void persistCommonIdentifiers(final RoomReservation primaryReservation,
         ...
         * occurrence index, occurrence date modified. Occurrence index and occurrence date modified
         * should be OK; conference id only needs setting when creating a new conference call;
         * unique id can change if the meeting is not found on the connected Exchange.
         */
        /*
        ...
        for (int i = 0; i < primaryReservations.size(); ++i) {
            ...
            final Integer conferenceId =
                    primaryOccurrence.getConferenceId() == null ? primaryOccurrence.getReserveId()
                            : primaryOccurrence.getConferenceId();
            ...
            for (final RoomReservation confCallReservation : confCallReservations) {
                ...
                if ((primaryOccurrence.getParentId() == null || primaryOccurrence.getParentId() == 0)
                    ...
                    storedReservation.setRecurringDateModified(primaryOccurrence
    ...
    public void updateCommonIdentifiers(final List<RoomReservation> primaryReservations,
            final List<List<RoomReservation>> allModifiedReservations, final boolean updateParentIds) {
     ...
     */
    public void canCancelConferenceCall(final Integer conferenceId) throws ReservationException {
        ...
        for (final RoomReservation reservation : reservations) {
            ...
            if (this.roomAllocationDataSource.get(allocation.getId()) == null) {
                ...
                throw new ReservationException(
                    ...
                    ConferenceCallReservationDataSource.class, reservation.getReserveId());
            }
    ...
    public final Integer[] getActiveReservationIdsInConference(final Integer conferenceId) {
        ...
        dataSource.addRestriction(Restrictions.eq(this.mainTableName, Constants.RES_CONFERENCE,
            conferenceId));
