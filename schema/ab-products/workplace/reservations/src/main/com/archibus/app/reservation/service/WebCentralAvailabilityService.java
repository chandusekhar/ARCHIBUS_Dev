 ...
 * @since 21.2
 */
public class WebCentralAvailabilityService extends RoomReservationServiceBase implements
    ...
    public Map<String, AttendeeAvailability> findAttendeeAvailability(
            final IReservation reservation, final Date date, final TimeZone requestedTimeZone,
            final String userEmail, final List<String> attendeeEmails) throws ExceptionBase {
        ...
        for (final String email : attendeeEmails) {
            final List<ICalendarEvent> events =
                    this.findAttendeeAvailability(date, requestedTimeZone, email);
     ...
     * attendee.
     *
     * @param date date in the specified requestedTimeZone
     * @param timeZone requested time zone
     * @param email email of the attendee
     * @return List of Calendar Events
     * @throws ExceptionBase translated exception
     */
    private List<ICalendarEvent> findAttendeeAvailability(final Date date, final TimeZone timeZone,
            final String email) throws ExceptionBase {
        ...
        if (!StringUtil.isNullOrEmpty(email)) {
            ...
            for (final RoomReservation foundReservation : foundReservations) {
                ...
                final RoomReservation reservation =
                        this.reservationDataSource.getActiveReservation(foundReservation
                            .getReserveId());
                reservation.setTimeZone(this.timeZoneCache.getBuildingTimeZone(reservation
                    .determineBuildingId()));
                ReservationUtils.convertToTimeZone(reservation, timeZone.getID());

                if (endDate.before(reservation.getStartDateTime())
                        || date.after(reservation.getEndDateTime())) {
