...
public class RoomReservationWfrBase extends RoomReservationServiceBase {
    ...
    protected List<RoomReservation> updateOtherReservationsInConferenceCall(
        ...
        if (confCallReservations != null && !confCallReservations.isEmpty()) {
            ...
            for (final RoomReservation confCallReservation : confCallReservations) {
                hasRoomConflictInConferenceCall =
                        hasRoomConflictInConferenceCall
                ...
                if (!checkForChangedTimes
                        || confCallReservation.getReserveId()
                            .equals(roomReservation.getReserveId())
    ...
    private boolean isTimePeriodEqual(final IReservation primaryReservation,
        ...
        linkedReservation.setTimeZone(this.timeZoneCache.getBuildingTimeZone(linkedReservation
            .determineBuildingId()));

        return primaryReservation.getTimePeriod().equals(
            ReservationUtils.getTimePeriodInTimeZone(linkedReservation,
                primaryReservation.getTimeZone()));
    }
    ...
    protected void cancelRecurringSingleRoomCalendarEvent(final RoomReservation reservation,
            final String comments, final List<IReservation> cancelledReservations,
        ...
        if (reservation.getReserveId().equals(reservation.getParentId())
            ...
            // Remove the room from each occurrence. This will change the occurrences to exceptions.
            for (final IReservation cancelledReservation : cancelledReservations) {
                final List<RoomReservation> confCallReservations =
                        this.updateOtherReservationsInConferenceCall(cancelledReservation,
                            comments, false);
                try {
                    this.calendarServiceWrapper.cancelSingleRoomCalendarEvent(cancelledReservation,
                        comments, confCallReservations);
    ...
    private boolean conferenceSeriesIsEmpty(final RoomReservation reservation) {
        return reservation.getConferenceId() == null
    ...
    protected void cancelRecurringConferenceCalendarEvent(final Integer firstConferenceId,
        ...
        if (firstConferenceId.equals(conferenceParentId)) {
            this.calendarServiceWrapper.cancelRecurringCalendarEvent(cancelledOccurrences.get(0),
                comments);
        } else {
    ...
    protected SortedSet<Date> getConflictDates(final List<RoomReservation> createdReservations) {
        ...
        for (final RoomReservation reservation : createdReservations) {
            if (reservation.getRoomAllocations().isEmpty() || reservation.hasRoomConflictInConferenceCall()) {
    ...
    protected void updateReserveRecordAfterSave(final DataRecord reservation,
        ...
        reservation.setValue(this.reservationDataSource.getMainTableName() + Constants.DOT
