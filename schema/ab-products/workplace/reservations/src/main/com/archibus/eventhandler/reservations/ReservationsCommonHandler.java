...
public class ReservationsCommonHandler extends ReservationsEventHandlerBase {
    ...
    public void notifyRequestedStd(final EventHandlerContext context, final String Std) {
        ...
        final TreeMap<String, String> valuesToMail = new TreeMap<String, String>();
        final ArrayList<TreeMap<String, String>> listResources =
                new ArrayList<TreeMap<String, String>>();
        final ArrayList<TreeMap<String, String>> listRoom =
                new ArrayList<TreeMap<String, String>>();
        ...
        final String errMessage = localizeMessage(context, ACTIVITY_ID,
            "NOTIFYREQUESTED" + Std.toUpperCase() + "_WFR",
            "NOTIFYREQUESTED" + Std.toUpperCase() + "ERROR", null);
        ...
        try {
            ...
            if (!resId.equals("")) {
                ...
                if ("for".equals(Std)) {
                    ...
                    final List<Map<?, ?>> records = retrieveDbRecords(context, sql);
                ...
                sql = "SELECT em.email as requested" + Std
                ...
                final List<Map<?, ?>> recordsSql1 = retrieveDbRecords(context, sql);

                if (!recordsSql1.isEmpty()) {
                    final Map<?, ?> recordOfSql1 = recordsSql1.get(0);
                    ...
                    sql = " SELECT locale " + " FROM afm_users " + " WHERE email=" + literal(
                        context, valuesToMail.get("reserve.requested" + Std + "mail"));
                    // this.log.info(ACTIVITY_ID+"-"+RULE_ID+" [sql]: "+sql);

                    final List<Map<?, ?>> recordsSql2 = retrieveDbRecords(context, sql);

                    // If the locale is found
                    if (!recordsSql2.isEmpty()) {
                        final Map<?, ?> recordOfSql2 = recordsSql2.get(0);
                ...
                if (!allQueriesOk) {
                    ...
                    sql = " SELECT em.email as createdbymail, " + " reserve.user_requested_" + Std
                            + ", " + " reserve.comments " + " FROM reserve, em ";
                    // if resId is not zero makes the query for a single a reserve,
                    // but in another case makes it for a group of reserves with a common father.
                    if (!resId.equals("0")) {
                    ...
                    final List<Map<?, ?>> recordsSql3 = retrieveDbRecords(context, sql);

                    if (!recordsSql3.isEmpty()) {
                        final Map<?, ?> recordOfSql3 = recordsSql3.get(0);
                        ...
                        sql = " SELECT locale " + " FROM afm_users " + " WHERE email="
                                + literal(context, getString(recordOfSql3, "createdbymail"));
                        // this.log.info(ACTIVITY_ID+"-"+RULE_ID+" [sql]: "+sql);

                        final List<Map<?, ?>> recordsSql4 = retrieveDbRecords(context, sql);

                        // If the locale is found
                        if (!recordsSql4.isEmpty()) {
                            final Map<?, ?> recordOfSql4 = recordsSql4.get(0);
                ...
                sql = " SELECT date_start, time_start, time_end, status, bl_id, fl_id, rm_id, config_id, "
                        + " rm_arrange_type_id, attendees_in_room, comments " + " FROM reserve_rm ";
                // if resId is not zero makes the query for a single a reserve,
                // but in another case makes it for a group of reserves with a common father.
                if (!resId.equals("0")) {
                    sql += " WHERE res_id= " + resId;
                } else {
                    sql += " WHERE EXISTS (SELECT 1 FROM reserve WHERE reserve.res_id=reserve_rm.res_id AND res_parent = "
                            + parentId + " ) ";
                }
                ...
                final List<Map<?, ?>> listRoomReserve = retrieveDbRecords(context, sql);
                ...
                if (!listRoomReserve.isEmpty()) {
                    ...
                    for (final Map<?, ?> recordOfSql5 : listRoomReserve) {
                        final TreeMap<String, String> roomToMail = new TreeMap<String, String>();
                        roomToMail.put("reserve_rm.attendees_in_room",
                            getString(recordOfSql5, "attendees_in_room"));
                        ...
                        roomToMail.put("reserve_rm.status", getEnumFieldDisplayedValue(context,
                            "reserve_rm", "status", getString(recordOfSql5, "status")));
                        ...
                        roomToMail.put("reserve_rm.config_id",
                            getString(recordOfSql5, "config_id"));
                        roomToMail.put("reserve_rm.rm_arrange_type_id",
                            getString(recordOfSql5, "rm_arrange_type_id"));
                        listRoom.add(roomToMail);
                        if (!valuesToMail.containsKey("reserve_rm.attendees_in_room")) {
                ...
                sql = " SELECT date_start,time_start,time_end,status,resource_id,quantity,bl_id,fl_id,rm_id,comments "
                        + " FROM reserve_rs ";
                // if resId is not zero makes the query for a single a reserve,
                // but in another case makes it for a group of reserves with a common father.
                if (!resId.equals("0")) {
                	...
                	// Since we don't show the dates, only show the resources for one date.
                    sql += " WHERE res_id= " + parentId;
                }
                sql += " ORDER BY date_start ";
                // this.log.info(ACTIVITY_ID+"-"+RULE_ID+" [sql]: "+sql);

                final List<Map<?, ?>> listResource = retrieveDbRecords(context, sql);
                if (!listResource.isEmpty()) {
                    ...
                    for (final Map<?, ?> recordOfSql6 : listResource) {
                        final TreeMap<String, String> resourcesToMail =
                        ...
                        resourcesToMail.put("reserve_rs.status", getEnumFieldDisplayedValue(context,
                            "reserve_rs", "status", getString(recordOfSql6, "status")));
                ...
                if (allQueriesOk) {
                    ...
                    final TreeMap<String, String> messages =
                    ...
                    if (isRecurring) {
                        ...
                        if ((!existsRoom) && (existsResource)) {
                            final Iterator<TreeMap<String, String>> res_it =
                                    listResources.iterator();
                            final TreeMap<String, String> resources_map = res_it.next();
                            final String res_date_start =
                                    resources_map.get("reserve_rs.date_start");
                            subject = messages.get("SUBJECT1") + " "
                                    + (parentId.equals("0") ? resId : parentId) + ", "
                    ...
                    if (isRegular) {
                        if ((existsRoom) || (existsRoom && existsResource)) {
                            ...
                            message += messages.get("BODY4") + " "
                                    + valuesToMail.get("reserve_rm.time_start") + "\n\n";
                            message += messages.get("BODY5") + " "
                                    + valuesToMail.get("reserve_rm.time_end") + "\n\n";
                            message += messages.get("BODY6") + " "
                                    + valuesToMail.get("reserve_rm.attendees_in_room") + "\n\n";
                            message += messages.get("BODY7") + " "
                                    + valuesToMail.get("reserve_rm.status") + "\n\n";
                            message += messages.get("BODY11") + " "
                    ...
                    if (isRecurring) {
                        if ((existsRoom) || (existsRoom && existsResource)) {
                            ...
                            String rm_comments = "";
                            for (final TreeMap<String, String> rooms : listRoom) {
                                message += rooms.get("reserve_rm.date_start") + " - "
                                        ...
                                        + rooms.get("reserve_rm.attendees_in_room") + " - "
                                        + rooms.get("reserve_rm.status") + "\n";
                                rm_comments = valuesToMail.get("reserve_rm.comments");
                            }
                    ...
                    if (isRegular) {
                        if (existsRoom && existsResource) {
                            ...
                            String rs_comments = "";
                            for (final TreeMap<String, String> resources : listResources) {
                                message += resources.get("reserve_rs.resource_id") + " - "
                                ...
                                rs_comments = resources.get("reserve_rs.comments");
                    ...
                    message += getActivityParameterString(context, ACTIVITY_ID,
                    ...
                    final String from = getActivityParameterString(context, ACTIVITY_ID,
                    ...
                    try {
                        // PC changed to solve KB 3016618
                        sendEmail(message, from, host, port, subject,
                            valuesToMail.get("reserve.requested" + Std + "mail"), null, null,
                            userId, password, null, CONTENT_TYPE_TEXT_UFT8, ACTIVITY_ID);
                    } catch (final Throwable e) {
                        handleNotificationError(context,
                            ACTIVITY_ID + "-" + RULE_ID + ": SENDEMAIL ERROR", errMessage, e,
    ...
    /**
     * Modify the given rooms' reservable property and create a configuration record if necessary.
     *
    ...
    /**
     * Modify the given room's reservable property and create a configuration record if necessary.
     *
                ...
                final DataSource configDs =
                        DataSourceFactory.createDataSourceForFields(Constants.ROOM_CONFIG_TABLE,
                            new String[] { Constants.BL_ID_FIELD_NAME, Constants.FL_ID_FIELD_NAME,
                                    ...
                                    "config_name" });
                configDs.addRestriction(Restrictions.eq(Constants.ROOM_CONFIG_TABLE,
                    Constants.BL_ID_FIELD_NAME, buildingId));
                configDs.addRestriction(Restrictions.eq(Constants.ROOM_CONFIG_TABLE,
                    Constants.FL_ID_FIELD_NAME, floorId));
                configDs.addRestriction(Restrictions.eq(Constants.ROOM_CONFIG_TABLE,
                    Constants.RM_ID_FIELD_NAME, roomId));

                // If not exists, create the configuration
                if (configDs.getRecord() == null) {
                    ...
                    configDs.saveRecord(configRecord);
                }
            }
        }
    }
    ...
    public void saveArrangementFixedResource(final String rmResourceStd) {
        ...
        final String errMessage = localizeMessage(context, ACTIVITY_ID,
            "SAVEARRANGEMENTFIXEDRESOURCE_WFR", "SAVEARRANGEMENTFIXEDRESOURCEERROR", null);

        try {
            final JSONObject roomResourceStandard = new JSONObject("" + jsonExpression + ")");
            String sql = "";
            sql = " UPDATE rm_resource_std " + " SET resource_std="
                    + literal(context, roomResourceStandard.getString("resource_std")) + ", "
                    + "eq_id=" + literal(context, roomResourceStandard.getString("eq_id")) + ", "
                    + "description="
                    + literal(context, roomResourceStandard.getString("description"))
                    + " WHERE bl_id=" + literal(context, roomResourceStandard.getString("bl_id"))
                    + " AND fl_id=" + literal(context, roomResourceStandard.getString("fl_id"))
                    + " AND rm_id=" + literal(context, roomResourceStandard.getString("rm_id"))
                    + " AND config_id="
            ...
            try {
                ...
                handleError(context,
                    ACTIVITY_ID + "-" + RULE_ID + ": Failed rm_resource_std table: " + sql,
                    errMessage, e);
            }

            sql = " INSERT INTO rm_resource_std (rm_arrange_type_id, bl_id, fl_id, rm_id, config_id, fixed_resource_id, resource_std, eq_id, description) "
                    + " SELECT a.rm_arrange_type_id AS rm_arrange_type_id, "
                    + literal(context, roomResourceStandard.getString("bl_id")) + " AS bl_id, "
                    + literal(context, roomResourceStandard.getString("fl_id")) + " AS fl_id, "
                    + literal(context, roomResourceStandard.getString("rm_id")) + " AS rm_id, "
                    + literal(context, roomResourceStandard.getString("config_id"))
                    + " AS config_id, "
                    + literal(context, roomResourceStandard.getString("fixed_resource_id"))
                    ...
                    + literal(context, roomResourceStandard.getString("resource_std"))
                    + " AS resource_std,  "
                    + literal(context, roomResourceStandard.getString("eq_id")) + " AS eq_id, "
                    + literal(context, roomResourceStandard.getString("description"))
                    + " AS description " + " FROM rm_arrange a " + " WHERE a.bl_id="
                    + literal(context, roomResourceStandard.getString("bl_id")) + " AND a.fl_id="
                    + literal(context, roomResourceStandard.getString("fl_id")) + " AND a.rm_id="
                    + literal(context, roomResourceStandard.getString("rm_id"))
                    + " AND a.config_id="
                    + literal(context, roomResourceStandard.getString("config_id"))
                    + " AND NOT EXISTS" + " ( SELECT 1 FROM rm_resource_std b "
                    + "   WHERE b.bl_id="
                    + literal(context, roomResourceStandard.getString("bl_id")) + " AND b.fl_id="
                    + literal(context, roomResourceStandard.getString("fl_id")) + " AND b.rm_id="
                    + literal(context, roomResourceStandard.getString("rm_id"))
                    + " AND b.config_id="
                    + literal(context, roomResourceStandard.getString("config_id"))
    ...
    public void closeReservations(final EventHandlerContext context) {
        ...
        final String errMessage = localizeMessage(context, ACTIVITY_ID, "CLOSERESERVATIONS_WFR",
        ...
        try {
            ...
            sql = " SELECT param_value " + " FROM afm_activity_params " + " WHERE activity_id='"
                    + ACTIVITY_ID + "' " + " AND param_id='DaysBeforeArchiving'";

            // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Select-1]: " + sql);

            final List<Map<?, ?>> listParamValue = retrieveDbRecords(context, sql);

            if (!listParamValue.isEmpty()) {
                final Map<?, ?> recordOfSql1 = listParamValue.get(0);
                ...
                sql = buildSqlToArchiveReserveRm(context, currentDate, daysBeforeArchiving);
                ...
                try {
                    ...
                    // table
                    sql = " DELETE FROM reserve_rm " + " WHERE " + currentDate + " >= "
                ...
                sql = " INSERT INTO hreserve_rs (res_id, rsres_id, date_start, time_start, time_end,"
                        ...
                        + " recurring_order, comments, status " + " FROM reserve_rs " + " WHERE "
                        + currentDate + " >= " + formatSqlAddDaysToExpression(context, "date_start",
                            Integer.toString(daysBeforeArchiving));
                ...
                try {
                    ...
                    // table
                    sql = " DELETE FROM reserve_rs " + " WHERE " + currentDate + " >= "
                            + formatSqlAddDaysToExpression(context, "date_start",
                                Integer.toString(daysBeforeArchiving));
                ...
                sql = buildSqlToArchiveReserve(context, currentDate, daysBeforeArchiving);
                ...
                try {
                    ...
                    // Remove the inserted reservations into the historical table from the original
                    // table
                    sql = " DELETE FROM reserve " + " WHERE " + currentDate + " >= "
                            + formatSqlAddDaysToExpression(context, "date_start",
                                Integer.toString(daysBeforeArchiving));
            ...
            handleError(context, ACTIVITY_ID + "-" + RULE_ID + ": Global Exception ", errMessage,
                e);
        }
     ...
     */
    static String buildSqlToArchiveReserve(final EventHandlerContext context,
            final String currentDate, final int daysBeforeArchiving) {
        String sql;
        final TableDef.ThreadSafe reserveTable =
                ContextStore.get().getProject().loadTableDef("reserve");
        final boolean hasUniqueId = reserveTable.findFieldDef("outlook_unique_id") != null;
        final boolean hasOccurrenceIndex = reserveTable.findFieldDef("occurrence_index") != null;
        final boolean hasConferenceId = reserveTable.findFieldDef("res_conference") != null;

        // Insert the reservations that meet the criteria into the historical table
        sql = " INSERT INTO hreserve (res_id, user_created_by, user_requested_by, user_requested_for,"
        ...
        if (hasUniqueId) {
            sql += ", outlook_unique_id ";
        }
        if (hasOccurrenceIndex) {
            sql += ", occurrence_index ";
        }
        if (hasConferenceId) {
            sql += ", res_conference ";
        }
        ...
        sql += " FROM reserve " + " WHERE " + currentDate + " >= " + formatSqlAddDaysToExpression(
            context, "date_start", Integer.toString(daysBeforeArchiving));
        return sql;
    }
     ...
     * @return SQL statement
     */
    static String buildSqlToArchiveReserveRm(final EventHandlerContext context,
            final String currentDate, final int daysBeforeArchiving) {
        String sql;
        final TableDef.ThreadSafe reserveRmTable =
                ContextStore.get().getProject().loadTableDef("reserve_rm");
        final boolean hasVerified = reserveRmTable.findFieldDef("verified") != null;
        final boolean hasAttendeesInRoom = reserveRmTable.findFieldDef("attendees_in_room") != null;
        // Insert the room reservations that meet the criteria into the historical table
        sql = "INSERT INTO hreserve_rm (res_id, rmres_id, date_start, time_start, time_end, "
        ...
        if (hasAttendeesInRoom) {
            sql += ", attendees_in_room ";
        }
        ...
        sql += " FROM reserve_rm " + " WHERE " + currentDate + " >= "
                + formatSqlAddDaysToExpression(context, "date_start",
                    Integer.toString(daysBeforeArchiving));
        return sql;
    }
     ...
     */
    private void setArchiveStatus(final EventHandlerContext context, final String errMessage,
            final String RULE_ID, final String tableName) {
        final String sql = " UPDATE " + tableName + " SET status = 'Closed' "
    ...
    public void sendEmailInvitations(final EventHandlerContext context) {
        ...
        if (context.parameterExists("Reservation")
            ...
            try {
                ...
                handleError(context,
                    ACTIVITY_ID + "-" + RULE_ID + ": Failed parsing Reservation: " + e.getMessage(),
                    errMessage, e);
            }
        }
        ...
        if (context.parameterExists("RoomConflicts")
            ...
            try {
                ...
                handleError(context, ACTIVITY_ID + "-" + RULE_ID
                        + ": Failed parsing RoomConflicts: " + e.getMessage(),
                    errMessage, e);
            }
        }
        ...
        // this.log.info("'"+ACTIVITY_ID+"-"+RULE_ID+"' [res_id]: "+resId+" [res_parent]:
        // "+parentId+" ");
        if (context.parameterExists("res_conference")) {
            conferenceId = (String) context.getParameter("res_conference");
        }
        ...
        try {
            ...
            if (!resId.equals("")) {
                ...
                // BEGIN: Get reservation info and the user to notify
                sql = " SELECT em.email as requestedbymail,reserve.user_requested_by,"
                        + " reserve.comments,reserve.reservation_name " + " FROM reserve, em ";
                if (!resId.equals("0")) {
                    sql += " WHERE reserve.res_id= " + resId;
                } else {
                    sql += " WHERE reserve.res_parent = " + parentId;
                ...
                if (!recordsSql1.isEmpty()) {
                    ...
                    // Search the locale of the user to notify
                    sql = " SELECT locale " + " FROM afm_users " + " WHERE email="
                            + literal(context, valuesToMail.get("reserve.requestedbymail"));
                    // this.log.info(ACTIVITY_ID+"-"+RULE_ID+" [sql]: "+sql);
                ...
                if (!allQueriesOk) {
                    ...
                    // BEGIN: Get reservation info and the creator to search for its locale
                    sql = " SELECT em.email as createdbymail, " + " reserve.user_requested_by, "
                            + " reserve.comments " + " FROM reserve, em ";
                    if (!resId.equals("0")) {
                        sql += " WHERE reserve.res_id= " + resId;
                    } else {
                ...
                sql = " SELECT em.email as requestedformail, reserve.user_requested_for"
                        + " FROM reserve, em ";
                if (!resId.equals("0")) {
                    sql += " WHERE reserve.res_id= " + resId;
                } else {
                ...
                final List<Map<?, ?>> recordsSql5 = retrieveDbRecords(context, sql);

                if (!recordsSql5.isEmpty()) {
                    final Map<?, ?> recordOfSql5 = recordsSql5.get(0);

                    // kb#3035551: add different ics file for requested by email
                    if (!getString(recordOfSql5, "requestedformail")
                        .equals(valuesToMail.get("reserve.requestedbymail"))) {
                ...
                sql = " SELECT date_start, time_start, time_end, reserve_rm.status, reserve_rm.bl_id, fl_id, rm_id, "
                ...
                if (jsReservation.has("date_start")) {
                    dateCheckTimezone =
                            getDateValue(context, jsReservation.getString("date_start"));
                }

                if (listRoomReserve.isEmpty()) {
                    valuesToMail.put("reserve_rm.comments", "");
                    valuesToMail.put("reserve_rm.date_start",
                        jsReservation.getString("date_start"));
                    valuesToMail.put("reserve_rm.time_start",
                        jsReservation.getString("time_start"));
                    valuesToMail.put("reserve_rm.time_end", jsReservation.getString("time_end"));
                    valuesToMail.put("reserve_rm.status", getEnumFieldDisplayedValue(context,
                        "reserve", "status", jsReservation.getString("status")));
                    final String buildingId = jsReservation.getString("bl_id");
                    valuesToMail.put("reserve_rm.bl_id", buildingId);
                    valuesToMail.put("reserve_rm.fl_id", "");
                    valuesToMail.put("reserve_rm.rm_id", "");
                    if (StringUtil.notNullOrEmpty(buildingId)) {
                        final String timezone =
                                LocalDateTimeUtil.getLocationTimeZone(null, null, null, buildingId);
                        valuesToMail.put("reserve_rm.tz_id", timezone);
                ...
                final TreeMap<String, String> messages =
                        getSendMailMessages(context, valuesToMail.get("locale"));
                ...
                String TimeZoneStr = (minutesoffset / 60) < 0
                        ? "+" + timeZoneFormatter.format(-minutesoffset / 60)
                        : timeZoneFormatter.format(-minutesoffset / 60);
                ...
                if ((invitation_type.equals("update"))
                    ...
                    subject += jsReservation.getString("date_start") + " "
                            + jsReservation.getString("time_start");
                }
                ...
                if ((invitation_type.equals("new")) || (invitation_type.equals("update"))) {
                    ...
                    if ((invitation_type.equals("new")) && (!parentId.equals("0"))) {
                        body += messages.get("BODY1_3") + " "
                                + valuesToMail.get("reserve.user_requested_by") + "\n\n";
                    } else {
                        body += messages.get("BODY1") + " "
                                + valuesToMail.get("reserve.user_requested_by") + "\n\n";
                    }
                } else if (invitation_type.equals("cancel")) {
                    body += messages.get("BODY1_2") + " "
                            + valuesToMail.get("reserve.user_requested_by") + "\n\n";
                }
                ...
                if (((invitation_type.equals("new")) && (!parentId.equals("0")))
                        || (invitation_type.equals("cancel") && !parentId.equals("0")
                                && (date_cancel == null || date_cancel.equals("")))) {
                    // Create recurring
                    if (invitation_type.equals("new")) {
                    ...
                    for (final TreeMap<String, String> rooms : listRoom) {
                        ...
                        final String date_room = rooms.get("reserve_rm.date_start");
                        dateCheckTimezone2 = getDateValue(context, date_room);
                        minutesoffset2 = -(tz2.getOffset(dateCheckTimezone2.getTime()) / 60000);
                        TimeZoneStr2 = (minutesoffset2 / 60) < 0
                                ? "+" + timeZoneFormatter.format(-minutesoffset2 / 60)
                                : timeZoneFormatter.format(-minutesoffset2 / 60);
                        absOffset2 = (minutesoffset2 > 0 ? minutesoffset2 : -minutesoffset2);
                        TimeZoneStr2 += (absOffset2 % 60) > 0
                                ? ":" + timeZoneFormatter.format(absOffset2 % 60) : "";

                        body += messages.get("BODY3") + " " + rooms.get("reserve_rm.time_start")
                                + " GMT" + TimeZoneStr2 + " ";
                        body += messages.get("BODY4") + " " + rooms.get("reserve_rm.time_end")
                                + " GMT" + TimeZoneStr2 + "\n";
                    }
                } else {
                    body += messages.get("BODY2") + " " + valuesToMail.get("reserve_rm.date_start")
                            + "\n";
                    body += messages.get("BODY3") + " " + valuesToMail.get("reserve_rm.time_start")
                            + " GMT" + TimeZoneStr + "\n";
                    body += messages.get("BODY4") + " " + valuesToMail.get("reserve_rm.time_end")
                            + " GMT" + TimeZoneStr + "\n";
                }
                body += "\n" + messages.get("BODY5") + " " + valuesToMail.get("reserve_rm.bl_id")
                        + "-";
                body += valuesToMail.get("reserve_rm.fl_id") + "-"
                        + valuesToMail.get("reserve_rm.rm_id") + "\n";
                ...
                final String path =
                        getParentContextAttributeXPath(context, "/*/preferences/@webAppPath")
                                ...
                                + getParentContextAttributeXPath(context,
                                    "/*/preferences/@schemaDirectory")
                                + java.io.File.separator + "per-site" + java.io.File.separator
                                + "temp";
                ...
                final String rm_time_start = jsReservation.getString("time_start");
                final String rm_time_end = jsReservation.getString("time_end");
                final String rm_date_start = jsReservation.getString("date_start");
                ...
                final TimePeriod timePeriod =
                        new TimePeriod(tDateStart, null, tTimeStart, tTimeEnd);

                final Date DateTimeStartGMT = timePeriod.getStartDateTime();
                final Date DateTimeEndGMT = timePeriod.getEndDateTime();
                ...
                for (int i = 0; i < attendees.length; i++) {
                    ...
                    line += ";RSVP=" + String.valueOf(require_reply);
                ...
                final ArrayList<String> attachments_requestedBy = new ArrayList<String>();
                // the organizer should not respond
                line = "ATTENDEE;CN=" + valuesToMail.get("reserve.requestedbymail") + ";RSVP=false"
                        + ":MAILTO:" + valuesToMail.get("reserve.requestedbymail");
                // KB 3050738 list all attendees in requestedBy ICS file
                line += "\n" + AttendeesSection;
                final String attendeesSection_requestedBy = line;
                // end kb3035551
                ...
                try {
                    ...
                    if (invitation_type.equals("new") || invitation_type.equals("cancel")) {
                        ...
                        filename = "reservation-" + jsReservation.getString("res_id") + "-";
                        filename += jsReservation.getString("date_start") + ".ics";

                        // kb#3035551: add different ics file for requested by email
                        filename_requestedBy =
                                "reservation-" + jsReservation.getString("res_id") + "-";
                        filename_requestedBy +=
                                jsReservation.getString("date_start") + "-requestedBy.ics";
                        ...
                        if (invitation_type.equals("new") && !parentId.equals("0")) {
                            ...
                            if (!res_date_end.equals("") && res_date_end != null) {
                                timePeriod.setEndDate(getDateValue(context, res_date_end));
                                final Date ResDateTimeEndGMT = timePeriod.getEndDateTime();
                                time = ResDateTimeEndGMT.getTime();
                                time += minutesoffset * 60 * 1000;
                                ResDateTimeEndGMT.setTime(time);
                                res_date_end = dateFormatter.format(ResDateTimeEndGMT) + "T"
                            ...
                            final String interval = jsReservation.has("recur_val3")
                                    ? jsReservation.get("recur_val3").toString() : "1";
                            ...
                            if (jsReservation.getString("recur_type").equals("day")) {
                                ...
                                parametersValues.put("rruleInternal",
                                    (res_recur_val1.get(0).toString()));
                                ...
                                if (jsReservation.has("recur_val2")) {
                                    ...
                                    final JSONArray res_recur_val2 = new JSONArray(
                                        "" + jsReservation.get("recur_val2") + " )");
                                ...
                                if (jsReservation.has("recur_val4")) {
                                    ...
                                    final JSONArray res_recur_val4 = new JSONArray(
                                        "" + jsReservation.get("recur_val4") + " )");
                            ...
                            if (jsRoomConflicts.length() > 0) {
                                ...
                                for (int i = 0; i < jsRoomConflicts.length(); i++) {
                                    rc_date_start = jsRoomConflicts.getJSONObject(i)
                                        .getString("date_start");
                                    rc_time_start = jsRoomConflicts.getJSONObject(i)
                                        .getString("time_start");
                                    final TimePeriod conflictPeriod =
                                            new TimePeriod(getDateValue(context, rc_date_start),
                                                null, getTimeValue(context, rc_time_start), null);
                                    final Date RcDateStartGMT = conflictPeriod.getStartDateTime();
                                ...
                                if (!excluded_dates.equals("")) {
                                    excluded_dates = excluded_dates.substring(0,
                                        excluded_dates.length() - 1);
                                }
                        ...
                        if (!conferenceId.equals("0")) {
                            parametersValues.put("uid", conferenceId);
                        ...
                        parametersValues.put("location",
                            valuesToMail.get("reserve_rm.bl_id") + "-"
                                    + valuesToMail.get("reserve_rm.fl_id") + "-"
                                    + valuesToMail.get("reserve_rm.rm_id"));
                        line = messages.get("SUBJECT1") + " ";
                        line += jsReservation.getString("date_start") + " ";
                        line += jsReservation.getString("time_start");
                        // If it's a recurrent reservation
                        if (((invitation_type.equals("new")) || (invitation_type.equals("update")))
                                && (!parentId.equals("0"))) {
                        ...
                        line += String.valueOf(valuesToMail.get("reserve.user_requested_by"))
                                + "\\n";
                        line += messages.get("BODY2") + " " + jsReservation.getString("date_start")
                                + "\\n";
                        line += messages.get("BODY3") + " " + jsReservation.getString("time_start");
                        line += " GMT" + TimeZoneStr + "\\n";
                        line += messages.get("BODY4") + " " + jsReservation.getString("time_end");
                        line += " GMT" + TimeZoneStr + "\\n";
                        line += messages.get("BODY5") + " "
                        ...
                        if (!String.valueOf(valuesToMail.get("reserve.comments")).equals("")) {
                            ...
                            line += String.valueOf(valuesToMail.get("reserve.comments"))
                                .replaceAll("\r\n", "\\\\n").replaceAll("\n", "\\\\n");
                            if (!String.valueOf(valuesToMail.get("reserve_rm.comments"))
                                .equals("")) {
                                line += "\\n";
                            }
                        }
                        if (!String.valueOf(valuesToMail.get("reserve_rm.comments")).equals("")) {
                            ...
                            // PC changed to solve KB item 3018280
                            line += String.valueOf(valuesToMail.get("reserve_rm.comments"))
                                .replaceAll("\r\n", "\\\\n").replaceAll("\n", "\\\\n");
                        }
                        ...
                        if (invitation_type.equals("cancel") && !parentId.equals("0")
                            ...
                            sql = "SELECT date_start, time_start, time_end, reserve_rm.bl_id, fl_id, rm_id, reserve_rm.comments, reserve_rm.res_id, rmres_id, city.timezone_id";
                            sql += " FROM reserve_rm, bl LEFT OUTER JOIN city ";
                            sql += " ON city.city_id=bl.city_id AND city.state_id=bl.state_id";
                            sql += " WHERE reserve_rm.status = 'Cancelled'";
                            sql += " AND res_id IN";
                            sql += " (SELECT res_id FROM reserve";
                            sql += " WHERE res_parent = " + parentId
                                    + " AND recurring_date_modified = 1)";
                            sql += " AND reserve_rm.bl_id=bl.bl_id";
                            sql += " ORDER BY date_start asc";

                            final List<Map<?, ?>> RoomReservesToCancel =
                            ...
                            if (!RoomReservesToCancel.isEmpty()) {
                                ...
                                for (final Map<?, ?> record : RoomReservesToCancel) {
                                    ...
                                    if (!(getString(record, "timezone_id").equals(""))) {
                                        tz3 = TimeZone
                                            .getTimeZone(getString(record, "timezone_id"));
                                    } else {
                                    ...
                                    TimeZoneStr3 = (minutesoffset3 / 60) < 0
                                            ? "+" + timeZoneFormatter.format(-minutesoffset3 / 60)
                                            : timeZoneFormatter.format(-minutesoffset3 / 60);
                                    absOffset3 =
                                            (minutesoffset3 > 0 ? minutesoffset3 : -minutesoffset3);
                                    TimeZoneStr3 += (absOffset3 % 60) > 0
                                    ...
                                    resToCancelResID =
                                            getIntegerValue(context, record.get("res_id"))
                                                .toString();
                                    final TimePeriod cancelPeriod =
                                            new TimePeriod(resToCancelDateStart, null,
                                                resToCancelTimeStart, resToCancelTimeEnd);
                                    final Date resToCancelDateTimeStartGMT =
                                            cancelPeriod.getStartDateTime();
                                    ...
                                    final Date resToCancelDateTimeEndGMT =
                                            cancelPeriod.getEndDateTime();
                                    time = resToCancelDateTimeEndGMT.getTime();
                                    time += minutesoffset3 * 60 * 1000;
                                    resToCancelDateTimeEndGMT.setTime(time);

                                    filename = "reservation-" + resToCancelResID + "-";
                                    filename +=
                                            String.valueOf(resToCancelDateStart) + "-cancel.ics";

                                    // kb#3035551: add different ics file for requested by email
                                    filename_requestedBy = "reservation-" + resToCancelResID + "-";
                                    filename_requestedBy += String.valueOf(resToCancelDateStart)
                                    ...
                                    parametersValues.put("location",
                                        resToCancelBl + "-" + resToCancelFl + "-" + resToCancelRm);
                                    line = messages.get("SUBJECT1") + " ";
                                    ...
                                    line += String.valueOf(resToCancelTimeEnd) + " GMT"
                                            + TimeZoneStr3 + "\\n";
                                    line += messages.get("BODY5") + " " + resToCancelBl + "-"
                                    ...
                                    if (!resToCancelComments.equals("")) {
                                        line += messages.get("BODY6_2") + "\\n";
                                        // PC changed to solve KB item 3018280
                                        line += resToCancelComments.replaceAll("\r\n", "\\\\n")
                                            .replaceAll("\n", "\\\\n");
                                    }
                                    parametersValues.put("description", line);
                                    // END: Create Config parameters
                                    ...
                                    // kb#3035551: add different ics file for requested by email
                                    absoluteFilename_requestedBy = createAttachments_requestedBy(
                                        context, parametersValues);
                                    if (!absoluteFilename_requestedBy.equals("")) {
                                        attachments_requestedBy.add(absoluteFilename_requestedBy);
                                    }
                    ...
                    if ((invitation_type.equals("update"))
                        ...
                        filename = "reservation-" + jsReservation.getString("res_id") + "-";
                        filename += jsReservation.getString("date_start") + "-cancel.ics";

                        // kb#3035551: add different ics file for requested by email
                        filename_requestedBy =
                                "reservation-" + jsReservation.getString("res_id") + "-";
                        filename_requestedBy +=
                                jsReservation.getString("date_start") + "-cancel-requestedBy.ics";
                        ...
                        final TimePeriod originalPeriod = new TimePeriod(tOriginalDate, null,
                            tOriginalTimeStart, tOriginalTimeEnd);
                        final Date OriginalDateTimeStartGMT = originalPeriod.getStartDateTime();
                        time = OriginalDateTimeStartGMT.getTime();
                        time += minutesoffset4 * 60 * 1000;
                        OriginalDateTimeStartGMT.setTime(time);
                        final Date OriginalDateTimeEndGMT = originalPeriod.getEndDateTime();
                    ...
                    handleNotificationError(context, ACTIVITY_ID + "-" + RULE_ID
                            + ": Failed sending invitations " + e.getMessage(),
                        errMessage, e, "");
                    this.log.error("Failed sending invitations", e);
                }
                ...
                if (to != null && !to.equals("")) {
                    try {
                        sendEmail(body, from, host, port, subject, to, null, null, userId, password,
                            attachments, CONTENT_TYPE_TEXT_UFT8, ACTIVITY_ID);
                    } catch (final Throwable e) {
                        handleNotificationError(context,
                            ACTIVITY_ID + "-" + RULE_ID + ": SENDEMAIL ERROR", errMessage, e, to);
                        this.log.error("Failed sending invitations", e);
                    }
            ...
            handleNotificationError(context,
                ACTIVITY_ID + "-" + RULE_ID + ": Failed sending invitations " + e.getMessage(),
                errMessage, e, "");
            this.log.error("Failed sending invitations", e);
    ...
    public void getNumberPendingReservations(final String xmlRecord) {
        ...
        final String errMessage = localizeMessage(context, ACTIVITY_ID,
            "GETNUMBERPENDINGRESERVATIONS_WFR", "GETNUMBERPENDINGRESERVATIONSERROR", null);
        ...
        if ((!bl_id.equals("")) && (!fl_id.equals("")) && (!rm_id.equals(""))
            ...
            sql = " SELECT COUNT(*) as pending FROM reserve_rm WHERE status <> 'Cancelled' "
                    + " AND status <> 'Rejected' AND bl_id = " + literal(context, bl_id)
                    + " AND fl_id = " + literal(context, fl_id) + " AND rm_id = "
                    + literal(context, rm_id) + " AND config_id = " + literal(context, config_id)
                    + " AND rm_arrange_type_id = " + literal(context, rm_arrange_type_id);

            try {
                final List<Map<?, ?>> reservePending = retrieveDbRecords(context, sql);
                if (!reservePending.isEmpty()) {
                    final Map<?, ?> record = reservePending.get(0);
    ...
    public void getNumberPendingResourceReservations(final String xmlRecord) {
        ...
        if ((!resource_id.equals("")) && ((reservable.intValue()) == 0)) {
            sql = " SELECT COUNT(*) as pending FROM reserve_rs WHERE status <> 'Cancelled' "
    ...
    public void notifyApprover(final EventHandlerContext context, final String res_type) {
        ...
        // notification rule error message
        final String errMessage = localizeMessage(context, ACTIVITY_ID, "NOTIFYAPPROVER_WFR",
            "NOTIFYAPPROVERERROR", null);
        ...
        try {
            ...
            if ((!rmrsResId.equals("")) && (!userToNotify.equals(""))) {
                ...
                sql = " SELECT email, locale FROM afm_users WHERE user_name = "
                ...
                if (!valuesToMail.get("approveremail").equals("")) {

                    // If we can't find the locale of the user to notify, search the locale of the
                    // creator of the reservation
                    if (valuesToMail.get("locale").equals("")) {
                        ...
                        final List<Map<?, ?>> recordsSql = retrieveDbRecords(context, sql);

                        // If locale is found
                        if (!recordsSql.isEmpty()) {
                            final Map<?, ?> recordOfSql = recordsSql.get(0);
                    ...
                    if (res_type.equals("room")) {
                        ...
                        sql = "SELECT rmres_id,reserve_rm.date_start,reserve_rm.time_start,reserve_rm.time_end,"
                    ...
                    // Get resource reservation info if it's a resource reservation
                        ...
                        sql = "SELECT rsres_id,reserve_rs.date_start,reserve_rs.time_start,reserve_rs.time_end,"
                    ...
                    if (allQueriesOk) {
                        ...
                        messages.put("SUBJECT",
                            localizeMessage(context, ACTIVITY_ID, "NOTIFYAPPROVER_WFR",
                                "NOTIFYAPPROVER_SUBJECT", valuesToMail.get("locale")));
                        for (int i = 1; i <= maxBody; i++) {
                            messages.put("BODY" + i,
                                localizeMessage(context, ACTIVITY_ID, "NOTIFYAPPROVER_WFR",
                                    "NOTIFYAPPROVER_BODY_PART" + i, valuesToMail.get("locale")));
                        }
                        ...
                        message += (res_type.equals("room") ? messages.get("BODY1")
                                : messages.get("BODY2")) + "\n\n";
                        ...
                        message += (res_type.equals("room") ? messages.get("BODY3")
                                : messages.get("BODY4")) + "\n\n";

                        message += (res_type.equals("room")
                                ? messages.get("BODY5") + " " + valuesToMail.get("rmres_id")
                                : messages.get("BODY6") + " " + valuesToMail.get("rsres_id"))
                                + "\n";
                        ...
                        if (res_type.equals("room")) {
                            ...
                            message += messages.get("BODY14") + " " + valuesToMail.get("config_id")
                                    + "\n";
                            message += messages.get("BODY15") + " "
                                    + valuesToMail.get("rm_arrange_type_id") + "\n\n";
                        } else {
                            message += messages.get("BODY16") + " "
                                    + valuesToMail.get("resource_id") + "\n";
                            message += messages.get("BODY17") + " " + valuesToMail.get("quantity")
                                    + "\n\n";
                        }
                        ...
                        try {
                            // PC changed to solve KB 3016618
                            sendEmail(message, from, host, port, subject,
                                valuesToMail.get("approveremail"), null, null, userId, password,
                                null, CONTENT_TYPE_TEXT_UFT8, ACTIVITY_ID);
                        } catch (final Throwable e) {
