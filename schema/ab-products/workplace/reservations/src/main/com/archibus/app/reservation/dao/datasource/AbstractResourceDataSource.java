...
public abstract class AbstractResourceDataSource extends AbstractReservableDataSource<Resource>
    ...
    static String getUniqueResourcesRestriction(final Integer reservationId,
        ...
        final String uniqueNotReserved = " NOT EXISTS (SELECT res_id FROM reserve_rs WHERE "
                + editRestriction + " reserve_rs.resource_id = resources.resource_id "
    ...
    private static String getOverlappingReservationRestriction(final DataSource dataSource) {
        ...
        if (dataSource.isOracle()) {
            sql = " ( reserve_rs.time_start - (resources.pre_block + resources.post_block) / (24*60) < ${parameters['endTime']} ) "
            ...
            sql = " ( DATEADD(mi, -resources.pre_block - resources.post_block, reserve_rs.time_start) < ${parameters['endTime']}) "
                    + " and ( DATEADD(mi, resources.pre_block + resources.post_block, reserve_rs.time_end) > ${parameters['startTime']}) ";

        } else {
            sql = " ( Convert(char(10), DATEADD(mi, -resources.pre_block - resources.post_block, reserve_rs.time_start), 108) < Convert(char(10), ${parameters['endTime']}, 108) ) "
