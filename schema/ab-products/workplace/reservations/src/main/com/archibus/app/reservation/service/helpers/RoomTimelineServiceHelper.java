...
public class RoomTimelineServiceHelper extends RoomReservationServiceBase {
    ...
    public static List<String> getBuildingsToSearch(final JSONObject locationFilter) {
        ...
        buildingIds.add(locationFilter.optString(
            com.archibus.app.reservation.dao.datasource.Constants.BL_ID_FIELD_NAME, ""));
    ...
    private void createRoomAllocationEventsWithTimeZone(final Date startDate,
        ...
        final JSONArray events = timeline.getJSONArray(EVENTS);
        final List<RoomAllocation> visibleRoomAllocations =
                getVisibleRoomAllocations(startDate, reservationIds, timeline, roomArrangement,
        ...
        for (final RoomAllocation roomAllocation : visibleRoomAllocations) {
            ...
            if (roomArrangement.equals(roomAllocation.getRoomArrangement())) {
                ...
                allocatedArrangement =
                        this.roomArrangementDataSource.get(roomAllocation.getBlId(),
                            roomAllocation.getFlId(), roomAllocation.getRmId(),
                            roomAllocation.getConfigId(), roomAllocation.getArrangeTypeId());
            }
            final JSONObject event =
                    TimelineHelper.createRoomReservationEventWithOffset(timeline,
    ...
    private List<RoomAllocation> getVisibleRoomAllocations(final Date startDate,
        ...
        final int timeZoneOffset =
                TimeZoneConverter.getCombinedOffset(startDate, roomArrangement.getDayStart(),
                    localTimeZoneId, timeZoneId);

        List<RoomAllocation> visibleRoomAllocations = null;
        if (timeZoneOffset == 0) {
            visibleRoomAllocations =
                    this.roomAllocationDataSource.getAllocatedRooms(startDate, roomArrangement,
                        reservationIds);
        } else {
            ...
            final List<RoomAllocation> roomAllocations =
                    getRoomAllocationsInRange(startDate, reservationIds, roomArrangement,
    ...
    private List<RoomAllocation> getRoomAllocationsInRange(final Date startDate,
        ...
        if (timelineStartDate != null && timelineStartDate.before(startDate)) {
            roomAllocations.addAll(this.roomAllocationDataSource.getAllocatedRooms(
                timelineStartDate, roomArrangement, reservationIds));
     ...
     */
    public RoomReservation createRoomReservation(final Integer reservationId, final Date startDate,
            final Time startTime, final Time endTime, final JSONObject locationFilter,
            final String timeZoneId) {
        ...
        if (reservationId != null && reservationId > 0) {
            final RoomReservation existingReservation =
