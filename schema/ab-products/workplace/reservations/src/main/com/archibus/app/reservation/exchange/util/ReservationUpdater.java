...
public final class ReservationUpdater {
    ...
    public static boolean updateReservation(final IReservation reservation,
        ...
        try {
            return updateReservation(reservation, AppointmentEquivalenceChecker.toCalendarEvent(appointment));
    ...
    public static boolean updateReservation(final IReservation reservation,
        ...
        try {
            return updateReservation(reservation, AppointmentEquivalenceChecker.toCalendarEvent(request));
        } catch (final ServiceLocalException exception) {
            // @translatable
     ...
     * @return true if equal, false if different
     */
    public static boolean updateReservation(final IReservation reservation, final ICalendarEvent event) {
        final boolean dateTimeEqual = AppointmentEquivalenceChecker.compareDateTime(reservation, event);
        if (!dateTimeEqual) {
            reservation.setStartDateTime(event.getStartDateTime());
            reservation.setEndDateTime(event.getEndDateTime());
        }
        
        final boolean subjectEqual = AppointmentEquivalenceChecker.compareSubject(reservation, event);
        if (!subjectEqual) {
            reservation.setReservationName(event.getSubject());
        }
        
        final boolean attendeesEqual =
                AppointmentEquivalenceChecker.compareToReservationAttendees(reservation,
                    event.getEmailAddresses());
        if (!attendeesEqual) {
