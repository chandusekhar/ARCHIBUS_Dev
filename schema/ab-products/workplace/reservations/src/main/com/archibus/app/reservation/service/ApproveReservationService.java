...
public class ApproveReservationService {
    ...
    public void approveReservation(final String reservationType, final DataSetList records) {
        ...
        if (RESERVATION_TYPE_ROOM.equals(reservationType)) {
            approveReservationServiceHelper.approveRoomAllocations(records);
        } else if (RESERVATION_TYPE_RESOURCE.equals(reservationType)) {
            approveReservationServiceHelper.approveResourceAllocations(records);
        }
    ...
    public void rejectReservation(final String reservationType, final DataSetList records,
        ...
        if (RESERVATION_TYPE_ROOM.equals(reservationType)) {
            approveReservationServiceHelper.rejectRoomAllocations(records, comments);
        } else if (RESERVATION_TYPE_RESOURCE.equals(reservationType)) {
            approveReservationServiceHelper.rejectResourceAllocations(records, comments);
        }
    ...
    public void checkRoomAndResourcesApproval() {
        processExpiredReservations(
            approveReservationServiceHelper.getExpiredRoomAllocationRecords(RM_ARRANGE_TABLE),
            RESERVATION_TYPE_ROOM, RM_ARRANGE_TABLE);
        
        processExpiredReservations(
            approveReservationServiceHelper.getExpiredResourceAllocationRecords(RESOURCES_TABLE),
    ...
    private void processExpiredReservations(final List<DataRecord> records,
        ...
        final String rejectMessage =
                EventHandlerBase.localizeMessage(ContextStore.get().getEventHandlerContext(),
                    "AbWorkplaceReservations", "CHECKROOMANDRESOURCESAPPROVAL_WFR",
                    "CHECKROOMANDRESOURCESAPPROVALREJECTMESSAGE", null);
        ...
        for (final DataRecord record : records) {
            ...
            switch (action) {
                ...
                default:
                    // do nothing
                    logger.warn("Skipping invalid action_approval_expired in " + tableName + ": "
                            + action);
                    break;
            }
        }
