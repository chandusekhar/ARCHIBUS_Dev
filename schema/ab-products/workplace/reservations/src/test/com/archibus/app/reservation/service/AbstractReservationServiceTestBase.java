...
public abstract class AbstractReservationServiceTestBase extends ConfiguredDataSourceTestBase {
    ...
    /** Initial resevration name for testing. */
    protected static final String INITIAL_RESERVATION_NAME = "test name";
    ...
    /** The email. */
    protected String email = "unittest2@procos1.onmicrosoft.com";
     ...
     * @throws ExceptionBase when the parameter is an invalid time
     */
    protected Time createTime(final String formattedTime) throws ExceptionBase {
        try {
            return new Time(this.timeFormatter.parse(formattedTime).getTime());
        } catch (final ParseException exception) {
            throw new ExceptionBase("Invalid time string", exception);
        }
    }

    /**
     * Create an SQL date for freemarker support.
     *
     * @param millis the date in milliseconds
     * @return the converted date
     */
    protected Date createSqlDate(final long millis) {
    ...
    protected DataRecord createReservation(final DataRecord reservation, final boolean recurrent) {
        if (recurrent) {
            reservation.setValue("reserve.recurring_rule",
        ...
        reservation.setValue("reserve.attendees", "jason.matthews@mailinator.com");
        reservation.setValue("reserve.reservation_name", INITIAL_RESERVATION_NAME);
    ...
    /**
     * Disable generating work requests for trades. Call this for unit tests that don't require
     * testing work request generation.
     *
     * <p>
    ...
    protected static void withoutWorkRequests() {
        SqlUtils.executeUpdate("tr", "UPDATE tr SET wr_from_reserve = 0 WHERE wr_from_reserve = 1");
    }
}
