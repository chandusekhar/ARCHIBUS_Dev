...
public class RoomReservationDataSourceTest extends ReservationDataSourceTestBase {
    ...
    /** The constant 10. Used in tests. */
    private static final int DELTA_BLOCK = 10;
    ...
    public void testSaveRoomReservationWithResources() {
        try {
            ...
            final RoomReservation savedReservation =
                    this.roomReservationDataSource.getActiveReservation(this.existingReservation
                        .getReserveId());
            Assert.assertEquals(1, savedReservation.getResourceAllocations().size());
            final ResourceAllocation savedAllocation =
                    savedReservation.getResourceAllocations().get(0);
            Assert.assertEquals(this.existingReservation.getStartDateTime(),
                savedAllocation.getStartDateTime());
            Assert.assertEquals(this.existingReservation.getEndDateTime(),
                savedAllocation.getEndDateTime());
    ...
    /**
     * Test saving a room reservation with a resource that has a larger pre- and post-block.
     */
    public void testAdjustResourceTimes() {
        final Resource resource = this.resourceDataSource.get(RESOURCE_ID);
        final RoomArrangement roomArrangement =
                this.roomArrangementDataSource.get(BL_ID, FL_ID, RM_ID, CONFIG_ID, ARRANGE_TYPE_ID);
        resource.setPreBlock(roomArrangement.getPreBlock() + DELTA_BLOCK);
        resource.setPostBlock(roomArrangement.getPostBlock() + DELTA_BLOCK);
        ...
        // save the room reservation with the resource
        this.roomReservationDataSource.save(this.existingReservation);

        // check that the resource times have been adjusted
        final RoomReservation savedReservation =
                this.roomReservationDataSource.getActiveReservation(this.existingReservation
                    .getReserveId());
        Assert.assertEquals(1, savedReservation.getResourceAllocations().size());
        final ResourceAllocation savedAllocation = savedReservation.getResourceAllocations().get(0);

        final Calendar calendar = Calendar.getInstance();
        calendar.setTime(this.existingReservation.getStartDateTime());
        calendar.add(Calendar.MINUTE, DELTA_BLOCK);
        final Date expectedStart = calendar.getTime();
        calendar.setTime(this.existingReservation.getEndDateTime());
        calendar.add(Calendar.MINUTE, -DELTA_BLOCK);
        final Date expectedEnd = calendar.getTime();

        Assert.assertEquals(expectedStart, savedAllocation.getStartDateTime());
        Assert.assertEquals(expectedEnd, savedAllocation.getEndDateTime());
    }
    ...
    public void testGetRoomReservation() {
        try {
            ...
            roomReservation =
                    this.roomReservationDataSource.getActiveReservation(this.existingReservation
                        .getReserveId());
            ReservationUtils.convertToTimeZone(roomReservation,
    ...
    public void testCancelReservationInTimeZone() {
        try {
            ...
            roomReservation =
                    this.roomReservationDataSource.getActiveReservation(this.existingReservation
                        .getReserveId());
            ReservationUtils.convertToTimeZone(roomReservation, Constants.TIMEZONE_UTC);
            this.roomReservationDataSource.cancel(roomReservation);
