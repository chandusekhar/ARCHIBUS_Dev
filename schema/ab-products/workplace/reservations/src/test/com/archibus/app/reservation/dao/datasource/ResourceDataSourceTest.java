...
public class ResourceDataSourceTest extends ReservationDataSourceTestBase {
    ...
    /** Chairs resource in HQ. */
    private static final String CHAIRS_HQ = "CHAIRS HQ";
    ...
    public void testCheckAvailableResource() {
        boolean allowed = this.resourceDataSource.checkResourceAvailable(COFFEE_HQ,
            this.existingReservation, this.existingReservation.getTimePeriod());
        Assert.assertTrue(allowed);

        allowed = this.resourceDataSource.checkResourceAvailable(COFFEE_JFK,
            this.existingReservation, this.existingReservation.getTimePeriod());
        Assert.assertFalse(allowed);
    }

    /**
     * Test detecting that a given resource standard is not allowed in the given room.
     */
    public void testResourceStandardsNotAllowed() {
        final Resource resource = this.resourceDataSource.get(CHAIRS_HQ);
        final RoomAllocation allocation = this.existingReservation.getRoomAllocations().get(0);
        final RoomArrangement room =
        ...
        Assert.assertTrue(room.allowsResourceStandard(resource.getResourceStandard()));
        Assert.assertTrue(this.resourceDataSource.checkResourceAvailable(CHAIRS_HQ,
            this.existingReservation, this.existingReservation.getTimePeriod()));

        // test with only one standard set
        room.setStandardsNotAllowed("'" + resource.getResourceStandard() + "'");
        Assert.assertFalse(room.allowsResourceStandard(resource.getResourceStandard()));
        this.roomArrangementDataSource.update(room);
        Assert.assertFalse(this.resourceDataSource.checkResourceAvailable(CHAIRS_HQ,
            this.existingReservation, this.existingReservation.getTimePeriod()));

        // test with other standards included
        room.setStandardsNotAllowed(
            "'IT-SUPPORT','" + resource.getResourceStandard() + "','TABLES'");
        Assert.assertFalse(room.allowsResourceStandard(resource.getResourceStandard()));
        this.roomArrangementDataSource.update(room);
        Assert.assertFalse(this.resourceDataSource.checkResourceAvailable(CHAIRS_HQ,
            this.existingReservation, this.existingReservation.getTimePeriod()));

        // when other room arrangements matching the location allow it, it should be allowed
    ...
    public void testGetUniqueResources() {
        ...
        try {
            ...
            final List<Resource> resources = this.resourceDataSource.findAvailableUniqueResources(
                this.existingReservation, this.existingReservation.getTimePeriod());
    ...
    public void testGetLimitedResources() {
        ...
        try {
            ...
            final List<Resource> resources = this.resourceDataSource.findAvailableLimitedResources(
                this.existingReservation, this.existingReservation.getTimePeriod());
    ...
    public void testGetCateringResources() {
        try {
            ...
            final List<Resource> resources = this.resourceDataSource
                .findAvailableUnlimitedResources(reservation2, reservation2.getTimePeriod());

            Assert.assertNotNull(resources);

            reservation2
                .setStartTime(new Time(this.timeFormatter.parse("1899-12-30 06:00:00").getTime()));
