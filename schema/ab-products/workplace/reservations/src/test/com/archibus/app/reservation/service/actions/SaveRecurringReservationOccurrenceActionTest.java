...
public class SaveRecurringReservationOccurrenceActionTest extends DataSourceTestBase {
    ...
    public void testHandleOccurrence() {
        try {
            ...
            RoomReservation savedReservation = this.savedReservations.get(0);
            ...
            // try booking the same time period again, this should create a reserve record in Conflicted status
            Assert.assertTrue(this.action.handleOccurrence(occurrenceDate));
            Assert.assertEquals(2, this.savedReservations.size());
            final RoomReservation conflictedReservation = this.savedReservations.get(1);
            Assert.assertEquals(Constants.STATUS_ROOM_CONFLICT, conflictedReservation.getStatus());
            Assert.assertTrue(conflictedReservation.getRoomAllocations().isEmpty());
            savedReservation = this.roomReservationDataSource.get(conflictedReservation.getReserveId());
            Assert.assertEquals(Constants.STATUS_ROOM_CONFLICT, savedReservation.getStatus());
            Assert.assertTrue(savedReservation.getRoomAllocations().isEmpty());
