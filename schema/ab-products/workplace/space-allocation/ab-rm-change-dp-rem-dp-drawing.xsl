<?xml version="1.0"?>
<!-- ab-rm-change-dp-rem-dp-drawing.xsl -->
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="addtionalLayers">
      ...
      <xsl:if test="$isDynamicHighlights!='true'">
        <!-- Highlight rooms with dv dp matching the user's -->
        <xsl:for-each select="/*/afmTableGroup/dataSource/data/records/record"><xsl:if test="@dp.dv_id=//preferences/@em_dv_id  and @dp.dp_id=//preferences/@em_dp_id">Z-RM-HL-<xsl:value-of select="@rm.bl_id" />-<xsl:value-of select="@rm.fl_id" />-<xsl:value-of select="@rm.rm_id" />;</xsl:if><xsl:if test="@bl.bl_id!=''">Z-BL-HL-<xsl:value-of select="@bl.bl_id"/>;</xsl:if></xsl:for-each>
      </xsl:if>
    </xsl:variable>
