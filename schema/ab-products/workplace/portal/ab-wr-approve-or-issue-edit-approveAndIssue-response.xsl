...
<!-- ab-wr-approve-or-issue-edit-approveAndIssue-response.xsl -->
<!-- XSLT for ab-wr-approve-or-issue-edit-approveAndIssue-response.axvw -->
			...
			<table  width="100%" valign="top">
				<tr><td>
					<table valign="top" class="showingTgrpTitleTable">
						<tr><td>
							<xsl:text/><xsl:value-of select="concat($wo_tgrp/title,': ',$str_wo_id)"/>
						<hr /></td></tr>
					</table>
				</td></tr>
				<tr><td>
					<xsl:call-template name="ReportColumnFormat">
						<xsl:with-param name="afmTableGroup" select="$wo_tgrp"/>
