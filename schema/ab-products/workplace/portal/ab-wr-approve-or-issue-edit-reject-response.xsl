...
<!-- ab-wr-approve-or-issue-edit-reject-response.xsl -->
<!-- XSLT for ab-wr-approve-or-issue-edit-reject-response.axvw -->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">	
	...
	<xsl:template match="/">
		...
		<body class="body" leftmargin="0" rightmargin="0" topmargin="0" bottomMargin="0">
			<xsl:variable name="hwr_tgrp" select="/*/afmTableGroup[position()=1]"/>
			<xsl:variable name="str_wr_id" select="$hwr_tgrp/dataSource/data/records/record/@hwr.wr_id"/>
			...
			<table  width="100%" valign="top">
				<tr><td>
					<table valign="top" class="showingTgrpTitleTable">
						<tr><td>
							<xsl:text/><xsl:value-of select="concat($hwr_tgrp/title,': ',$str_wr_id)"/>
						<hr /></td></tr>
					</table>
				</td></tr>
				<tr><td>
					<xsl:call-template name="ReportColumnFormat">
						<xsl:with-param name="afmTableGroup" select="$hwr_tgrp"/>
