...
//normal lookup is not working here since the field name is not normal
//in filter or console????
...
function gettingRecordsData()
{
	...
	//a permant SQL restriction
	var strSQLRestriction = '<restriction type="sql" sql="(rm.rm_std = \'CONFERENCE\' OR ( rm.rm_cat = \'SUPPORT\' AND rm.rm_type IN ( \'CONFERENCE\', \'TRAINING\' ) ))" />';
	//even strReturned is empty, <restrictions></restrictions> is necessary to be passed to server
	strReturned = '<userInputRecordsFlag><restrictions><restriction type="parsed">' + strReturned + '</restriction>'+strSQLRestriction+'</restrictions></userInputRecordsFlag>';
	return strReturned;
}
...
function onSelectV(strSerialized, iCounter)
{
	...
	if(strField != "")
	{
		...
		if(strSerialized != "")
		{
			...
			//getting all records
			strData = strData + '<userInputRecordsFlag>' + gettingRecordsDataFromForm(objForm) + '</userInputRecordsFlag>';
...
{
	var strReturned = "";
	var bl_id = objForm.elements["values1"].value;
	var fl_id = objForm.elements["values2"].value;
	var rm_id = objForm.elements["values3"].value;
	var rm_name = objForm.elements["values4"].value;
	bl_id = trim(bl_id);
	fl_id = trim(fl_id);
	rm_id = trim(rm_id);
	rm_name = trim(rm_name);
	strReturned = '<record rm.bl_id="'+bl_id+'" rm.fl_id="'+fl_id+'" rm.rm_id="'+rm_id+'" rm.name="'+rm_name+'"/>';
	return strReturned;
}
//???????????record???restriction????
function onSelectV_rm_name(strSerialized, iCounter)
{
	var strXMLData = "";
	var objForm  = document.forms[afmInputsFormName];
	var strField = objForm.elements['field'+iCounter].value;
	if(strField != "")
	{
		...
		if(strSerialized != "")
		{
			...
			//passing a sql restriction to restrict room's name list
			var bl_id = objForm.elements["values1"].value;
			var fl_id = objForm.elements["values2"].value;
			var rm_id = objForm.elements["values3"].value;
			bl_id = trim(bl_id);
			fl_id = trim(fl_id);
			rm_id = trim(rm_id);
			var str_sql_restriction = "";
			if(bl_id!="")
			{
				if(str_sql_restriction=="")
					str_sql_restriction = str_sql_restriction + "rm.bl_id LIKE '"+bl_id+"%' ";
				else
					str_sql_restriction = str_sql_restriction + "AND rm.bl_id LIKE '"+bl_id+"%' ";
			}
			if(fl_id!="")
			{
				if(str_sql_restriction=="")
					str_sql_restriction = str_sql_restriction + "rm.fl_id LIKE '"+fl_id+"%' ";
				else
					str_sql_restriction = str_sql_restriction + "AND rm.fl_id LIKE '"+fl_id+"%' ";
			}
			if(rm_id!="")
			{
				if(str_sql_restriction=="")
					str_sql_restriction = str_sql_restriction + "rm.rm_id LIKE '"+rm_id+"%' ";
				else
					str_sql_restriction = str_sql_restriction + "AND rm.rm_id LIKE '"+rm_id+"%' ";
			}
			if(str_sql_restriction!="")
			{
				str_sql_restriction = '<restriction type="sql" sql="'+str_sql_restriction + '"/>';
				strData = strData + str_sql_restriction;
			}
			strXMLValue = strSerializedInsertingDataFirstPart + strData +  strSerializedInsertingDataRestPart;
			//calling OpenSelectVWindow() to open a new window for server
			//to show available values for specified field
