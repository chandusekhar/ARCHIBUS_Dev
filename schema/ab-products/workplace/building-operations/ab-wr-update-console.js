...
function gettingRecordsData()
{
	...
	if(objMainToolbarFrame!=null)
	{
		...
		if(tempArray!=null)
		{
			...
			//"SUPERVISOR_WR_UPDATE_INFO", which will be used to retreat info
			tempArray["SUPERVISOR_WR_UPDATE_INFO"] = newTempArray;
		}
	}
	//????
	...
	var strSQLStatement = "wr.status IN ('I','HP','HA','HL') ";
	if(wr_id != "")
	...
	if(bl_id != "")
		strSQLStatement = strSQLStatement + 'AND wr.bl_id LIKE \''+bl_id+'\' ';
	if(eq_id != "")
	...
	strSQLStatement = 'sql="'+strSQLStatement + '"';
...
function onPageLoad()
{
	...
	if(objMainToolbarFrame!=null)
	{
		...
		{
			var tempArray = objMainToolbarFrame.arrReferredByAnotherFrame1["SUPERVISOR_WR_UPDATE_INFO"];
			if(tempArray!=null)
			{
