...
function gettingRecordsData()
{
	...
	if(objMainToolbarFrame!=null)
	{
		...
		if(tempArray!=null)
		{
			...
			tempArray["WR_REVIEW_INFO"] = newTempArray;
		}
	}
	//????
	wr_id =convert2validXMLValueAndLiteralizeValue(wr_id);
	requestor =convert2validXMLValueAndLiteralizeValue(requestor);
	description =convert2validXMLValueAndLiteralizeValue(description);
	prob_type =convert2validXMLValueAndLiteralizeValue(prob_type);
	bl_id =convert2validXMLValueAndLiteralizeValue(bl_id);
	eq_id =convert2validXMLValueAndLiteralizeValue(eq_id);
	...
	var strSQLStatement = "";
	if(wr_id != "")
		strSQLRestriction = strSQLRestriction + '<clause relop="AND" op="=" value="'+wr_id+'"><field name="wr_id" table="wr"/></clause>';
...
function onPageLoad()
{
	...
	if(objMainToolbarFrame!=null)
	{
		...
		{
			var tempArray = objMainToolbarFrame.arrReferredByAnotherFrame1["WR_REVIEW_INFO"];
			if(tempArray!=null)
			{
...
in common.js
*/
function onSelectV(strSerialized, strField, strTemp, ascending)
{ 
	...
	if(strField != "")
	{
		...
		if(strSerialized != "")
		{
			...
			strData = '<fields><field role="self" ascending="'+ascending+'" table="'+temp_table+'" name="'+temp_field+'"/></fields>';
			//getting all records
