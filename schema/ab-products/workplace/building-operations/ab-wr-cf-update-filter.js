...
function gettingRecordsData()
{
	...
	//vvvvv we add a permanent SQL restriction to only display open requests
	var strSQLRestriction = '<restriction type="sql" sql="wrcf.wr_id IN (SELECT wr_id FROM wr  WHERE status IN (\'I\',\'HP\',\'HA\',\'HL\'))" />';

	//vvvvv even strReturned is empty, <restrictions></restrictions> is necessary to be passed to server
	strReturned = '<userInputRecordsFlag><restrictions>'+ strSQLRestriction +'<restriction type="parsed">' + strReturned + '</restriction></restrictions></userInputRecordsFlag>';
	return strReturned;
}
...
function validationInputs(n)
{
	...
	else if(objValue.value != "")
	{
		...
		{
			if(!validationNumeric(objValue,decima, truel))
				bReturned = false;
		}
