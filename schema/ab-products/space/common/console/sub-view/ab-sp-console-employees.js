...
var spaceExpressConsoleEmployees = View.createController('spaceExpressConsoleEmployees', {
    ...
    refresh: function(filter) {
    	...
    	if (filter) {
			...
			addTeamEmRestrictionParameter(this.emFilter); 
        }
    ...
    employeeGrid_onUnassign: function() {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            if ( '--' != rows[i].getFieldValue('em.location') ){
				hasLocation = true;
				break;
			}
        }
    ...
    /**
     * Handle the move order of employee assignments.
     * All the assignments will be cancelled and only generate move orders processed my Move management.
     */
    createNewMoveOrder: function() {
    	var employeePAList = this.drawingPanel.getSidecar().get('pendingAssignments');
    	if (!employeePAList || employeePAList.length < 2) {
    		View.alert(getMessage("insufficientEmployeeAssignments"));
    	} else {
    		var thisController = this;
    		var createGroupMoveOrderDialog = View.openDialog("ab-sp-console-create-group-move-order.axvw", null, true, {
    			width : 800,
    			height: 600,
    			collapsible: false,
    			afterViewLoad: function(dialogView) {
    				var dialogController = dialogView.controllers.get('createGroupMoveOrderController');
    				dialogController.employeePendingAssignment = employeePAList;
    				dialogController.onClose = thisController.handleFinishCreateGroupMoveOrders.createDelegate(thisController);
    			}
    		});
    	}
    },
    ...
    /**
     * Cancel employee assignments to wait for the completion of move management.
     */
    handleFinishCreateGroupMoveOrders: function() {
    	this.employeePendingAssignments_onCancelEmployeePendingAssignments();
    	View.closeDialog();
    	View.alert(getMessage('moveOrderCreated'));
    },
    
    /**
     * Add those employee assignments to an existing move order.
     */
    addToExistingMoveProject: function() {
    	var employeePAList = this.drawingPanel.getSidecar().get('pendingAssignments');
    	if(!employeePAList || employeePAList.length < 1) {
    		View.alert(getMessage('insufficientExistingEmployeeAssignments'));
    	} else {
    		var thisController = this;
    		var addToExistingMoveOrderDialog = View.openDialog("ab-sp-console-add-existing-move-order.axvw", null, true, {
    			width : 900,
    			height: 600,
    			collapsible: false,
    			afterViewLoad: function(dialogView) {
    				var dialogController = dialogView.controllers.get('addExistingMoveOrderController');
    				dialogController.employeePendingAssignment = employeePAList;
    				dialogController.onClose = thisController.handleFinishCreateGroupMoveOrders.createDelegate(thisController);
    			}
    		});
    ...
    employeeGrid_afterRefresh: function() {
		...
		//Added for 23.1: shows a concatenated list of teams to which the employee is a member on the As-Of date in the 'Teams' column.
		if ( this.isTeamSpaceEnabled ) {
			for (var x=0, column; column = this.employeeGrid.columns[x]; x++) {
				if ( column.id==='em.em_teams' )	{
					if (!column.hidden){
						var emTeamDs = View.dataSources.get('employeeTeamDS');
						var asOfDate = View.controllers.get('spaceExpressConsole').asOfDate;
						this.employeeGrid.gridRows.each(function(row) {
							var emId = row.getFieldValue('em.em_id');
							emTeamDs.addParameter('emId', emId);
							emTeamDs.addParameter('asOfDate', asOfDate);
							var teams = emTeamDs.getRecords();
							var teamList = "";
							for (var i=0; i<teams.length; i++ ){
								teamList += teams[i].getValue('team.team_id')+",";
							}
							teamList = teamList.substring(0, teamList.length-1);
							row.setFieldValue('em.em_teams', teamList ? teamList.substring(0, teamList.length): "" );
						});
					}
				}
			}
			...
			for (var x=0, column; column = this.employeeGrid.columns[x]; x++) {
				if ( column.id==='em.em_teams' )	{
					if (!column.hidden){
						this.employeeGrid.showColumn('em.em_teams', false);
						this.employeeGrid.update();
					}
				}
			}
		}
    ...
    /**
     * Added for 23.1: according to current status of the teams tab, show/hide column 'Teams' in employees grid.
     */
    enableTeamSpace: function(enabled) {
		for (var x=0, column; column = this.employeeGrid.columns[x]; x++) {
			if ( column.id==='em.em_teams' )	{
				this.employeeGrid.showColumn(column.id , enabled);
				this.employeeGrid.update();
			}
		}
...
/**
 * Added for 23.1: Show a pop-up window that displays all of that employee¡¯s team assignments (regardless of the filter dates).
 */
function showEmTeamAssignments() {
	var thisController = View.controllers.get('spaceExpressConsoleEmployees');
    var employeeGrid = View.panels.get('employeeGrid');
    var rowIndex = employeeGrid.rows[employeeGrid.selectedRowIndex];
    var emId = rowIndex["em.em_id"];
	var asOfDate = View.controllers.get('spaceExpressConsole').asOfDate;
	
	var restriction = "em_id='"+makeLiteral(emId)+"'"
	restriction += " and ( team.date_start is null or  ${sql.yearMonthDayOf('team.date_start')} <='"+asOfDate+"') ";
	restriction += " and ( team.date_end is null or  ${sql.yearMonthDayOf('team.date_end')}>='"+asOfDate+"') ";
