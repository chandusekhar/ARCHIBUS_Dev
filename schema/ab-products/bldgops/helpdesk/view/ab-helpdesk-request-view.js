...
var abHelpdeskRequestViewController = View.createController("abHelpdeskRequestViewController",{
	...
	requestPanel_beforeRefresh: function(){
		this.requestPanel.actions.get("survey").show(true);
		this.requestPanel.actions.get("verification").show(true);
		...
		this.costsPanel.show(false);
		this.satisfactionPanel.show(false);
	},
	...
	*/
	doPrepareWorks: function(){
		...
		var quest = this.descriptionPanel.getFieldValue("activity_log.act_quest");
		...
		var steps = ABHDC_getStepInformation('activity_log','activity_log_id',activityLogId,this.historyPanel,"history",true);
		...
		if(steps.length != null && steps.length > 0) {
			...
			if (lastStep) {
				...
				if(lastStepType.match("verification")){
					this.requestPanel.actions.get("survey").show(false);
				}else if(lastStepType.match("survey")){
					this.requestPanel.actions.get("verification").show(false);
				}else{
					this.requestPanel.actions.get("survey").show(false);
					this.requestPanel.actions.get("verification").show(false);
				}
...
function showSatisfactionSurveyForm(){
	if (lastStepType!=null&&lastStepType.match("survey")) {
		...
		var activity_log_id = panel.getFieldValue("activity_log.activity_log_id");
...
function showOnDemand(){
	...
	if((panel.getFieldValue("activity_log.wr_id")=='' || panel.getFieldValue("activity_log.wr_id")== 0)
		 && (panel.getFieldValue("activity_log.wo_id") == '' || panel.getFieldValue("activity_log.wo_id") == 0)){
		alert("No workorder or work request for this request");
	} else {	
		if(panel.getFieldValue("activity_log.wr_id")!='' && panel.getFieldValue("activity_log.wr_id") != 0){	
			var restriction = new Ab.view.Restriction();
			restriction.addClause("wr.wr_id",parseInt(panel.getFieldValue("activity_log.wr_id")),'=');
			View.openDialog("ab-helpdesk-request-ondemand-wr.axvw",restriction, false);			
		}else if(panel.getFieldValue("activity_log.wo_id")!='' && panel.getFieldValue("activity_log.wo_id")!=0) {
			var restriction = new Ab.view.Restriction();
			restriction.addClause("wo.wo_id",parseInt(panel.getFieldValue("activity_log.wo_id")),'=');
