...
var helpDeskCreateBasicController = View.createController("helpDeskBasicController", {
	...
	requestPanel_afterRefresh: function(){
        ...
        if(this.requestPanel.getFieldValue("activity_log.activity_log_id") == ''){
			View.panels.get("requestPanel").actions.get("deleteRequest").show(false);
		}else{
			View.panels.get("requestPanel").actions.get("deleteRequest").show(true);
		}
	...
	doPrepareWorks: function(){
		...
		var conditionAssessmentRestriction = null
		if(valueExists(ABHDC_getTabsSharedParameters()["condAssessmentRestrication"])){
			conditionAssessmentRestriction = ABHDC_getTabsSharedParameters()["condAssessmentRestrication"];
		} else if (valueExists(ABHDC_getTabsSharedParameters(true)["condAssessmentRestrication"])) {
			conditionAssessmentRestriction = ABHDC_getTabsSharedParameters(true)["condAssessmentRestrication"];
		}
		
		if(valueExists(conditionAssessmentRestriction)){
			this.handleConditionAssessmentRestriction(conditionAssessmentRestriction);
		} else{
	...
	handleConditionAssessmentRestriction: function(passedRestriction) {
		...
		clause = passedRestriction.findClause('activity_log.pr_id');
		if (clause != null && clause != undefined
				&& clause.value != null && clause.value != undefined){
			this.locationPanel.setFieldValue("activity_log.pr_id", clause.value);	
		} else {
			this.locationPanel.setFieldValue("activity_log.pr_id", '');
		}
		...
		clause = passedRestriction.findClause('activity_log.ta_id');
		if (clause != null && clause != undefined
				&& clause.value != null && clause.value != undefined){
			this.locationPanel.setFieldValue("activity_log.ta_id", clause.value);
		} else {
			this.locationPanel.setFieldValue("activity_log.ta_id", '');
		}
	...
	doWorkForEditServiceRequest: function() {
		...
		//KB3050449 - load sla priorities after all request parameters filled in the interface 
...
requestPanel_onDeleteRequest: function(){
