...
function returnCf() {
	...
	if (selectedRecords.length > 0) {
		for ( var i = 0; i < selectedRecords.length; i++) {
			...
			var record = {};
			record['wr.wr_id'] = id;
			record['wr.cf_comments'] = comments;
			records.push(record);
		}
		
		try {
			var result = Workflow.callMethod('AbBldgOpsOnDemandWork-WorkRequestService-returnWorkRequestsFromCf', records);
		} catch (e) {
			Workflow.handleError(e);
			return;
		}
			
	    closeQuickWindowAndRefreshConsole(View.panels.get("returnPanel"));
	}
}
...
function enableFormFieldsBaseOnWorkRequestStatus(formId,statusFormId) {
	...
	if(status=='I' || status=='HP' || status=='HL'|| status=='Com'|| status=='Rej'|| status=='S'){
		enable = true;
	...
	//eable fields when status is Issued and On Hold 
	//KB3042530 - enable actuals field when status is completed
	if(status=='I' || status=='HP' || status=='HL'|| status=='HA'||status=='Com' || status=='S'){
		enable = false;
	...
	fieldArray = ['wrtr.tr_id','wrpt.part_id','wrpt.pt_store_loc_id','wrcf.cf_id','wrtl.tool_id','wr_other.other_rs_type'];
	for(var i=0;i<fieldArray.length;i++){
		form.enableField(fieldArray[i], enable);
	}
