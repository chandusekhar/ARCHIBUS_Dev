...
var opsConsoleFilterRestrictionController = View.createController('opsConsoleFilterRestrictionController', {
	 ...
	 */	
	isRequestCraftspersonSql: "(wr.status IN ('I','HA','HP','HL','Rej')  AND  EXISTS(SELECT 1 FROM wrcf WHERE wrcf.wr_id = wr.wr_id and wrcf.status = 'Active' and (wrcf.cf_id IN (select cf.cf_id from cf where cf.email = ${sql.literal(user.email)})"
	...
	craftspersonRestriction: "((SELF_ASSIGN_RESTRICTION)"
		+ " OR (wr.status IN ('I','HA','HP','HL') AND  EXISTS(SELECT 1 FROM wrcf WHERE wrcf.wr_id = wr.wr_id and wrcf.status='Active' and (wrcf.cf_id IN (select cf.cf_id from cf where cf.email = ${sql.literal(user.email)})"
		+ "   OR wrcf.cf_id${sql.concat}'craftsperson' IN(${parameters['cfWorkflowSubstitutes']}))"
		+ ")))",
	...
	stepCompleterRestriction: "(EXISTS(SELECT 1 FROM wr_step_waiting where wr_step_waiting.wr_id=wr.wr_id and wr_step_waiting.status = wr.status"
		...
		+ "     OR wr_step_waiting.cf_id${sql.concat}wr_step_waiting.step_type IN (${parameters['cfWorkflowSubstitutes']}) "
	...
	checkSchemaAndSetRestriction : function() {
		...
		if(Workflow.callMethod('AbBldgOpsOnDemandWork-WorkRequestService-checkSchemaExisting','work_team', 'cf_assign').value){
			this.isWorkTeamSelfAssignSql = "  NOT EXISTS(SELECT 1 FROM wrcf WHERE wrcf.wr_id = wr.wr_id and wrcf.status = 'Active') AND" 
	...
	setPermanentRestrictionByRole : function(roleName) {
		...
		if(typeof BldgOpsConsoleLoadPendingSteps == 'undefined' || (valueExists(BldgOpsConsoleLoadPendingSteps) && BldgOpsConsoleLoadPendingSteps == '1')){
			...
			this.wrList.addParameter('leftJoinPendingSteps', "LEFT OUTER JOIN wr_step_waiting ON wr.wr_id=wr_step_waiting.wr_id AND (wr_step_waiting.role_name = ${sql.literal(user.role)} OR wr_step_waiting.user_name = ${sql.literal(user.name)} OR wr_step_waiting.cf_id${sql.concat}'craftsperson' IN (${parameters['cfWorkflowSubstitutes']}) OR wr_step_waiting.cf_id${sql.concat}wr_step_waiting.step_type IN (${parameters['cfWorkflowSubstitutes']}) OR wr_step_waiting.em_id${sql.concat}'supervisor' IN (${parameters['emWorkflowSubstitutes']}) OR wr_step_waiting.em_id${sql.concat}wr_step_waiting.step_type IN (${parameters['emWorkflowSubstitutes']}))");
	...
	setFilterRestriction : function() {
		...
		if(valueExists(BldgOpsConsoleLoadPendingSteps) && BldgOpsConsoleLoadPendingSteps == '0' && whatToShow == 'pendingSteps'){
			this.filterRestriction += " AND EXISTS(SELECT 1 FROM wr_step_waiting where wr_step_waiting.wr_id=wr.wr_id AND (wr_step_waiting.role_name = ${sql.literal(user.role)} OR wr_step_waiting.user_name = ${sql.literal(user.name)} OR wr_step_waiting.cf_id${sql.concat}wr_step_waiting.step_type IN (${parameters['cfWorkflowSubstitutes']}) OR wr_step_waiting.em_id${sql.concat}wr_step_waiting.step_type IN (${parameters['emWorkflowSubstitutes']}))) ";
		}
	...
	getFilterRestriction : function() {
		...
		+ this.addRestrictionFromFieldArray("bigBadFilter", 
				[ 'wrcf.cf_id', 'wrpt.part_id', 'eq.eq_std', 'wr.work_team_id','wr.dv_id', 'wr.dp_id', 'wr.eq_id', 'wr.wr_id', 'wr.wo_id', 'wr.requestor', 'wr.rm_id' , 'wrtr.tr_id','wr.pmp_id','wr.pms_id'  ]) 
	...
	setGroupByConfiguration : function(groupByDS, whatToShow) {
		...
		if (groupByDS == 'cfIdDS') {
			...
			this.wrList.addParameter('assignToSql', " CASE WHEN wrcf.cf_id IS NULL THEN '(Unassigned)' ELSE wrcf.cf_id END ");
			this.wrList.addParameter('assignToSqlOracle', " CASE WHEN wrcf.cf_id IS NULL THEN '(Unassigned)' ELSE wrcf.cf_id END ");
		} else {
			if (whatToShow == 'myApproved') {
