...
var opsConsoleWrListController =  View.createController('opsConsoleWrListController', {
	 ...
	 */
	statusOrderArray : [ 'Rej','R', 'A', 'AA', 'I', 'S', 'HA', 'HL', 'HP', 'Com' ],
	...
	/**
	 * Display double line for description field, true|false
	 */
	showDoubleLineForDescription : false,
	...
	/**
	 * can current craftsperson return
	 */
	canCurrentCfReturn : false,
	...
	afterInitialDataFetch : function() {
		...
		//query to see if current cf can return 
		var cfRecord = View.dataSources.get('cfAssignDS').getRecord();
		if(cfRecord){
			this.canCurrentCfReturn = cfRecord.getValue('cf.cf_unschedule') == '1' ? true : false;
		}
	...
	wrList_afterRefresh : function() {
		...
		//KB3048918 - enable rejected status bulk action
	...
	getFirstRowActions : function(rows) {
		...
		for ( var index = 1; index <= 5; index++) {
			...
			if(rows[0].getFieldValue('wr.status') == 'Rej' && firstRowActions['action' + index] == 'update' ){
	...
	prepareActionsForCraftsperson : function(row) {
		...
		if (status == 'I') {
			...
			actions['action1'] = 'update';
			actions['action2'] = 'completeCf';
			
			if(this.canCurrentCfReturn){
				actions['action3'] = 'returnCf';
				if (stepType) {
		...
		} else if (status == 'HA' || status == 'HP' || status == 'HL') {
	...
	displayDoubleLineForDescription : function(action) {
		// set displayDoubleLineForDescription flag
		this.showDoubleLineForDescription = action.checked;
		this.wrFilter.getSidecar().set('showDoubleLineForDescription', this.showDoubleLineForDescription);
		...
		if (this.showDoubleLineForDescription) {
	...
	onHighlightEscalatedRequests : function(action) {
		...
		this.wrFilter.getSidecar().set('highlightEscalatedReqeusts', this.highlightEscalatedReqeusts);
		this.wrFilter.getSidecar().save();
	...
	/**
	 * Reset Select row icons.
	 */
	resetSelectedRowIcons : function() {
		var wrIds = [];
		for ( var i = 0; i < opsConsoleWrListActionController.selectedWrRecordsForAction.length; i++) {
			wrIds.push(opsConsoleWrListActionController.selectedWrRecordsForAction[i].getValue('wr.wr_id'))
		}
		
		if(wrIds.length>0){
			this.checkIconsDS.addParameter('checkIconsRes', 'wr.wr_id IN(' + wrIds.toString() + ')');
			var records = this.checkIconsDS.getRecords();
			this.wrList.gridRows.each(function(row) {
				for ( var i = 0; i < records.length; i++) {
					if(row.getFieldValue('wr.wr_id') == records[i].getValue('wr.wr_id') &&
							(row.getFieldValue('wr.status') == 'A' 
						        || row.getFieldValue('wr.status') == 'AA')  ){
						if (records[i].getValue('wr.isEstimate')== 'true') {

							jQuery(row.actions.get('action1').button.dom).addClass('completedAction');

						}else{
							jQuery(row.actions.get('action1').button.dom).removeClass('completedAction');
						}
			
						if (records[i].getValue('wr.isSchedule')== 'true') {
			
							jQuery(row.actions.get('action2').button.dom).addClass('completedAction');
			
						}else{
							jQuery(row.actions.get('action2').button.dom).removeClass('completedAction');
						}
