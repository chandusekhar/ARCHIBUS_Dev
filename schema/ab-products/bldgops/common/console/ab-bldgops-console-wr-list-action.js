...
var opsConsoleWrListActionController =  View.createController('opsConsoleWrListActionController', {
	...
	/**
	 * Event handler for complete craftsperson
	 *  
	 */
	wrList_onCompleteCf : function(action) {
		this.onCompleteCf(action);
	},
	
	/**
	 * Event handler for return craftsperson
	 *  
	 */
	wrList_onReturnCf : function(action) {
		this.onReturnCf(action);
	},
	...
	showWorkRequestDetails: function(row) {
		...
		this.selectedWrRecordsForAction = [ row.getRecord() ];
	...
	onEstimation : function(action) {
		...
		if (View.WRids.length > 0) {
			...
			this.openDialog('ab-bldgops-console-wr-estimate.axvw',restriction, 1500, 1000, getMessage('estimateTitle') + ' ' + (action.button ? action.row.getFieldValue('wr.wr_id') : ''), true);
		}
	},
	...
	onUpdate : function(action) {
		...
		if (action.button) {
			...
			View.WRrecords = this.getSelectedWrIdObjects(action);
			this.openDialog('ab-bldgops-console-wr-bulk-update.axvw',{}, 1200, 1200, getMessage('bulkUpdateTitle'),true);
	...
	onComplete : function(action) {
		...
		if(!this.existPendingRequests()){
			...
			var completePanel = this.getCompletePanel();
			this.showQuickPanel(action, completePanel, completePanel.id=='completePanel'?500:1100, 200, getMessage('completeTitle') + ' ' + (action.button ? action.row.getFieldValue('wr.wr_id') : ''));
	 ...
	 * Open complete quick panel to do complete work request(s).
	 */
	getCompletePanel : function() {
		var panel  = this.completePanel;
		for(var i=0;i<View.WRrecords.length;i++){
			this.assignmentsCheckDS.addParameter('wrId', View.WRrecords[i]['wr.wr_id']);
			if(this.assignmentsCheckDS.getRecords().length>0){
				panel = this.completeOptionPanel; 
				break;
			}
		}
	...
	/**
	 * complete work request by cf.
	 */
	onCompleteCf : function(action) {
		View.WRrecords = this.getSelectedWrIdObjects(action);
		if(!this.existPendingRequests()){
			this.showQuickPanel(action, this.completeCfPanel, 500, 200, getMessage('completeTitle') + ' ' + (action.button ? action.row.getFieldValue('wr.wr_id') : ''));
	...
	/**
	 *  return work request by cf.
	 */
	onReturnCf : function(action) {
		View.WRrecords = this.getSelectedWrIdObjects(action);
		if(!this.existPendingRequests()){
			$('return_comments').value = '';
			this.showQuickPanel(action, this.returnPanel, 500, 200, getMessage('returnTitle') + ' ' + (action.button ? action.row.getFieldValue('wr.wr_id') : ''));
		}else{
			View.showMessage(getMessage('pendingRequestWhenReturn'));
		}
		...
		if(!this.existPendingRequests()){
			...
			if(cfRecord){
				for ( var i = 0; i < this.selectedWrRecordsForAction.length; i++) {
					...
					Workflow.callMethod('AbBldgOpsOnDemandWork-WorkRequestService-completeReturnedWrcfRecords', wrId);
	 ...
	 * Open view as dialog
	 */
	openDialog : function(viewName, restriction, width, height, title, maximize) {
	     if(!View.dialog){
	    	var config =  {
					width : width,
					height : height,
			...
			View.openDialog(viewName, restriction, false, config);
	     }
	},
