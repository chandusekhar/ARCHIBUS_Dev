...
var slaFilterController = View.createController('slaFilter', {
	...
	/**
	 * Request parameter filter restriction
	 */
	requestFilterRes : '1=1',

	/**
	 * Response parameter filter restriction
	 */
	responseFilterRes : '1=1',

	/**
	 * Workflow steps filter restriction
	 */
	workflowStepsFilterRes : '1=1',
	...
	/**
	 * Open add new SLA quick panel.
	 */
	onAddOnDemandSLA : function() {
		this.trigger('app:operation:express:sla:addNewSLA','OD');
	},
	 ...
	 */
	onAddPmSLA : function() {
		this.trigger('app:operation:express:sla:addNewSLA','PM');
	},
	...
	getFilterRestriction : function() {
		...
		this.requestFilterRes = this.getRequestFilterRes();

		// get response parameter filter restriction
		this.responseFilterRes = this.getResponseFilterRes();

		// get workflow steps filter restriction
		this.workflowStepsFilterRes = this.getWorkflowStepsFilterRes();
	...
	getResponseFilterRes : function() {
		...
		var responseFilterFields = [ 'helpdesk_sla_response.cf_id', 'helpdesk_sla_response.manager', 'helpdesk_sla_response.servcont_id',
		...
		if(Ext.get('bigBadFilter_dispatch_work_team').dom.checked){
			var workTeam = View.panels.get('bigBadFilter').getFieldQueryParameter('helpdesk_sla_response.work_team_id');
			responseFilterRes += " AND helpdesk_sla_response.work_team_id" + workTeam;
		}
		
		if(Ext.get('bigBadFilter_dispatch_supervisor').dom.checked){
			var supervisor = View.panels.get('bigBadFilter').getFieldQueryParameter('helpdesk_sla_response.supervisor');
			responseFilterRes += " AND helpdesk_sla_response.supervisor" + supervisor;
		}

		if (responseFilterRes != '1=1 AND 1=1 AND 1=1 AND 1=1 AND 1=1') {
