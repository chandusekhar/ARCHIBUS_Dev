...
View.createController('onDemandWorkflowTabStep', {
	...
	/**
	 * Show optional steps actions for PM
	 */
	showOptionalStepActionsForPM : function(level) {
		var controller = this;
		_.each(SLA_ALL_STEPS, function(step, index) {
			...
			for ( var i = 0; i < stepTypes.length; i++) {
				...
				if (type == 'notification' || type == 'verification')
	...
	showOptionalSteps : function(responseParameter) {
		...
		if (responseParameter.autoIssue) {
			...
			if (workRequestsOnly == '0' && 'PREVENTIVE MAINT' != View.controllers.get('editDetailsWizard').selectedSLA.requestParameters.probType) {
