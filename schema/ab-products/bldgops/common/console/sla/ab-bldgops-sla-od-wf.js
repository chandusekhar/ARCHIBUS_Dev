...
View.createController('onDemandWorkflowTab', {
	...
	events : {
		...
		/**
		 * Event handler for click assign cf radio .
		 * 
		 * @param event
		 */
		"click input[name='assignToCfRadio']" : function(event) {
			SLA_onChagneAssignToCfRadio(this.A_StepForm,event)
		},
	...
	afterViewLoad : function() {
		...
		if(Workflow.callMethod('AbBldgOpsOnDemandWork-WorkRequestService-checkSchemaExisting','helpdesk_sla_response', 'cf_role').value){
			SLA_moveFormField('A_StepForm', 'helpdesk_sla_response.cf_role', 'craftspersonRole');
			var cfRoleFiledCommand = this.A_StepForm.fields.get("helpdesk_sla_response.cf_role").actions.get(0).command.commands[0];
			cfRoleFiledCommand.dialogRestriction = "helpdesk_roles.step_type='craftsperson'";
			cfRoleFiledCommand.selectFieldNames = 'helpdesk_roles.role';
			cfRoleFiledCommand.visibleFieldNames = 'helpdesk_roles.role';
			cfRoleFiledCommand.sortValues = "[{'sortOrder':1,'fieldName':'helpdesk_roles.role'}]";
	    }else{
	    	jQuery('#craftspersonRole').hide();
	    	jQuery('#assignToCf').hide();
	    }
	...
	checkCfValidation : function() {
		var cfId  = this.A_StepForm.getFieldValue('helpdesk_sla_response.cf_id');
		if(cfId){
			var restriction = this.getSelectCfRestriction();
			if(this.cfSelectValueDS.getRecords(restriction + " AND (cf.cf_id = '" + cfId + "')").length > 0){
				return true;
			}else{
				// get work team and supervisor
				var workTeamId = this.A_StepForm.getFieldValue("helpdesk_sla_response.work_team_id").toUpperCase();
				var supervisor = this.A_StepForm.getFieldValue("helpdesk_sla_response.supervisor").replace(/\'/g, "''").toUpperCase();
				View.showMessage(String.format(getMessage('cfNotValid'), workTeamId + supervisor));
				return false;
			}
		}
	...
	loadResponseParameters : function(selectedSLA) {
		...
		if ('PREVENTIVE MAINT' != selectedSLA.requestParameters.probType) {
			...
			View.controllers.get('onDemandWorkflowTabStep').showOptionalStepActionsForPM();
		}
	},
	...
	hidePanelsAndFieldsForPmSla : function(selectedSLA) {
		...
		jQuery('#showRejectForm').hide();
		jQuery('#showStoppedForm').hide();
		jQuery('#showCancelledForm').hide();
	},
	...
	showFieldValues : function(responseParameter) {
		...
		this.A_StepForm.setFieldValue('helpdesk_sla_response.cf_role', responseParameter.cfRole);
		...
		if (responseParameter.cfId) {
			Ext.fly("assignToCf").dom.checked = true;
		} else {
			Ext.fly("assignToCf").dom.checked = false;
		}
		
		if (responseParameter.cfRole) {
			Ext.fly("assignToCfRole").dom.checked = true;
		} else {
			Ext.fly("assignToCfRole").dom.checked = false;
		}
		
		if (!responseParameter.cfRole && !responseParameter.cfId) {
			Ext.fly("assignToCf").dom.checked = true;
		}

		SLA_onChagneDispatchForm(this.A_StepForm);
		SLA_onChagneAssignToCfRadio(this.A_StepForm);
	...
	updateResponseParameters : function() {
		...
		responseParameter.cfRole = this.A_StepForm.getFieldValue('helpdesk_sla_response.cf_role');
