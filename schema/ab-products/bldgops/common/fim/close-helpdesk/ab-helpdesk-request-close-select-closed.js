var abHpdReqCloseSlktClosedController = View.createController("abHpdReqCloseSlktClosedController",{
	activityType: '',
	requestDateFrom: '',
	requestDateTo: '',
	...
	requestConsole_afterRefresh: function(){
		...
		var ifRefresh = ABHDC_getTabsSharedParameters()["refresh_from_ab_helpdesk_request_close_select_closed"]; 
		if( valueExists(ifRefresh)&& ifRefresh == true){
			ABHDC_getTabsSharedParameters()["refresh_from_ab_helpdesk_request_close_select_closed"] = false;
	...
	setRequestConsole: function(){
		...
		this.requestConsole.setFieldValue("activity_log.date_closed.from",this.requestDateFrom);
  		this.requestConsole.setFieldValue("activity_log.date_closed.to",this.requestDateTo);
  	}, 
	...
	requestConsole_onClear: function(){
		this.requestConsole.setFieldValue("activity_log.activity_type",'');	
 		this.requestConsole.setFieldValue("activity_log.date_closed.from",'');	
		this.requestConsole.setFieldValue("activity_log.date_closed.to",'');
 	...
 	saveRequestConsoleParameters: function(){
		...
		var requestDateFrom = this.requestConsole.getFieldElement("activity_log.date_closed.from").value;
		var requestDateTo = this.requestConsole.getFieldElement("activity_log.date_closed.to").value;
 	...
 	getRestriction: function(){
 		...
 		var dateRequestedFrom = this.requestConsole.getFieldElement("activity_log.date_closed.from").value;
		var dateRequestedTo = this.requestConsole.getFieldElement("activity_log.date_closed.to").value;
		...
		restriction.removeClause('activity_log.date_closed.from');
		restriction.removeClause('activity_log.date_closed.to');
		 
		if (dateRequestedFrom != '') {
			restriction.addClause('activity_log.date_closed', dateRequestedFrom, '&gt;=');
		}
		if (dateRequestedTo != '') {
			restriction.addClause('activity_log.date_closed', dateRequestedTo, '&lt;=');
		}
	...
	requestReportGrid_onArchive: function(){
		...
		try {
			var result = Workflow.callMethod('AbBldgOpsHelpDesk-RequestsService-archiveRequests', records);
		}catch(e){
