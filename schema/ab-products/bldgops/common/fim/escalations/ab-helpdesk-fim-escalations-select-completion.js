var abHpdFimEscalationsSlktCompletedController = View.createController("abHpdFimEscalationsSlktCompletedController",{
	...
	requestConsole_afterRefresh: function(){
		...
		var ifRefresh = ABHDC_getTabsSharedParameters()["refresh_from_ab_helpdesk_fim_escalations_select_completion"]; 
		if( valueExists(ifRefresh)&& ifRefresh == true){
			ABHDC_getTabsSharedParameters()["refresh_from_ab_helpdesk_fim_escalations_select_completion"] = false;
	...
	setRequestConsole: function(){
		...
		this.requestConsole.setFieldValue("activity_log.date_escalation_completion.from",this.requestDateFrom);
  		this.requestConsole.setFieldValue("activity_log.date_escalation_completion.to",this.requestDateTo);
  	}, 
	...
	requestConsole_onClear: function(){
		...
		this.requestConsole.setFieldValue("activity_log.activity_type",'');	
 		this.requestConsole.setFieldValue("activity_log.date_escalation_completion.from",'');	
		this.requestConsole.setFieldValue("activity_log.date_escalation_completion.to",'');
 	...
 	saveRequestConsoleParameters: function(){
		...
		var requestDateFrom = this.requestConsole.getFieldElement("activity_log.date_escalation_completion.from").value;
		var requestDateTo = this.requestConsole.getFieldElement("activity_log.date_escalation_completion.to").value;
 	...
 	getRestriction: function(){
 		...
 		var dateRequestedFrom = this.requestConsole.getFieldElement("activity_log.date_escalation_completion.from").value;
		var dateRequestedTo = this.requestConsole.getFieldElement("activity_log.date_escalation_completion.to").value;
		var status = this.requestConsole.getFieldElement("activity_log.status").value;
		...
		restriction.removeClause('activity_log.date_escalation_completion.from');
		restriction.removeClause('activity_log.date_escalation_completion.to');
		...
		if (dateRequestedFrom != '') {
			restriction.addClause('activity_log.date_escalation_completion', dateRequestedFrom, '&gt;=');
		}
		
		if (dateRequestedTo != '') {
			restriction.addClause('activity_log.date_escalation_completion', dateRequestedTo, '&lt;=');
		}
