 ...
 * @author lei
 */
var defProgramController = View.createController('defRMTypeCat', {
    ...
    //Operation Data Type //'CATEGORY','TYPE'
    operDataType: "",
    
    afterViewLoad: function(){
    	this.treeview = View.panels.get('cate_tree');
    	this.treeview.setTreeNodeConfigForLevel(1,           			// index of the tree level
                [{fieldName: 'doctype.doc_type'},   					// field value, full length
                 {fieldName: 'doctype.summary', length: 40}]);    	    // field value, trimmed
    },
    ...
    showMenu: function(e, item){
        ...
        var menutitle_newBuilding = getMessage("doctype");
    ...
    onAddNewButtonPush: function(menuItemId){
        ...
        if (this.curTreeNode) {
            ...
            if (nodeLevelIndex == 0) {
                cateId = this.curTreeNode.data["doccat.doc_cat"];
            }
            else {
                cateId = this.curTreeNode.data["doctype.doc_cat"];
            }
        }
        else {
        ...
        switch (menuItemId) {
            case "CATEGORY":
                ...
                break;
            case "TYPE":
                restriction.addClause("doctype.doc_cat", cateId, '=');
    ...
    cate_detail_onDelete: function(){
        ...
        this.commonDelete("regCatTab1DS", "cate_detail", "doccat.doc_cat");
    },
    type_detail_onDelete: function(){
        this.operDataType = "TYPE";
        this.commonDelete("reGTypeTab2DS", "type_detail", "doctype.doc_type");
    },
    ...
    setCurTreeNodeAfterUpdate: function(curEditPanel, parentNode){
        if (this.operType == "DELETE") {
            ...
            switch (this.operDataType) {
                case "CATEGORY":
                    var pkFieldName = "doccat.doc_cat";
                    break;
                case "TYPE":
                    var pkFieldName = "doctype.doc_type";
                    break;
            }
