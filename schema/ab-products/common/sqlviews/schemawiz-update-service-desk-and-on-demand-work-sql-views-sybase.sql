...
IF EXISTS (SELECT 1 FROM sysobjects WHERE name = 'activity_log_step_waiting' AND type = 'V') DROP VIEW activity_log_step_waiting;
IF EXISTS (SELECT 1 FROM sysobjects WHERE name = 'activity_logview' AND type = 'V') DROP VIEW activity_logview;
IF EXISTS (SELECT 1 FROM sysobjects WHERE name = 'hactivity_logmonth' AND type = 'V') DROP VIEW hactivity_logmonth;
IF EXISTS (SELECT 1 FROM sysobjects WHERE name = 'hwr_month' AND type = 'V') DROP VIEW hwr_month;
IF EXISTS (SELECT 1 FROM sysobjects WHERE name = 'wr_step_waiting' AND type = 'V') DROP VIEW wr_step_waiting;
IF EXISTS (SELECT 1 FROM sysobjects WHERE name = 'activity_log_hactivity_log' AND type = 'V') DROP VIEW activity_log_hactivity_log;
...
CREATE VIEW hactivity_logmonth as select *,
    cast(datepart(year,date_requested) as char(4))+'-'+cast(datepart(month,date_requested) as char(2)) as month,
    cast(datepart(year,date_requested) as char(4))+'-'+cast(datepart(week,date_requested) as char(2)) as week,
    ISNULL(assigned_to,vn_id) as service_provider,cost_estimated-cost_actual as cost_var from hactivity_log;
