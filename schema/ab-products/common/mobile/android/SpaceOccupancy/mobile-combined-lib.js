window.onerror=function(d,b,a){var c=function(){if(typeof Common.util.SynchronizationManager!=="undefined"){Common.util.SynchronizationManager.fireEvent("exception")}if((typeof Space!=="undefined")&&(typeof Space.SpaceDownload!=="undefined")){Space.SpaceDownload.fireEvent("exception")}},e;if(b&&a){console.log("[ERROR] URL: "+b+" Line: "+a)}d=d.replace("Uncaught","");d=d.replace("Error:","");if(d.indexOf("Type 'null' is not an object")!==-1){return}if(d.indexOf("Type 'undefined' is not an object")!==-1){return}if(d.indexOf("Unable to get property")!==-1){return}if(Ext.MessageBox&&Ext.Msg){Ext.Msg.alert("",d)}else{alert(d)}if(Common.log.Logger){if(b&&a){e=b+" Line: "+a+" "+d}else{e=d}Common.log.Logger.log(e,"error")}c();if(Ext.Viewport){Ext.Viewport.setMasked(false)}};!function n(t, e, o) {
    function i(u, f) {
        if (!e[u]) {
            if (!t[u]) {
    ...
    1: [function (n, t) {
        ...
        var o = t.exports = {};
        o.nextTick = function () {
            var n = "undefined" != typeof window && window.setImmediate, t = "undefined" != typeof window && window.postMessage && window.addEventListener;
