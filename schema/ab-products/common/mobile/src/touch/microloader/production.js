// Add ARCHIBUS Mobile namespace and initialize the appCacheData array.
// This provides a way for us to reference the Abm.appCacheData variable in the application code.
var Abm = Abm || {};
Abm.appCacheData = [];
...
(function(global) {
    ...
    function readAppDataFromFile(id, onCompleted, scope) {
        var fileName = id + '.abdata';
        Configuration.readFile(fileName, function(fileData) {
            var appCacheData;
            if(typeof fileData !== null && typeof fileData === 'object') {
                appCacheData = fileData;
            } else {
                appCacheData = JSON.parse(fileData);
            }
            Abm.appCacheData = appCacheData;
            onCompleted.call(scope);
        },function(error) {
            console.log('[ERROR] Read data in microloader failed');
            onCompleted.call(scope);
        });
    }

    function appDataGetItem(key) {
        var i;
        for(i=0; i<Abm.appCacheData.length; i++) {
            if(Abm.appCacheData[i].key === key) {
                return Abm.appCacheData[i].value;
            }
        }
        return null;
    }

    function appDataStoreItem(key, value) {
        ...
        for (i=0; i<Abm.appCacheData.length; i++) {
            if(Abm.appCacheData[i].key === key) {
                Abm.appCacheData[i].value = value;
                itemExists = true;
            }
        }
        ...
        if(itemExists) {
            ...
            Abm.appCacheData.push({key: key, value: value});
        }
    }

    /* End of ARCHIBUS  custom functions */
...
//                }
            ...
            msViewportStyle.appendChild(
                document.createTextNode(
                    "@media screen and (orientation: portrait) {" +
                        "@-ms-viewport {width: 560 !important;}" +
                    "}" +
                    "@media screen and (orientation: landscape) {" +
                        "@-ms-viewport {width: 800 !important;}" +
                    "}"
                )
            );
    ...
    Ext.blink = function(manifest) {
        ...
        //loadAppData(manifest.id, function() {
        readAppDataFromFile(manifest.id, function() {
            // Get a copy of the application id. This will be used in the Common.util.AppCacheManager class.
            Abm.appKey = manifest.id;
