/******************************************************************
	ab-room-reservation-filter.js
	some javascript API used in
	ab-room-reservation-filter.js are defined in
...
function gettingRecordsData()
{
	...
	for(var i = 1; i < iTotalRowsForFilter+1; i++)
	{
		...
		if((strValues == "") && (objOperator.value!="IS NULL" && objOperator.value!="IS NOT NULL"))
		{
			//not going on
			break;
		}
	...
	//even strReturned is empty, <restriction
	//type="parsed"></restriction> is necessary to be passed to server
