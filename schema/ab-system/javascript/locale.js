...
function SetLocaleCommonJSVariables(name, value){
	var strName = name.toUpperCase();
	var strValue = "" + value;
	if(strName == "CURRENCYSYMBOL")
		strCurrencySign = strValue;
	else if (strName == "DECIMALSEPARATOR")
...
function SetLocaleTimeJSVariables(name, value){
	var strName = name.toUpperCase();
	var strValue = "" + value;
	if(strName == "FORMAT" && strValue!="" ){
		//similar with date format
		timePattern = strValue;
		bDefaultTimeFormat = false;
	}else if(strName == "SHORT" && strValue!="" && bDefaultTimeFormat){
	    //XXX: H:mm -> HH:mm (24-hour format) but h:mm is 12-hour
