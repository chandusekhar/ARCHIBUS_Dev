/******************************************************************
	view-definition-form-content-table-filter.js
	some javascript API used in
	view-definition-form-content-table-filter.js are defined in
	...
	Javascript Api to set up filters in Tgrp.
	 ...
	 and setSerializedInsertingDataVariables(strSerialized); are in
	 common.js
	 some extr functions are defined in common-sort-visiblefields.js
 ******************************************************************/
...
function setupSelectedEunmValue(n)
{
	...
	if(objEnumList != null)
	{
		for(var i=0;i<objEnumList.length;i++)
		{
			if(objEnumList[i].selected)
			{
				var objValue	= objForm.elements['values'+n];
				objValue.value = objEnumList[i].value;
				break;
			}
		}
...
function onInitialLoad(abSchemaSystemGraphicsFolder)
{
...
function onSelectField(n)
{
	...
	{
		//operator is empty
		if(objForm.elements['operator'+n]!="")
			objForm.elements['operator'+n].value="";

		//value is empty
		if(objForm.elements['enumValues'+n].style.display=="")
		{
			objForm.elements['enumValues'+n].style.display="none";
			objValue.style.display="";
		}
		if(objForm.elements['values'+n].value!="")
			objForm.elements['values'+n].value = '';

		//set next Conjunction to AND????
		if(n==1)
		{
			if(objForm.elements['conjunction2'].value!="")
				objForm.elements['conjunction2'][0].selected=1;
		}
...
function gettingRecordsData()
{
	...
	for(var i = 1; i < iTotalRowsForFilter+1; i++)
	{
		...
		if((objOperator.value=="IS NULL" || objOperator.value=="IS NOT NULL")|| strValues!="")
		{
			...
			if(objOperator != null && !objOperator.disabled && objOperator.value != "")
			{
				...
				if(objOperator.value == "LIKE" || objOperator.value == "NOT LIKE")
					if(strValues!="" && strValues.indexOf("%")<0)
						strValues = strValues + "%";
			}
...
function onSelectV(strSerialized, iCounter)
{
	...
	if(strField != "")
	{
		...
		if(typeUpperCase == "JAVA.SQL.DATE"){
			Calendar.getController("values"+iCounter,abSchemaSystemGraphicsFolder);
			return false;
		}
...
function checkAndSetupSelectedFieldValueInput(objForm,n)
{
	...
	if(temp_array != null)
	{
		...
		for(var strStored in temp_array)
		{
			...
			objEnumValueInput[temp_i] = newObjItem;
			//????????????
			if(strStored == strTempValue)
				objEnumValueInput[temp_i].selected = 1;
			else
				objEnumValueInput[temp_i].selected = 0;
			temp_i = temp_i + 1;
		}
...
//input field)
function validationAndConvertionDateAndTime(n, bPageLoaded)
{
	var objForm = document.forms[afmInputsFormName];
	var strField = objForm.elements['field'+n].value;
	var objValue = objForm.elements['values'+n];
	if(strField != "" && objValue.value != "")
	{
		...
		if(typeUpperCase == "JAVA.SQL.DATE")
		{
			...
			if(bPageLoaded && field_value != null && field_value != "")
				dateArrayObj = field_value.split("-");
			else
				dateArrayObj = null;
			//validationAndConvertionDateInput() defined in date-time.js
		...
		else if(typeUpperCase == "JAVA.SQL.TIME")
		{
			...
			if(bPageLoaded && field_value != null && field_value != "")
				...
				TimeArrayObj = null;
			//validationAndConvertionTimeInput(tempObj, fieldID,
			//TimeArrayObj, sRequired, bShow, bReplaced) is defined in
			//date-time.js
...
function onclickDetailsButton(obj_button, textString)
{
	var temp_obj=document.getElementById('SQLRestrictionsArea');
	if(temp_obj!=null)
	{
		if(temp_obj.style.display=='none')
		{
			temp_obj.style.display='';
			obj_button.value=textString+"<<";
		}
		else
		{
