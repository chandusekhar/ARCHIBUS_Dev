...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="task" translatable="true">Task</xsl:variable>
						...
						<xsl:when test="$hasChildren='false'">
							<xsl:choose>
								<xsl:when test="count($afmAction)>0">
									...
									<td nowrap="1">
										<img alt="{$task}" src="{$abSchemaSystemGraphicsFolder}/ab-icon-tree-task.gif" BORDER="0" ID="IMG_{concat($nodeID,'_',position())}" />
									</td>
									<xsl:choose>
