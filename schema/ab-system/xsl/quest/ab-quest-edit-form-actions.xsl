							...
							<xsl:choose>
								<xsl:when test="$type='delete'">
									<xsl:if test="not(@enabled)">
										<input class="AbActionButtonFormStdWidth" type="button" value="{title}" title="{tip}" onclick='return onQuestDelete("{$afmTableGroupID}","{@serialized}","_self",true)'/>
									</xsl:if>
								</xsl:when>
								<xsl:otherwise>
									<xsl:choose>
										...
										<xsl:otherwise>
											<input class="AbActionButtonFormStdWidth" type="button" value="{title}" title="{tip}" onclick='return onQuestSave("{$afmTableGroupID}","{@serialized}","_self",true)'/>
										</xsl:otherwise>
									</xsl:choose>
								</xsl:otherwise>
							</xsl:choose>
						...
						<xsl:otherwise>
							<xsl:choose>
								<xsl:when test="$type='addNew'">
									<xsl:if test="not(@enabled)">
										<input  class="AbActionButtonFormStdWidth"  type="button" value="{title}" title="{tip}" onclick='return onQuestAddNew("{$afmInputsForm}","{@serialized}","_self",false)'/>
									</xsl:if>
								</xsl:when>
								<xsl:otherwise>
									
									<input  class="AbActionButtonFormStdWidth"  type="button" value="{title}" title="{tip}" onclick='return onQuestCancel("{$afmTableGroupID}","{@serialized}","_self",false)'/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:otherwise>
					</xsl:choose>
