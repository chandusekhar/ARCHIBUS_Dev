...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">	
	...
	<xsl:variable name="totalRowsForFilter" select="2"/><!--select="6"/-->
	...
	<xsl:template match="/">
		...
		<head>	
			...
			<script language="JavaScript" src="{$abSchemaSystemJavascriptFolder}/ab-common-filter.js"><xsl:value-of select="$whiteSpace"/></script>
			<!-- template: SetUpLocales is in locale.xsl-->
			<xsl:call-template name="SetUpLocales"/>
		</head>
		<body onload='onInitialLoad()' class="body" leftmargin="0" rightmargin="0" topmargin="0" bottomMargin ="0">
						...
						<tr align="center">
							<td align="center">
								...
								<table  class="bottomActionsTable">
									...
									<tr><td>
										<!-- afmAction -->
										<xsl:variable name="OKAction" select="//afmTableGroup/afmAction[@type='applyRestriction1']"/>
										<table  class="bottomActionsTable">
											<tr><td>
												<table><tr>
													<td nowrap="1" valign="bottom" class="AbActionButtonForm">
														<A title="{$OKAction/title}" href="#" onclick='sendingDataFromHiddenForm("","{$OKAction/@serialized}","{$OKAction/@frame}","{$OKAction/subFrame/@name}",true,""); return false;'>
	...
	<xsl:template name="SelectValue">
		...
		<xsl:if test="count($afmAction) &gt; 0">
			<input class="selectValue_AbActionButtonFormStdWidth" name="selectV{$iCounter}" title="Click to Display Selection List" type="button" value="{$afmAction/title}" onclick='onSelectV("{$afmAction/@serialized}",{$iCounter}); selectValueInputFieldID="values{$iCounter}";'/>	
		</xsl:if>
	</xsl:template>
