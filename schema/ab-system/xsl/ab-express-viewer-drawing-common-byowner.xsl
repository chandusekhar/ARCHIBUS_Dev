...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:variable name="dhlRmLayers">
     ...
     <xsl:for-each select="/*/afmTableGroup/dataSource/data/records/record">Z-BL-HL-<xsl:value-of select="@bl.bl_id" />;Z-RM-HL-<xsl:value-of select="@rm.bl_id" />-<xsl:value-of select="@rm.fl_id" />-<xsl:value-of select="@rm.rm_id" />;</xsl:for-each>

     <xsl:if test="$isDynamicHighlights!='true' and $strStdTable!=''">
       <xsl:value-of select="$layerName"/>;
     </xsl:if>

     <xsl:if test="$isDynamicHighlights='true'">
      <xsl:if test="$dhlLayerUpper='EM'">Z-RM-DHL;</xsl:if>
      <xsl:if test="$dhlLayerUpper!='EM'">Z-<xsl:value-of select="$dhlLayerUpper"/>-DHL;</xsl:if>
     </xsl:if>
  </xsl:variable>
  ...
  <xsl:template name="drawingBody">
	...
	<xsl:variable name="absoluteAppPath_expressViewer5" select="//preferences/@absoluteAppPath"/>
	<xsl:variable name="isDwfViewer7" select="//preferences/@dwfViewer7"/>
	<body   class="body" onload="setUpDWFFileLink();loadDwfViewer('{$width}','{$height}','{$absoluteAppPath_expressViewer5}', '{$isDwfViewer7}');loadDrawing();" leftmargin="0" rightmargin="0" topmargin="0">
		<xsl:call-template name="DWF_HTML_BODY_CONTENT" />
		<div><xsl:call-template name="drawingLegend" /></div>
	</body>
  </xsl:template>
