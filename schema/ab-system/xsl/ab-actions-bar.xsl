		...
		<table  align="center" class="bottomActionsTable">
			<tr><td>
				<table align="center"><tr align="center">
					<xsl:if test="$TYPE=1">
						<td><input  class="AbActionButtonFormStdWidth" type="button" value="{$OK/title}" title="{$OK/title}" onclick='sendingDataFromHiddenForm("","{$OK/@serialized}","{$OK/@frame}","{$OK/subFrame/@name}",true,"{$newWindowSettings}")'/></td>
						<td><input  class="AbActionButtonFormStdWidth" type="button" value="{$CANCEL/title}" title="{$CANCEL/title}" onclick='sendingDataFromHiddenForm("","{$CANCEL/@serialized}","{$CANCEL/@frame}","{$CANCEL/subFrame/@name}",false,"{$newWindowSettings}")'/></td>
					</xsl:if>
					<xsl:if test="$TYPE=0">
				...
				<table  align="center" ><tr  align="center">
					<xsl:if test="$TYPE=1">
						<xsl:for-each select="$ACTIONS">
							<td><input class="AbActionButtonFormStdWidth" type="button" value="{title}" title="{title}" onclick='sendingDataFromHiddenForm("","{@serialized}","{@frame}","{subFrame/@name}",true,"{$newWindowSettings}")'/></td>
						</xsl:for-each>
					</xsl:if>
					<xsl:if test="$TYPE=0">
						<xsl:for-each select="$ACTIONS">
